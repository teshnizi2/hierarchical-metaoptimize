...
NOTHING ELSE** — §0 item 15 **[SUPERSEDED LABEL: §1 A4(b) — 286 named the wrong section; §1 A4 is the GroupNorm
row and contains no such clause. Corrected at CORRECTIONS **287**; 286's own fix table carries the same mislabel and
is not rewritten]** and §2's claim paragraph read *"below ~1e-2"* / *"below about 1e-2"* and now read
...
data file was touched, so the corpus commit below is unchanged**; **at 287 ONE MORE INSTANCE OF THE SAME PHRASE,
IN §5's T22, WHICH 286 DID NOT SWEEP FOR** — same fix, same reason, again no number —
against `master` at
...
6. **One configuration, one horizon, three seeds, axes one at a time.** CIFAR-100 (plus one CIFAR-10 association cell),
   ...
   α-scaled decay only — **α-independent weight decay is untested everywhere.**
...
15. **EVERY NUMBER IN THIS FILE SITS AT ONE WEIGHT-DECAY VALUE, THAT VALUE IS FAR OFF COMMON PRACTICE, AND THE LADDER
   ...
   **nothing inside that unrun decade may be named** (`LADDER-IS-FOUR-POINTS`); *"the collapse exists only at 0.1"*
   ...
   identifies is the **CONJUNCTION** of scalar grouping with an α-scaled decay at or near 0.1: at 0.1 the grain matters
   enormously, at 1e-2 and below neither grain reaches the collapse bar. *"A property of wd 0.1, not of scalar
   grouping"* is equally forbidden. **α-INDEPENDENT weight decay is still untested in either direction**
   (`DECOUPLED-NOT-TESTED`, descoped with reasons at 281.2), so R2 of §10.6 stays unanswered. The α-scaled decay
...
the α-scaled base weight decay with BOTH grains run IN BATCH at every rung**, plus one carrier companion. Registered at
...
with an α-scaled decay at or near 0.1 — at 0.1 the grain matters enormously and below it neither grain reaches the bar
— so neither *"a general granularity effect"* nor *"a property of wd 0.1 independent of grain"* is writable.
(4) **α-INDEPENDENT decay is untested in either direction** and **which route the decay acts through is not separated**.
...
§3.3d) has landed and it is ADVERSE: the collapse this paragraph is about is **present at α-scaled weight decay 0.1 and
...
> α-scaled weight decay applied to normalisation scales — a configuration common practice avoids (He et al.,
...
> network. **That configuration is one extreme value of one axis, and we have now measured the axis**: at the α-scaled
...
> follows is therefore a sentence about α-scaled weight decay at or near 0.1, and must be written carrying that
...
> their own step-size group recovers the whole scalar-to-layerwise gap, and removing their α-scaled decay alone removes
...
their SCOPE, which is now known rather than assumed. (ii) **It says nothing about α-INDEPENDENT weight decay**
...
4. **α-scaled weight decay on normalisation scales is a precondition** of the whole phenomenon, at **four** grains and on
...
* **α-independent weight decay is untested** at every cell; every result above is about **α-scaled** decay.
...
|---|---|---|
| T1 | "This is a configuration pathology, not a finding." | **Largely conceded, and it must be written that way.** `LIMITS-PREP` §6 named this as the single biggest weakness and its own conditional has fired on the weight-decay branch. Momentum 0.99 is exonerated (E18); α-scaled WD on the norm scales is not — it is a **precondition** (E8–E11, E19). The correct statement of the ceiling is **"we did not meet the collapse at any cell tested with the decay off"**, never "a reader will not meet it". |
...
| T11 | "The parent reports scalar (SGDm, Lion) working on ImageNet." | **Open, and it is the sharpest external counterweight.** Our IN-489 scalar sits at 1.00. The obvious untested confounder was momentum 0.99 — now exonerated (E18) — which makes α-scaled WD the live candidate, but we have run nothing on ImageNet and cannot. Parent §7.3 also reports blockwise no better than scalar there. |
...
| **T22** | **"Everything you have is at α-scaled weight decay 0.1, which is two hundred times what anyone uses, and you never varied it."** | **THE SECOND HALF IS NO LONGER TRUE, AND THE FIRST HALF IS NOW MEASURED RATHER THAN CONCEDED — AND IT LANDS.** *(REWRITTEN at CORRECTIONS 285, the `cwd5` landing; it previously read "CONCEDED WITHOUT QUALIFICATION … there is no ladder".)* **The ladder has been run**: `cwd5`, 27 runs, seeds {146, 147, 148}, four rungs — **0.1 / 1e-2 / 1e-3 / 5e-4** — with **both grains in batch at every rung**, FINAL `THRESHOLD-W1-W2` (§1 A5, §3.3d, E28–E30). **The collapse is PRESENT at 0.1 (`G_W1` +46.0700 pp = +87.46 SE, `COLLAPSE`) and ABSENT at all three lower rungs** — at 1e-2 the scalar arm sits 35.2203 pp above that rung's collapse bar, and the co-primary **`G_W4` = −4.5267 pp = −8.59 SE** says that at the standard CIFAR value the layerwise arm is, if anything, slightly *below* the scalar one. **So the referee is RIGHT and the section must say so first**: this is a corner case, not a hazard; the mechanism line is a diagnostic of ONE extreme configuration; and the audit's scalar cells carry the disclosure that they sit at a decay ~200× the standard value at which that configuration is broken. **What the campaign gains is a QUALIFICATION it can defend instead of an assertion it could not**: the scalar row is a granularity measurement **at 1e-2 and below** — the three rungs we ran — and at 0.1 it is measuring a broken configuration. *(Wording brought into line at CORRECTIONS **287**: this sentence still read "below about 1e-2" after 286 fixed the same phrase in §0 item 15 and in §2's claim paragraph. It is the THIRD instance of 285.6 W1's error class — it excludes the tested 1e-2 rung, which is a measured `NOGAP` rung, and softens a BRACKETING PAIR into an approximate boundary inside the unrun decade. No number, bar, state, contrast, branch word or stamp moves.)* **FOUR BOUNDS travel with the answer**: the ladder has FOUR points, so the transition is a BRACKET between 1e-2 and 0.1 and nothing inside that decade may be named (`LADDER-IS-FOUR-POINTS`); `NOGAP` is a bound, not a measured equality, and `G_W2` is itself **not resolved** (−2.07 SE, ±2 SE upper end 0.04 pp from zero — DESCRIPTIVE / UNSURE); what is identified is the **CONJUNCTION** of scalar grouping with a decay at or near 0.1, so neither *"a general granularity effect"* nor *"a property of wd 0.1 independent of grain"* is writable; and **α-INDEPENDENT weight decay is STILL untested in either direction** (`DECOUPLED-NOT-TESTED`, descoped with reasons at 281.2 — **§10.6 R2 remains unanswered, and a referee who asked for both halves of the chair's gate has had one answered**). The two old mitigations still stand and still do not answer the charge: **wd 0.1 is the parent's own value**, which `LIMITS-PREP` §2.3 records the mechanism cell as matching (that section's Appendix-Table-4 alignment marked **UNSURE**); and the intervention removes decay from **1,536 parameters or fewer while conv and linear weights keep wd 0.1 throughout** (`CONV-LINEAR-WD-KEPT`), so the finding is not "large decay hurts". |
...
1. **Framing (the live one, and it has changed shape).** Momentum 0.9 does **not** remove the collapse; weight decay 0
   ...
   T20). Is the section still a **mechanism section**, stated as conditional on α-scaled weight decay — or is it a
   ...
   ***AND AT CORRECTIONS 285 THE QUESTION NARROWED AGAIN, BECAUSE THE LADDER LANDED ADVERSE.*** *`cwd5` says the
   collapse is present at α-scaled wd 0.1 and absent at 1e-2, 1e-3 and 5e-4 (§1 A5, §3.3d, T22). **The "mechanism
...
2. **Which experiment gets bought, and in what order? There are now two candidates and they answer different
   referees.** **O-14, the weight-decay ladder plus an α-independent arm**, answers the *configuration* charge — §10's
   reading is that **without it the subsection is not safe to publish in any form**, because every number sits at
   α-scaled wd 0.1, roughly 200× common practice, with no rung between that and zero (§0 item 15, T22). **O-13, the
   ...
   **My reading, offered as a recommendation and not a decision: O-14 first**, because it protects the section's right
   ...
   CORRECTIONS 281 → 285 — `cwd5`, 27 runs, 18.5092 GPU-h, FINAL `THRESHOLD-W1-W2`, **adverse**; its α-independent half
   ...
   want the α-independent arm reinstated despite 281.2's reasons, so that R2 is answered too? (c) does he want **O-2**
...
4. How should the parent's §7.3 ImageNet result (scalar SGDm+Lion works; blockwise no better than scalar) be positioned
   against our IN-489 scalar at 1.00, now that momentum 0.99 is exonerated and α-scaled WD is the live candidate (T11)?
...
2. **Reframe it.** Not as a mechanism, not as a localisation result: as **a diagnostic that protects the audit's own
   scalar arm**.
3. **Buy one experiment first.** The weight-decay ladder plus an α-independent arm (**O-14**). His position is that
   ...
   ***THE LADDER HALF HAS NOW BEEN RUN, AND IT SAYS THE CHARGE LANDS ANYWAY*** *(CORRECTIONS 285; §1 A5, §3.3d): the
   ...
   when it was first written. **The α-independent half was never bought** (`DECOUPLED-NOT-TESTED`, descoped with reasons
...
from the launcher's side rather than the referee's. (ii) **The α-independent half of the chair's ask is DESCOPED with
...
**We can now answer that question, and the answer is neither embarrassing nor a mechanism.** Removing α-scaled weight
...
likely to be lifted into the paper*** *(CORRECTIONS 283.6 **W7**): it holds* ***AT THIS CELL AND AT α-SCALED WEIGHT
...
512-parameter BatchNorm scale from the α-scaled decay moves the same configuration from 22.8213 to 67.9567. Whether
...
> *At the audit's CIFAR-100 ResNet-18 cells, **and at the α-scaled weight decay of 0.1 at which those cells are
...
| **R2** | **"α-independent or α-scaled?"** *(written as "Decoupled or coupled?" before CORRECTIONS 292's naming fix)* | **NO, and it is now a DESCOPED question rather than a forgotten one** (281.2): the base update is α-scaled by construction and no α-independent λ is equal-strength when the per-step dose is `wd · a` with `a` learned. `DECOUPLED-NOT-TESTED`. **A referee may still insist, and would be entitled to.** |
...
3. **O-14 as a gate is a strong claim, and the gate has now been run.** *(REWRITTEN at 285.)* The chair's position
   ...
   **My own view, offered as a judgement and labelled as one: yes, because the section's real job is to qualify the
   ...
   an audit row a reader is entitled to discount and no answer to why.** **But the α-independent half is still descoped,
