...
| the one fix | **WORDING, not a number.**  §1 A4(b) and §2's claim paragraph read *"below ~1e-2"* / *"below about 1e-2"*; both now read ***"at 1e-2 and below"***.  The old form excluded the tested 1e-2 rung AND softened a BRACKETING PAIR into an approximate boundary inside the **unrun** decade — the same error class as 285.6 **W1**.  285.7's copy of the phrase stands as written and is **superseded**, since CORRECTIONS entries are not rewritten |
