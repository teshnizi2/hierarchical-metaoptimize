...
\begin{abstract}
MetaOptimize \citep{sharifnassab2025metaoptimize} meta-learns one step size per parameter
group and reports that finer partitions help inconsistently. That inconsistency has never been
...
that question up with 2{,}173 runs ($\approx$1{,}625 GPU-hours; 1{,}724 admissible) on CIFAR-10
and CIFAR-100 --- ResNet-10, -18, -34 and -50 across the corpus, and ResNet-18, -34 and -50 in
...
of that $Q$} (between-base $Q = 95.12$ on 3 df). \textbf{We do not claim the base optimiser is
...
accounting for half of it is \textbf{not} --- power against $\Astat = \Dstat/2$ is 0.46. We
...
once: of the 420 admissible runs in the partition families, 408 carry a \textbf{Lion}
...
interior optimum; \S\ref{sec:related} states exactly what is left.
...
\item \textbf{A pre-registered permutation null, reported with its resolution}: at fixed count
  \emph{and} fixed per-tensor size multiset, membership is worth $-0.009 \pm 0.157$ pp,
  95\% CI $[-0.317, +0.298] = [-55\%, +51\%]$ of $\Dstat$, MDE 0.440 pp. This
  \textbf{bounds} alignment's contribution rather than eliminating it, in one batch at one
  design point, and we report the registration defect that limits it (\S\ref{sec:alignment}).
...
CSV's \texttt{plateau} column and is \textbf{not used}: two of this project's headlines were
withdrawn for quoting it (Appendix~\ref{app:discrepancy}.2), and it is banned as a primary.
...
24 is in a count-matched arm of any cell reported in this paper; seven are truncated snapshots of
the in-flight \arm{rp1} batch (\S\ref{sec:inflight}), which contributes no number here} --- see
the attrition ledger in \S\ref{sec:repro}, where attrition
inside the primary contrasts is zero. Of 2{,}173 rows, \textbf{1{,}724 are admissible}.
...
\item \textbf{\S\ref{sec:moderator} pools across step-size clip boxes, and
  ...
  the state that flatters it. On the eleven-cell pool of the previous draft, partitioning $Q$ by
...
are addressed by four batches submitted while this draft was being written. Two of the four ---
\arm{bm2} and \arm{sm4} --- have since completed and been scored by running their registered
scorers \textbf{unedited}, and their verdicts are folded into \S\ref{sec:moderator},
\S\ref{sec:prescription}, \S\ref{sec:tail}, \S\ref{sec:normalisation} and \S\ref{sec:threats}.
Two --- \arm{rp1} and the \arm{hz3} seed-5 trio --- are not scored, and \textbf{no number from
either enters any claim in this paper}. The registrations are stated here in full regardless of
...
R1 & \arm{rp1} & 24 & the alignment null's power \textbf{and} its permutation-seed confound
     (\S\ref{sec:alignment}) & \texttt{c97\_rp1\_score.py} & complete on disk, \textbf{not
     scored} \\
...
\textbf{R1 is complete on disk and deliberately unscored.} All 24 \arm{rp1} runs reach 100 epochs
in their own \texttt{.out} series. Seven of the twenty-four were ingested into
\texttt{results/all\_runs.csv} mid-flight and their rows still carry the partial epoch counts of
that snapshot, so the run table is behind the disk for this batch. We have not run
\texttt{analysis/c97\_rp1\_score.py}, and \textbf{no \arm{rp1} number appears anywhere in this
paper}, including in \S\ref{sec:alignment} where it would matter most. The registration stands as
written, including the clause that binds us: if \arm{rp1} returns an interval that still spans
half of $\Dstat$, the alignment leg is to be reported as \textbf{underdetermined}, not as a null.

\textbf{R2 is queued.} The three seed-5 jobs sit \texttt{PENDING} on the same congested partition
on which the original \arm{hz3} seed-3 and seed-4 runs waited 16 and 23 hours, so a wait is
expected rather than anomalous. STANDING RULE 20's post-launch ARGS check is owed on them the
moment they start.
...
cells the maximum is \arm{ml2}'s $+0.337$ and the previous draft's $+0.34$ bound still holds.
...
\arm{rl3} rungs and the two \arm{nl1} bases, which share a batch, and \textbf{\arm{hz3} (row 9),
...
reaching the contrast at all. We therefore quote no $\Dstat$ for it here or anywhere else; its
arm means are in Appendix~\ref{app:arms} and its role in the design is discussed under T7 in
\S\ref{sec:threats}. Consequently \textbf{every cell in this paper uses BatchNorm.}
...
238 uniform-chunk, \arm{nodewise1d} and \arm{permnode} runs in the corpus outside the in-flight
\arm{rp1} batch are admissible, and the
sixteen batches involved contribute 272 runs of which 272 are admissible
(\S\ref{sec:repro}, Table~\ref{tab:attrition}). Two further count-matched contrasts exist in the
...
Three of the fourteen were not available to the previous draft. Two are \arm{bm2}, the registered
...
base optimiser & base state, from the runs' own \texttt{ARGS} line & cells & batches &
  pooled $\Dstat$ (pp) & within-level $Q$ \\
\midrule
SGD     & no momentum, no second moment & 2 & 2 & $\mathbf{+1.000 \pm 0.067}$ & 0.17 / 1 \\
RMSProp & second moment 0.999, no momentum & 2 & 2 & $\mathbf{+0.720 \pm 0.128}$ & 1.37 / 1 \\
SGDm    & momentum 0.99, no second moment & 8 & 7 & $\mathbf{+0.556 \pm 0.045}$ & 4.21 / 7 \\
AdamW   & momentum 0.9 + second moment 0.999 & 2 & 2 & $\mathbf{+0.189 \pm 0.052}$ & 1.61 / 1 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Between base optimisers, $Q = 95.12$ on 3 df ($p = 1.7\times10^{-20}$) --- 92.8\% of the
total; within levels, $Q = 7.36$ on 10 df ($p = 0.69$).} \textbf{Every level of this moderator
...
previous draft, \emph{three} of the four levels were singletons, so $Q_{\mathrm{between}}$ was
...
figure on the previous draft's eleven cells was 88.4\% of a $Q$ of 36.40, and against the legacy
...
partition's 92.8\%, and leaves a residual $Q$ of \textbf{34.64 on 9 df ($p\ 6.9\times10^{-5}$)}
inside the momentum-present group --- the SGDm-to-AdamW gap survives the collapse. \textbf{The
...
$+0.189 \pm 0.052$ with $Q = 1.61$ on 1 df. The previous draft kept \arm{sm3} out of this
...
$p\ 1.7\times10^{-20}$; the within-level remainder is 7.36 on 10 df, $p$ 0.69. On the previous
draft's eleven cells the same partition read 32.20 of 36.40 (88.4\%), and against the legacy
...
\paragraph{What would settle it, and it is running.}
A single batch closes all three of the statistical gaps above: \textbf{R1 --- \arm{rp1},
\arm{permnode} at three permutation draws $\times$ six seeds with the permutation seed decoupled
from the run seed, plus a six-seed \arm{nodewise} arm; 24 jobs (\S\ref{sec:inflight}).} No patch
is needed --- \arm{permnode<S>} already accepts an explicit $S$; \arm{c77} simply passed the run
seed. That design (i) takes $\se(\Astat)$ from 0.157 to $\approx 0.09$, which brings the MDE to
$\approx 0.25$ pp, below half of $\Dstat$ and inside the pre-registered band for the first time;
(ii) yields a \textbf{two-way variance decomposition} that separates permutation variance from
seed variance, so limit 2 above becomes a measured number instead of a caveat; and (iii) supplies
the replication $\Astat$ has never had. Until it reports, the sentence this paper is entitled to
is the bounded one above and not a stronger one. \textbf{If \arm{rp1} returns an interval that
still spans half of $\Dstat$, the alignment leg is to be reported as underdetermined rather than
as a null}, and we register that in advance here and in that batch's own scorer.
...
\textbf{The scope line is not where the previous draft put it.} Under an AdamW base with a
...
from one batch at one design point --- and out-of-sample predictability of $\Dstat$
...
fourteen, so that the effect of enlarging the family is visible rather than absorbed. Each $p$ is the two-sided Welch test on the two arm means; $p\ (z)$ is the normal
...
freedom and we say so elsewhere in this paper. The enlarged family is also heterogeneous where the
pre-specified one was not --- fourteen-cell $\Gstat$ pool $+0.093 \pm 0.022$, $Q = 28.25$ on 13 df
($p = 0.0084$), against the twelve-cell $+0.067 \pm 0.023$, $Q = 18.21$ on 11 df ($p = 0.077$) ---
and the two cells that make it so are the two new AdamW-base ones.
...
The fifteen-test family is heterogeneous ($\Gstat$ pool $+0.128 \pm 0.020$, $Q = 42.98$ on 14 df,
$p = 8.6\times10^{-5}$), driven
...
M4 was narrowed in the previous draft to ``an SGDm base with a Lion meta-optimiser'', with AdamW
...
single-batch reading the previous draft quoted, the independent \arm{nl1}-versus-\arm{aw1}
...
The previous draft read the four-level base decomposition as agreeing with M6 about which
...
$-0.021 \pm 0.077$ ($t\ -0.27$) to the $-0.161 \pm 0.067$ above. The paper is weaker here than the
previous draft claimed, and in the direction the level model predicts.
...
$+2.55$ pp --- nearly three times the error of simply predicting the corpus mean. A covariate that
...
$\Dstat$ and an MDE of 0.440 pp $= 76\%$ of $\Dstat$. It excludes alignment as the principal
...
without one is a claim we cannot support. R1 (\arm{rp1}, 24 jobs, in flight,
\S\ref{sec:inflight}) is designed to halve the interval and to decompose the two variance sources.
...
flat budget effect into a growing one (\S\ref{sec:budget}). Both are one-line mistakes that
...
\arm{nodewise1d}, \arm{chunk*} or \arm{permnode*}. There are \textbf{420} of them, of which
\textbf{408 are meta $=$ Lion and 12 are meta $=$ RMSProp}; the twelve are \arm{sm4}. Before
\arm{sm4} the count was 408 of 408. (An earlier version of this paper quoted ``367 of 367'' here.
...
once, at $n = 3$ v $3$, with an interval spanning $[-55\%, +51\%]$ of $\Dstat$ and with permutation
variance confounded with seed variance: \textbf{a moderate alignment contribution is not
excluded}, and the batch that would exclude it (R1, \arm{rp1}, 24 jobs) is running rather than
reported.
...
cells, and on the eleven cells the previous draft pooled it moved the estimate from
...
\texttt{python3 code/c98\_reproduce.py --census} measures it:
...
The deposit is \textbf{6.2 MB in 137 files}, with \texttt{MANIFEST.md5} covering every one of them
and \texttt{make verify} checking all 137 against it. \textbf{It has no DOI, and this paper prints
...
\textbf{rows in \texttt{results/all\_runs.csv}} & \textbf{2{,}173} & 1{,}624.8 &
  2{,}150 carry a wallclock \\
...
$-$ \texttt{window\_ok = 1}, \texttt{complete = 0} & $-24$ & &
  truncated runs, 7 of them in-flight \arm{rp1} snapshots; \texttt{complete} is
  \texttt{epochs\_done} $\ge 0.95 \times$ requested \\
\textbf{admissible} & \textbf{1{,}724} & 1{,}558.4 & the gate of Eq.~\ref{eq:adm} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The 449 inadmissible rows are not failures; they are short-budget probes.} All 425
\texttt{window\_ok = 0} rows requested $\le$ 20 epochs (392 at 20, 33 at 2--5) and so could never
carry a five-epoch plateau at their requested budget. Only 24 of the 449 are full-budget runs, and
those are the truncated ones. By granularity, the 449 are: \arm{layerwise} 136, \arm{weightwise}
111, \arm{nodewise} 108, \arm{resnet18\_blocks} 56, \arm{scalar} 30, \arm{permnode} 7 (the
in-flight \arm{rp1} snapshots), no-partition baseline 1 ---
...
count-matched contrast contribute \textbf{272 runs, of which 272 are admissible}. More strongly:
\textbf{every} uniform-chunk, \arm{nodewise1d} and \arm{permnode} run in the corpus outside the
in-flight \arm{rp1} batch --- 238 rows
--- is admissible, so no count-matched cell could have been lost to the gate even in principle.
(Counting \arm{rp1} the figure is 249 of 256; the seven exceptions are its mid-flight snapshots,
and they enter no contrast here.)
...
\paragraph{Compute.}
2{,}150 runs carry a wallclock; they total \textbf{1{,}625 GPU-hours} over 29 distinct nodes and
...
3$\times$ the budget; and its variation across configurations has a candidate moderator that we
can decompose but not identify --- splitting the fourteen same-contrast ResNet-18 cells by base
optimiser leaves them homogeneous inside every level ($Q\ 7.36$ / 10 df, $p\ 0.69$) and accounts
...
$[-0.317, +0.298] = [-55\%, +51\%]$ of $\Dstat$, against an MDE of 0.440 pp. A moderate alignment
contribution is not excluded and a further batch is running to bound it more tightly. The surviving
...
seeds, which replicated both levels and is in \S\ref{sec:moderator}; and R4, the second-moment
corner, which refuted its mechanism and is in \S\ref{sec:normalisation}. Two remain: the alignment
replication with the permutation seed decoupled (R1, complete on disk and deliberately unscored)
and the box- and hardware-matched budget trio (R2, queued). To those we add two experiments the
...
(\S\ref{sec:threats} T9, \S\ref{sec:budget}). Both replacements match the project's own later
...
write time: \textbf{2{,}173 rows, 1{,}724 admissible, 1{,}625 GPU-hours} summed over the 2{,}150 runs
...
is 6.2 MB, carries an md5 manifest for every file, and re-derives every number
...
regenerates \textbf{two} of the ten scorer verdicts we quote in full (\texttt{c87\_rl3},
\S\ref{sec:rule11}; \texttt{c87\_hz3}, \S\ref{sec:budget}), \textbf{two} in part
(\texttt{c81\_cc1}'s $\Dstat$ and $\Gstat$ legs but not \S\ref{sec:field}'s field block;
\texttt{c83\_gc1}'s S1 but not its arm-asymmetry guard), and \textbf{six} not at all --- including
