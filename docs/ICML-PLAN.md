...
| 1.6 | The route is identified: weight shrink vs meta-trace, and α-independent decay | **MISSING — LAUNCHED, NOT LANDED** (cycle-5 audit, 2026-09-22): patch `PATCH_DECAYROUTE` PROVED (CORRECTIONS 305, job 5081090, 410/0, `$WS/harness_cdr1`); **`crd1`** (307): 18 jobs **5081273–5081290**, seeds 196–198, 6 arms (`shrink_only` / `trace_only` at wd 0.1, `alpha_indep:3.15e-4` at wd 0; Λ = csh1's onset-window Q_M, NOT the initial-shrink match, which is degenerate at dose ratio 1.8e-4, 307.3); 12.3 GPU-h exp / 54 bound; 0 / 18 started.  The audit cell's α-independent arm is `cai1` (306, §4a).  `DECOUPLED-NOT-TESTED` (281.2; one flag moves both routes) **STANDS until both land** | 3 variants at κ 0.1 on ResNet18_c100: shrink-only, trace-only, α-independent (matched on initial per-step shrink) × 2 grains × 3 seeds | 18 runs ≈ 13 GPU-h | 40-80-line opt-in patch like `PATCH_DECAYMASK` + inertness proof | registration | MUST (reviewers will ask) |
...
### 4a. THE LIVE QUEUE (CORRECTIONS 297) — at most three, ranked; **all three REGISTERED AND LAUNCHED 2026-09-22 (299–301), plus 1.14 (`cvl1`, 303) and the two α-independent batches (`cai1` 306, `crd1` 307); ranks 2 and 3 LANDED (309 `crt1` WEAKENED-TO-TIE, 310 `csh1` HORIZON-DOES-NOT-REPRODUCE); off-queue 1.14 LANDED (312 `cvl1` VAL-DIFFERS-UNRESOLVED + SELECT-SAME, PARTIAL)** *[was: none landed (`crt1` 36 / 36 COMPLETED, unscored)]*
