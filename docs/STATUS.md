...
| still owed | nothing from this batch.  **`cwd5` (281, 27 runs, jobs 5052142–5052169) is COMPLETE and UNSCORED**, and **its ingest is BLOCKED by 281.12's RULE 16 defect** — `CARW2` both prints a `DECAY_MASK: on` witness and deviates on an ARGS value, and `corpus_exclusions.py` lists one witness per run; the verified fix is **owed under its own CORRECTIONS number BEFORE that ingest** |
