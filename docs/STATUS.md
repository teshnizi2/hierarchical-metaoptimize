...
Updated 9 Sep 2026 (**cycle 148 RECONCILIATION**, 191: THREE tracks, THREE entries (`188` `cdep1`, `189` `cvg1`, `190` `ciso2`), **no collision**; **36 jobs in flight on `alice2`, 9 started, 0/36 `RUN_DONE`, NOT ONE NUMBER QUOTABLE**; RULE 20 `--batch-consistency` **PASS 9/9 on `cdep1`** and **UNVERIFIED (0 started) on `ciso2` and `cvg1`**; all three RULE 21 margins re-derived by wall clock (**+231 s / +152 s / +415 s**); ≈**45.9 GPU-h** committed across the account; corpus **2,761**, unchanged). *[superseded: Updated 9 Sep 2026 (**cycle 148, Track A**, 188: `cdep1` REGISTERED AND LAUNCHED — the IDENTITY-vs-DEPTH control `187.4` named as `ciso1`'s binding limitation; 18 jobs, RULE 21 margin **+231 s**, coverage 6/18, **nothing landed, no number quotable**; corpus **2,761**, unchanged).]* *[superseded: Updated 9 Sep 2026 (**cycle 147**, 187: `ciso1` LANDED, attacked five ways, INGESTED — `FINAL: UNRESOLVED-PARTIAL | ISO-59-NEITHER | ISO-3-PINS | CTRL-59-PINS | ONE-61-FREE | PREMISE-REPLICATES | ONE-RESCUES | SPECIFIC:CTRL-NULL | ISO-RESCUES-BUT-59-NOT-FREE`).]* Detail lives here; chat stays short. *[superseded: Updated 8 Sep 2026 (**cycle 146 reconciliation**, 186: Tracks A (184) and B (185) reconciled, no collision; H-DOMINATE's position stated; `ciso1` 7/15 running, no number quotable).]*
Authority: `docs/CORRECTIONS.md` (highest number wins, now **191**) *[was 188 at the Track-A write; `189` and `190` landed within twenty minutes of it]* > `docs/FINDINGS.md` > everything else.
Manuscript and deposit are both at **`2f4fd9a`** (parent `58c0c85`). **Nothing under `paper/` touched this cycle** (`git status --porcelain paper/` empty). *[191: re-verified after all three tracks committed — `git diff --stat d39d147..HEAD -- paper/` empty and `git status --porcelain -- paper/` empty.]*
Draft = `paper/paper.tex` + `paper/DRAFT-v4.md` (**76 pp**). Corpus = **2,761 rows** (**+15: `ciso1` INGESTED at 187 — keyed on `(run, job_id)`: 15 added, 0 changed, 0 removed, 0 other-batch; 12.82 GPU-h by the `.out` minutes lines, 13.02 h by `sacct` Elapsed. `alice2` queue after the ingest: all 15 `ciso1` jobs 4925518-4925532 `COMPLETED 0:0`, nothing submitted or cancelled; `alice` not accessed**). *[superseded wording follows:]* Corpus = **2,746 rows** (**+6: `ctd1` INGESTED — keyed on `(run, job_id)`: 6 added, 0 changed, 0 removed, 0 other-batch; 5.08 GPU-h. `alice2` queue at 23:31 CEST: 15 `ciso1` jobs 4925518–4925532 from Track B's cycle-146 commit `be15a15` (7 running + 8 pending, Submit 23:27:52) — not this track's, not scored; `alice` not accessed**). *[superseded wording follows:]* Corpus = **2,740 rows** (**+60: `cru1` INGESTED this cycle** — keyed on `(run, job_id)`: **60 added, 0 changed, 0 removed**). **BOTH CLUSTER QUEUES ARE EMPTY. ALL THREE BATCHES OF THE 24-HOUR PUSH ARE COMPLETE, SCORED AND INGESTED.** *[cycle 144 wording; superseded at 182/183: `alice2` now holds the 6 `ctd1` jobs (4924919–4924924), 4 running + 2 pending at 21:23 CEST, 0/6 `RUN_DONE`; `alice` not accessed.]* `cru1` cost **39.2000 GPU-h** (60 jobs, ingested `wallclock_min`). **Nothing was submitted or cancelled this cycle, and no job this campaign did not submit was touched on either account.** *[cycle 144; cycle 145 Track B submitted `ctd1` (182); the verification entry 183 submitted and cancelled nothing.]* *[186, 23:40 CEST 8 Sep: `alice2` = 15 `ciso1` jobs, 7 RUNNING (started 23:28:49) + 8 PENDING, 0/15 `RUN_DONE`, 0 tracebacks; `alice` not accessed.]*
...
## CYCLE 166 (LANDING) — **CORRECTIONS 317 (`crd1`, ICML-PLAN row 1.6's route half) + 318 (`cai1`, row 1.6's α-independent half, THE ONE INGEST OF BOTH, and what both do to the paper).  BOTH LAND.  THE COLLAPSE TRAVELS THROUGH THE α-SCALED WEIGHT SHRINK (WITH ITS UNDECAYED DIRECT TERM) AND NEEDS THE DECAY TO MOVE WITH α; AT THE AUDIT's OWN CELL α-INDEPENDENT DECAY IS RUN AND UNREADABLE AT BOTH STANDARD-RECIPE Λ, SO THE HEADLINE IS NEITHER CONFIRMED NOR REMOVED.  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,551 / 3608.3 GPU-h / 420 EXCLUSIONS.**

| item | value |
|---|---|
| `crd1` verdict | **`ROUTE-IS-WEIGHT-SHRINK \| INDEP-NOGAP \| S-COLLAPSE+T-NOGAP+I-NOGAP`** + 12 stamps + the 16 registered bounds (FINAL verbatim at 317.2) — the registered prediction (307.6, prior 0.25) |
| `cai1` verdict | **`AI-UNREADABLE \| SC-PARTIAL \| I5-BOXBOUND+I4-UNHEALTHY \| I5-SCALAR-UNRESOLVED+I4-SCALAR-COLLAPSED`** + 11 stamps + the 7 registered bounds (FINAL verbatim at 318.2) — the shared one-rung / unreadable bin (prior 0.12); `AI-SURVIVES` (0.15) and `AI-ARTEFACT-VANISHES` / `-REVERSES` (0.30 together) both NOT reached |
| bounds first (`crd1`) | sufficiency not necessity; S's trace keeps the direct term −a·wd·w UNDECAYED, so no sentence may say "the shrink alone" (307.11's parenthesis travels with every route sentence); interventions, not hypergradients; T's null rests on a reference only **+5.2460 pp** above REF_MIN 55; ONE Λ, constant schedule, 13.37× the source's cumulative shrink, `ALPHA-INDEP-NOT-A-PURE-ROUTE-CONTROL`, both I grains under-fitting (TRAIN 70.95 / 71.40); no in-batch OFF anchor; every COLLAPSE / NOGAP a bound; κ 0.1 cell, one network, 100 epochs, three seeds |
| bounds first (`cai1`) | the seven registered bounds (one cell; two Λ bracket and locate nothing; the α-scaled reference between batches; `ROTATIONAL-EQUILIBRIUM-CONFOUND`; shrink-matched not ratio-matched, Λ/a 1.9–74× at I5 and 10–179× at I4; constant schedule; floor readings are bounds), plus: I5 unreadable **only through the 5 % RECORD box gate** (chunk777 7.02–7.38 % of records; ≈ 0.04 % of its groups in the window; nodewise box-free), I4 unreadable **through health** (77.42–80.78, all four box-bound), σ the in-batch 0.490126 (**175.1 %** above the floor), the scalar reading UNTUNED (311 A5) |
| numbers (`crd1`) | G_S **+47.2933** pp (+90.99 SE) [+46.2538, +48.3328], R 0.3249, 3/3 seeds at the bar → COLLAPSE; G_T **−3.4747** (−6.69 SE) → NOGAP, scalar ahead; G_I **+0.6393** (+1.23 SE) → NOGAP.  Levels 22.7653 / 70.0587 (S), 63.7207 / 60.2460 (T), 62.9967 / 63.6360 (I); σ the frozen 0.636566, SE 0.519754.  Measured: SRS shrink 3.1446e-4 (peak 5.7684e-4, cumulative 1.1782, turns 8700 ×3), TRS factor 0.99968586 horizon 3183, AI* Λ 3.1500e-4 cumulative 15.75; dose 1.0439 / 0.0000 / 0.5701 |
| numbers (`cai1`) | levels I5 / I4: ch 90.4470 / 79.0840, nd 90.1250 / 77.4190, k01 90.7655 / 80.7820, kL 90.3980 / 79.8105; D_I5 **+0.3220** [−0.3711, +1.0151] (printed, not read), D_I4 **+1.6650**; T at I5 +0.3185 / +0.6405 (min T − 2 SE −0.3746), at I4 +1.6980 / +3.3630 below the health bar; G_I5 −0.3675, G_I4 −0.9715; SE 0.346571; G-SHRINK on all 320,000 records: 5.000002e-05 (I5) and 5.000000e-04 (I4) |
| licences (verbatim) | `crd1`: *"the alpha-scaled weight shrink with the trace factor removed collapses the scalar step size to at most half its in-batch layerwise level, and the learned trace factor with the weights undecayed does not"*; *"With alpha-INDEPENDENT decay at the onset-matched per-step dose … scalar and layerwise stay within the 10 pp bar"* (full sentences at 317.8).  `cai1`: primary *"both LAMBDA rungs are unreadable: no partition reading under alpha-independent decay; the levels themselves are the finding."*; scalar *"at least one LAMBDA's scalar reading is unresolved or gated: report both rungs' intervals; no combined scalar sentence."* |
| joint reading (307.7) | from the two FINAL lines' tokens only: the trace route is insufficient in BOTH forms at the mechanism cell — constant γ (`csh1`) and the LEARNED horizon (`crd1` T) — so T-C is excluded as a sufficient account in both, as bounds; and neither the constant horizon nor the horizon plus a constant shrink collapses |
| gates | by-hand `argsline_guard` audits 18 / 32 clean, PASS; RULE 20 at full coverage PASS on both halves for both batches (two passes each, the refute pass re-running from its own stage) BEFORE any number was read; scorers `8684f086…` (231 lines) and `fb766d93…` (53 lines) UNEDITED rc 0 on both hosts, re-run post-ingest (only the path echoes and the DISCLOSURE line 3,115 → 3,131 differ; both FINALs byte-identical); independent parsers 57/0 and 45/0, byte-identical on both hosts; my own stdlib re-derivations agree; both refutes **hold** (0 refutations; `crd1` 4 wording fixes, `cai1` one wrong sub-claim inside a bound replaced — the I4 box onset IS the maximum Lion rate under float32, re-derived here — plus 3 wording fixes; all applied; no RULE 16 defect) |
| ingest | `fecd462`, ONE ingest of both: 3,501 → **3,551** (added **50** = 18 + 32, changed 0, removed 0, 0 of 133,038 cells changed, no proof job); `CORPUS-EXCLUSIONS.tsv` 370 → **420** (crd1 18 at CORRECTIONS 317: 12 one-kind + 6 two-axis, 308.4's reading; cai1 32 two-axis at 318; both byte-identical to their PROPOSED files); `--check` exit 0 PASS (25 batches, **decay-route 50 True — the first rows under 308's kind**, two-axis 19 → 57, ARGS witness 171 → 209, SIGMA_R18ALL 0.636566 df 274 unchanged); `c98b` exit 0, 618/618 |
| **WHAT IT DOES TO THE PAPER (317.10, 318.10)** | **Collapse section:** the route sentence, with the 307.11 parenthesis and `SUFFICIENCY-NOT-NECESSITY`; T-C excluded in BOTH forms; the collapse is a property of α-scaled decay (PyTorch AdamW's default form), not of decay as such, at this cell.  **Audit headline:** UNCHANGED — neither survives, weakens nor becomes an artefact; it keeps its "at α-scaled decay" qualifier and **is not removed**; what may be added is the levels (77.4–80.8 % at a recipe-matched shrink; 90.1–90.8 % at the ten-times-smaller one; chunk777 at the box on 7.0–7.4 % of records).  **`DECOUPLED-NOT-TESTED`:** retired for the collapse section at the mechanism cell only; NOT lifted for the headline.  **ICML-PLAN 1.6 → PARTIAL — LANDED (317, 318).**  Venue TMLR; 0.6–0.7 not re-estimated |
| docs | CORRECTIONS **317** and **318** appended ('Next free' 319); MASTER-TABLE rows **245** / **246** appended, header 3,551 / 3608.3, appended rows 33 → **35**, tally 181 → **183**, `wallclock_min` rows 3,486 → **3,536**, `c73` exit 0; `ICML-PLAN` amended (317 + 318 paragraph, row 1.6, §4a heading and both off-queue rows); this table |
| still running (not read) | `crt2` (45 jobs 5083795–5083839) — untouched |
| site | see the site row of this cycle's notebook import (rows MT245 / MT246) |
| discipline | ZERO GPU; **`alice` NOT contacted**; nothing under `paper/` read; no clone / checkout / worktree of the campaign repo; nothing downloaded; no job submitted or cancelled; the synced run files kept in `../runs_alice2` (the committed sha files cite them) |

## CYCLE 165 (LANDING) — **CORRECTIONS 315 (`g3b`, ICML-PLAN §4a rank 1 and row 1.16, its ONE ingest, and what it does to the paper).  `g3b` LANDS: ON CIFAR-100 THE AUDIT's 0.1 SIGN REPRODUCES IN BATCH; AT 5e-4 THE COUNT-MATCHED CONTRAST IS UNREADABLE (THE CHUNK771 ARM IS BOX-BOUND), SO NO SURVIVE / VANISH SENTENCE; SCALAR BEATS BOTH PARTITIONS AT 5e-4, UNTUNED; THE DENOMINATOR HOLDS WITH THE MetaOptimize ARM AT 5e-4 (1.16 PARTIAL — LANDED FOR `cdn1`, NOMINAL-VALUE FORM).  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,501 / 3561.7 GPU-h / 370 EXCLUSIONS.**

| item | value |
|---|---|
| `g3b` verdict | **`UNRESOLVED-BOXBOUND-W4 \| SCALAR-BEATS-BEST \| DENOM-HOLDS \| W1-SURVIVES+W4-BOXBOUND`** + 6 bounds + 5 stamps (FINAL verbatim at 315.2) |
| bounds first | one CIFAR-100 cell, two rungs, never pooled with `cgw1`; α-scaled decay only; 5e-4 a nominal value (W4 arms 38.9–709.8× below a standard recipe's shrink, up to 18.2× apart among themselves); denominator between batches, SGD lr chosen on test (1.14); **chW4 box-bound on all 4 seeds** (rec_hi ≤ 0.0796, ≤ 0.027 % of groups at the edge from epoch ≈ 92; onset UNSURE), kLW4 box-bound too; scalar reading untuned (311 A5); σ in-batch 0.431036 (57.6 % above the floor); ch / nd W4 levels below every predicted band |
| numbers | D_W1 **+1.2170** (+3.99 SE) SURVIVES, `ANCHOR-MATCHES-POOL`; D_W4 **+0.0645** [−0.5451, +0.6741] BOXBOUND; Tch / Tnd_W4 **+6.0345 / +6.0990** pp (19.8 / 20.0 SE); G_W4 −1.9870; GAPch_W4 **+14.5857** (+44.61 SE; descriptively +5.7117 at 0.1, matching `cdn1`'s +5.6993); levels ch 71.8810 / 63.0070, nd 70.6640 / 62.9425, k01 35.1490 (collapsed) / 69.0415, kL 70.7495 / 67.0545 |
| licences (verbatim) | primary: "a primary-rung partition arm is box-bound; the contrast is not an audit replication.  Itself a fact: at 5e-4 the step sizes reach the box."  scalar: "… no audited partition beats a single shared step size -- cgw1's reading holds on the audit's second dataset." (cgw1's UNTUNED reading; the 309.8 clause travels with its +2.6–2.8 pp).  denom: "… the denominator deficit is not an artefact of running the MetaOptimize arm at 0.1 (ICML-PLAN 1.16), at this cell, between batches, with the SGD arm's lr chosen on the test set (1.14)." (full sentences at 315.8) |
| gates | by-hand `argsline_guard` audit owed at 299.12: 32 clean PASS; RULE 20 full coverage PASS (32/32, two passes); scorer `f21f57bd…` UNEDITED rc 0 on both hosts (84 lines), re-run post-ingest (only path echoes differ; disclosure 3,115 unchanged); independent parser 33/0 byte-identical on both hosts; my own stdlib re-derivation agrees; refute **holds** (0 refutations; 6 wording / disclosure fixes F1–F6, all applied; no RULE 16 defect) |
| ingest | `4f7e191`: 3,469 → **3,501** (added **32**, changed 0, removed 0, 0 of 131,822 cells changed, no proof job); `CORPUS-EXCLUSIONS.tsv` 354 → **370** (16 W4 `ARGS_WD_BASE` rows, byte-identical to the PROPOSED file, registered at 315); `g3b-` into `bin/PROTECTED.txt`; `--check` exit 0 PASS (23 batches, ARGS witness 171, SIGMA_R18ALL 0.636566 df 274 unchanged); `c98b` exit 0, 618/618 |
| **WHAT IT DOES TO THE PAPER (315.10)** | The 0.1 sign is reproduced in batch on both datasets.  **The 5e-4 partition question still rests on ONE readable cell** (`cgw1`'s interval); for CIFAR-100 only the box fact may be stated.  **Scalar row:** untuned, scalar above both partitions on CIFAR-10 (+2.6–2.8, at least in part a tuning artefact there) and CIFAR-100 (+6.03 / +6.10, tuning untested there); the CIFAR-10 headline stays "ties (a bound) after re-tuning".  **1.16 PARTIAL — LANDED (315) for `cdn1`:** the κ confound is not supported in its nominal-value form (lowering the MetaOptimize arm's decay widens the gap), realised-decay match untested, `cau1` / `cuc1` not re-run.  Venue TMLR; 0.6–0.7 not re-estimated |
| docs | CORRECTIONS **315** appended ('Next free' 316); MASTER-TABLE row **244** appended, header 3,501 / 3561.7, tally 180 → **181**, `c73` exit 0; `ICML-PLAN` amended (315 paragraph, row 1.16, §4a rank 1 LANDED); this table |
| still running (not read) | `cai1`, `crd1`, `crt2` — untouched |
| site | notebook `2928fcb` on `main` (fast-forward of `11a8788`; pins campaign `12a4c5d`, ingest `4f7e191`): **MT244 Open** (as MT240 / MT243); 181 records / 163 research, 3,501 runs, outcomes 56 / 41 / 40 / 26; 16 one-kind `ARGS_WD_BASE` rows (ARGS-value runs 155 → 171); phase-20; **four later-evidence relations added from 315.10 (e)** (MT240 / MT191 / MT199 → MT244 `replicates`, MT175 → MT244 `qualifies`; 44 in all, every quote found); npm test 225 / 225 (tests first: mech10 8 tests failed before the import), test:data 127 OK with all four inputs, no skips; export --check, copy:check, dates:check (4,757), tsc and build PASS; `package --verify` NOT run (its `npm ci` fetches from the registry), offline substitute passed (ZIP extracted with the reused node_modules of the `site-mt241` worktree: 225 / 225 and build); GitHub commit status `success`, Production deployment 6594600195 `success` (via `gh api` only; no Vercel URL opened) |
| discipline | ZERO GPU; **`alice` NOT contacted**; nothing under `paper/` read; no clone / checkout / worktree of the campaign repo; no notebook site or Vercel URL opened; nothing downloaded; registration-era `~/stage_g3b*` on alice2 KEPT (cited by 299) |

## CYCLE 164 AUDIT — **CORRECTIONS 314 (zero GPU): 312 / 313 audited; ALL GATES PASS; one mis-citation (A1), one optimistic attachment narrowed (A2), licence-fragment precision (A3), stale queue states (A4).  312's verdict, 1.14 PARTIAL and the scalar row STAND.  `g3b` 32 / 32 COMPLETED, unscored.**

| item | value |
|---|---|
| gates | origin == local; 312 / 313 over their stubs, numeric order; MASTER-TABLE 243 lines (header bracketed, row 243 appended); corpus 3,469; exclusions 354 / 22 batches; `--check` exit 0; `c73` exit 0; site `11a8788` deployment `success`, MT243 Open; 40 / 40 later-evidence quotes found in their cited entries |
| paper effect of 312 | LICENSED; A2: `cvl1`'s held-out SC state is `cvl1`'s own (45k, in batch), not `cgw1`'s +2.6–2.8 pp; A3: the draft carries the FULL registered licence sentences (312.8), the selection one with its "(grain x decay)" scope clause |
| queue (14:12Z, read-only) | `g3b` 32 / 32 COMPLETED (landing owed); `crd1` 9 / 18 COMPLETED; `cai1` 7 running / 25 pending; `crt2` 45 / 45 pending (5083795–5083839, names and seeds as 313) |

## CYCLE 164 (LANDING) — **CORRECTIONS 312 (`cvl1`, ICML-PLAN 1.14, and its ONE ingest, and what it does to the paper).  `cvl1` LANDS: THE AUDIT CELL RE-READ ON A HELD-OUT SPLIT CHANGES THE TWO PARTITION STATES BUT NOT RESOLVABLY (NO SENTENCE EITHER WAY); SC IS THE SAME STATE ON BOTH READERS; VALIDATION SELECTS THE SAME CONFIGURATION.  1.14 IS PARTIAL (ms / α0 NOT RE-SELECTED).  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,469 / 3528.2 GPU-h / 354 EXCLUSIONS.**

| item | value |
|---|---|
| `cvl1` verdict | **`VAL-DIFFERS-UNRESOLVED \| SELECT-SAME \| W1-TUNDECIDED/VSURVIVES+W4-TUNDECIDED/VVANISHES+SC-TSCALAR-BEATS-BEST/VSCALAR-BEATS-BEST`** + 7 bounds + 11 stamps (FINAL verbatim at 312.2) |
| bounds first | one cell, two rungs; **ms / α0 NOT re-selected** (ms 1e-4 was chosen on test, 302.1); trains on 45,000 images; one split; **primary seed-fragile** (drop s187 → `VAL-AGREES`; VAL W1 clears 2 SE by 0.0719); the modal token (prior 0.45) licenses no sentence; α-independent decay not tested; VAL W4 VANISHES is a bound |
| numbers | D_W1 TEST **+0.2590** UNDECIDED / VAL **+0.4330** SURVIVES, gap +0.1740 not resolved; D_W4 TEST **+0.1660** UNDECIDED / VAL **−0.0920** VANISHES, gap −0.2580 not resolved (paired SE 0.2012); SC k01 − ch / nd TEST +2.81 / +2.98, VAL +3.15 / +3.06 pp, gap +0.2525 not resolved; both readers pick **kLW1** (VAL margin +0.0940 pp); σ in-batch 0.2476 (TEST) / 0.2554 (VAL) |
| licences (verbatim) | primary: "the change is not distinguishable from reader noise at this seed count; report both states and the paired intervals; no sentence that the ranking does or does not hold on validation."  selection: "selecting on validation would have picked the same one, among these 8." |
| gates | by-hand `argsline_guard` audit owed at 303.10: 32 clean PASS; RULE 20 full coverage PASS (32/32, two passes); scorer `85c73e21…` UNEDITED rc 0 on both hosts, re-run post-ingest (only path echoes differ; disclosure 3,115 unchanged); independent parser 40/0 byte-identical on both hosts; refute **holds** (0 refutations; 3 licence overreaches + 3 precision fixes, all applied; split shas re-derived from the labels; no RULE 16 defect) |
| ingest | `5545d88`: 3,437 → **3,469** (added **32**, changed 0, removed 0, 0 of 130,606 cells changed, no proof job); `CORPUS-EXCLUSIONS.tsv` 322 → **354** (16 W1 `VAL_SPLIT` one-kind + 16 W4 two-axis `ARGS_WD_BASE`, byte-identical to the PROPOSED file, registered at 312); `--check` exit 0 PASS (22 batches, completeness 218/218, two-axis 19, ARGS witness 155, SIGMA_R18ALL 0.636566 df 274 unchanged); `c98b` exit 0, 618/618 |
| **WHAT IT DOES TO THE PAPER (312.10)** | **1.14 closes for its RE-REPORTING half at the audit cell only (PARTIAL).**  The draft states that every audit number is a TEST reading with ms chosen on test (302.1), reports `cvl1`'s W1 / W4 states with their paired intervals and the selection licence verbatim, and may NOT say any ranking holds or fails on validation.  Scalar row unchanged: "ties (a bound) after re-tuning" (309; `cvl1` runs `crt1`'s untuned M2).  The 0.1 sign: `ar1`'s ms-3e-4 reading (+0.697, 109.2) is a TEST-read check against the ms-selection worry; the in-batch 45k TEST D_W1 is UNDECIDED (a caution, non-gating).  Denominator and collapse section untouched.  Venue TMLR; 0.6–0.7 not re-estimated |
| docs | CORRECTIONS 312 written over its stub (its text entered origin inside the `crt2` track's commit `e445db3`, from the shared working tree; byte-identical, noted at 312.11); MASTER-TABLE row **243** appended, header 3,469 / 3528.2, tally 179 → **180**, `c73` exit 0; `ICML-PLAN` amended (312 paragraph, row 1.14 PARTIAL, §4a off-queue 1.14 LANDED); this table |
| still running (not read) | `g3b`, `cai1`, `crd1` — untouched; `crt2` (313) registered by its own track |
| site | notebook `11a8788` on `main` (fast-forward of `1ec1906`; pins campaign `a3d422a`, ingest `5545d88`): **MT243 Open** (as MT240; bears on MT240, which keeps Open); 180 records / 162 research, 3,469 runs, outcomes 56 / 41 / 40 / 25; new patch kind `VAL_SPLIT` (16 W1 rows) and 16 two-axis W4 rows; phase-19; no later-evidence relation added (312 states its bearing in other words); npm test 216 / 216, test:data 123 OK with all four inputs, no skips; export --check, copy:check, dates:check (4,718), tsc and build PASS; `package --verify` NOT run (its `npm ci` fetches from the registry), offline substitute passed (ZIP extracted with the reused node_modules: 216 / 216 and build); GitHub commit status and Production deployment `success` (via `gh api` only; no Vercel URL opened) |
| discipline | ZERO GPU; **`alice` NOT contacted**; nothing under `paper/` read; no clone / checkout / worktree of the campaign repo; no notebook site or Vercel URL opened; nothing downloaded; `~/stage_cvl1_land` on alice2 KEPT (cited for re-runs) |

## CYCLE 163 (LANDING) — **CORRECTIONS 309 (`crt1`) + 310 (`csh1` and the ONE ingest of both, and what both do to the paper).  BOTH BATCHES LAND.  THE SCALAR-BEATS-BOTH ROW IS WEAKENED TO A TIE AFTER RE-TUNING (NOT REFUTED, NOT ROBUST); T-C IS EXCLUDED AS A SUFFICIENT RIVAL OF THE COLLAPSE, FOR A CONSTANT γ.  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,437 / 3501.9 GPU-h / 322 EXCLUSIONS.**
...
| `csh1` bounds first | constant γ, not the learned horizon; sufficiency, not necessity; two γs bracket; **cell P's reference only +1.97 pp over REF_MIN** (a bound on readability, not on nearness to collapse); every NOGAP a bound; one cell, κ 5e-4, 100 ep, 3 seeds; `crd1` not run |
| `csh1` numbers | G = L − k: A **−4.0127**, M **−5.6273**, P **−3.6780** pp (scalar ahead everywhere; 0/9 seeds at a collapse bar); levels G1S 72.4673 / G1L 68.4547, GMS 66.2980 / GML 60.6707, GPS 60.6487 / GPL 56.9707; RHO_D ≤ 0.0454 (dose absent); descriptive: γ < 1 costs both grains 6–12 pp and they under-fit |
| gates | RULE 20 full coverage PASS on both (36/36, 18/18; two passes each); scorers `7fe5ca76…` / `ba36f00b…` UNEDITED rc 0 on both hosts, re-run post-ingest (only path echoes differ; corpus disclosure 3,115 does not move — every new row is listed); independent parsers 22/0 and 44/0, byte-identical on both hosts; both refute passes **hold** (5 + 6 fixes, 0 refutations; no RULE 16 defect) |
| ingest | `5db62be`: 3,383 → **3,437** (added **54**, changed 0, removed 0, 0 of 128,554 cells changed, no proof job); `CORPUS-EXCLUSIONS.tsv` 268 → **322** (crt1 36 + csh1 18 `ARGS_WD_BASE`, witnesses generated from each run's ARGS line; csh1 `looks_like` = `k01` / `kL`, registered at 310); `--check` exit 0 PASS (ARGS witness 139, 75/75 standard-cell rows listed, SIGMA_R18ALL 0.636566 df 274 unchanged); `c98b` exit 0, 618/618 |
| **WHAT IT DOES TO THE PAPER (310.10)** | **scalar row: "untuned +2.6–2.8 pp; after re-tuning, ties the better partition (a bound)", at this cell, with the α-scaled, under-decay, grid-edge and σ-margin qualifiers — no longer a headline win.**  Audit headline otherwise as 297 (sign at 0.1, 5e-4 interval).  **Collapse section: gains the T-C sentence (310.8) — supports its causal sentences against one rival in one form, undercuts nothing**; the learned horizon waits for `crd1`.  Venue TMLR, 0.6–0.7 (judgement, not re-estimated) |
| docs | MASTER-TABLE rows **241–242** appended, header 3,437 / 3501.9, tally 177 → **179**, `c73` exit 0; `ICML-PLAN` amended (309 / 310 paragraph, §4a ranks 2–3 LANDED); this table; Venue row note |
| still running (not read) | `cvl1`, `g3b`, `cai1`, `crd1` — untouched; `crd1` lands the 307.7 joint rows with `csh1` |
| site | notebook `2274dbd` on `main` (pins campaign `a53bce1`): MT241 **Mixed** (crt1; bears on MT240, which keeps Open), MT242 **Mixed** (csh1, as MT237); 179 records, 3,437 runs, outcomes 56 / 41 / 40 / 24; npm test 197 / 197, test:data 119 OK no skips, export --check / package --verify / dates:check PASS; GitHub commit status and Production deployment `success` (checked via `gh api` only; no Vercel URL opened) |
| owed to the draft (`paper/` NOT touched) | the scalar row rewritten as a tie with its four qualifiers; the abstract / audit headline made consistent (abstract gate); the collapse section's T-C sentence |
| audit (CORRECTIONS 311) | independent audit of 308–310 PASSES (counts, checks, traceability, site `2274dbd` success); two miscounted refute headings (309.6, 310.6) corrected; 310.10(c)'s "can no longer be killed by a one-hyperparameter retune" narrowed (α0 > 1e-2 untested, grid edge); the untuned +2.6–2.8 pp must carry "at least in part, a tuning artefact"; `g3b` / `cvl1` / `cai1` run `crt1`'s untuned M2 config |
| discipline | ZERO GPU; read-only `sacct` on `alice2`; **`alice` NOT contacted**; nothing under `paper/` read; no clone / checkout / worktree of the campaign repo; no notebook site or Vercel URL opened; nothing downloaded |

## CYCLE 162 (PATCH + CODE DEBTS + TWO REGISTRATIONS; CYCLE-5 AUDIT) — **CORRECTIONS 304 (exclusions `VAL_SPLIT` kind, zero GPU), 305 (`PATCH_DECAYROUTE`, PROVED), 306 (`cai1`) + 307 (`crd1`).  α-INDEPENDENT DECAY IS NOW RUNNING, NOT RUN: 50 JOBS, ≈39.3 GPU-h EXPECTED, 118 HARD BOUND.  `DECOUPLED-NOT-TESTED` STANDS.  NOTHING LANDED, NOT ONE NUMBER QUOTABLE.  CORPUS 3,383, UNCHANGED.**

| item | value |
|---|---|
| 304 (Track E, 374c3a4) | `VAL_SPLIT` = 9th kind in `corpus_exclusions.py`; test 239/0 Mac + alice2; 59/65 invariance items byte-identical, rest additive or by design; csh1's γ needs no kind (CSV column, now tested C45); `g3b-` `crt1-` `cvl1-` in `bin/PROTECTED.txt` |
| 305 (Track F1, 42060a8) | `DECAY_ROUTE=shrink_only / trace_only / alpha_indep:Λ` in `$WS/harness_cdr1` (HF `17ee0a28`); OFF bitwise inert; α-indep trace = finite difference to ≤ 4.05e-11; proof job **5081090** 410/0 (0.22 GPU-h); `delta'` reading UNSURE (305.2) |
| `cai1` (306, audit cell, wd 0) | 32 jobs **5081292–5081323**, seeds 192–195, 8 arms (4 grains × Λ 5e-5 / 5e-4); 26.99 exp / 64 bound; RULE 21 +15 min 16 s; 0 / 32 started |
| `crd1` (307, ICML-PLAN 1.6, mechanism cell) | 18 jobs **5081273–5081290**, seeds 196–198, 6 arms (SR / TR at wd 0.1, AI Λ 3.15e-4 at wd 0 × scalar / layerwise); 12.34 / 54; RULE 21 +14 min 55 s (≥ +1 min 05 s by ls-remote); 0 / 18 started |
| running batches (10:01 UTC) | `crt1` **36 / 36 COMPLETED** (unscored); `csh1` 13 COMPLETED / 5 running; `cvl1` 10 running / 22 pending; `g3b` 32 pending; 104 pending / 15 running on `QOSMaxGRESPerUser` |
| audit | `origin/master` == local; 304–307 in place and in order, no stub, none withdrawn, next free **308** (one duplicate next-free line removed); `sacct` names / counts / seeds match 306 / 307 exactly, no duplicates (5081291 is another user's); seeds 192–198 disjoint from every block and absent from `results/all_runs.csv` (max 162); both RULE 21 margins positive |
| owed before any number | RULE 20 at full coverage once started (`cAI1_rule20.sh`, `cRD1_rule20.sh`; both UNVERIFIED, nothing started); score only at N / N RUN_DONE with the documented invocation |
| owed at landing | **a `DECAY_ROUTE` kind in `corpus_exclusions.py` (code gap, not yet written)** before either batch ingests: `cai1` 32 rows (two-axis with wd 0), `crd1` 18 rows + 6 `ARGS_WD_BASE` (AI arms); `cvl1` 32 rows in 304.8's format; `ARGS_WD_BASE` rows `g3b` 16, `crt1` 36, `csh1` 18 |
| discipline | `alice` NOT contacted (every entry says so; grep clean); `paper/` unchanged in git (0 commits touch it since acb0b99); no clone / checkout / worktree disclosed or made; no STATUS / ICML-PLAN edits by the tracks; this audit edited the CORRECTIONS next-free line, ICML-PLAN §3 row 1.6 and §4a, and this table |

## CYCLE 161 (REGISTRATION + LAUNCH, FOUR TRACKS; CYCLE-4 AUDIT) — **CORRECTIONS 299 (`g3b`), 300 (`crt1`), 301 (`csh1`), 302 (`PATCH_VALSPLIT`) + 303 (`cvl1`).  ALL FOUR BATCHES REGISTERED, VERIFIED BY A SEPARATE STAGE AND SUBMITTED ONCE: 118 JOBS, ≈92.8 GPU-h EXPECTED, 254 HARD BOUND.  NOTHING LANDED, NOT ONE NUMBER QUOTABLE.  CORPUS 3,383, UNCHANGED.**

| item | value |
|---|---|
| `g3b` (299, ICML-PLAN §4a rank 1) | 32 jobs **5080798–5080829**, seeds 170–173, 8 arms; 24.4 GPU-h exp / 64 bound; RULE 21 +1 h 02 min 48 s; 0 / 32 started |
| `crt1` (300, rank 2) | **36** jobs **5080605, 5080607–5080626, 5080628–5080642** (5080627 not ours), seeds 176–178, 12 arms; 34.4 / 72; RULE 21 +8 min 38 s; **17 COMPLETED (RUN_DONE 17), 16 running, 3 pending** |
| `csh1` (301, rank 3) | **18** jobs **5080645–5080662**, seeds 180–182, 6 arms; 12.3 / 54; RULE 21 +14 min 33 s; 0 / 18 started |
| `cvl1` (303, 1.14; patch 302, 0.30 GPU-h of proof jobs 5080195 FAILED-control / 5080389 PASS 82/0) | 32 jobs **5080667–5080698**, seeds 184–187, 8 arms; 21.65 / 64; RULE 21 ≥ +7 min 20 s; 0 / 32 started |
| audit (08:35 UTC) | `origin/master` == local; 299–303 in place and in order, no stub, none withdrawn, next free **304**; `sacct` names / counts / seeds match every entry, no duplicates, no stray jobs; seed blocks disjoint and absent from `results/all_runs.csv` (max seed 162); all RULE 21 margins positive; 84 pending on `QOSMaxGRESPerUser` (16 GPUs concurrent) |
| ETA (ESTIMATE, FIFO at 16 concurrent L4) | `crt1` ≈ 09:40 UTC, `csh1` ≈ 10:15, `cvl1` ≈ 11:30, `g3b` ≈ 13:00–14:00 |
| owed per batch before any number | RULE 20 at full coverage once all started (every launcher's guard 7 was UNVERIFIED: nothing had started); `cvl1` also the by-hand `argsline_guard.py --name cvl1-` audit; score only at N / N RUN_DONE with the documented invocation |
| owed at landing | `ARGS_WD_BASE` rows: `g3b` 16, `crt1` 36, `csh1` 18; **`cvl1` needs a new `VAL_SPLIT` kind in `corpus_exclusions.py` first** (32 rows, 16 two-axis; a code gap, 303.9); `g3b-`, `crt1-`, `cvl1-` into the repo's `bin/PROTECTED.txt` (only `csh1-` is there) |
| discipline | `alice` NOT contacted by any track (every entry says so; grep clean); `paper/` unchanged in git; **three disclosed paper/-adjacent slips, nothing read**: 300.10 and 301.10 scratch `git worktree` checkouts (301.10 also a discarded `ls paper`), 301.11 an `ls -d paper` on a stage that has none; no STATUS / ICML-PLAN edits by the tracks; this audit edited CORRECTIONS headers 300 / 303 and the next-free line, §4a and this table |

## CYCLE 160 (LANDING + THE PATH DECISION) — **CORRECTIONS 295 (`caw2`) + 296 (`cgw1` and the ONE ingest of both) + 297 (THE PATH DECISION).  BOTH BATCHES LAND; C1 IS CLOSED BY ITS OWN GATE ON BOTH HALVES; THE CAMPAIGN WRITES ONE PAPER, FOR TMLR.  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,383 / 3458.9 GPU-h / 268 EXCLUSIONS.**

| item | value |
|---|---|
| `caw2` verdict | **`NO-COLLAPSE-CONTROL-DOSE-NOT-REACHED \| M-COLLAPSE+L-NOGAP+A-NOGAP+X-NOGAP+K-COLLAPSED \| ATTR-BASE-PROTECTS \| GATE-DOES-NOT-FIRE`** + 26 stamps |
| `caw2` bounds first | the dose arm reached **`RHO_X` 0.0591** of the control's realised shrink (bar 0.5), so immunity at the control's dose is **UNDECIDED**; every NOGAP is a bound; the base swap cannot be separated from its ~50× lower dose or its momentum change; K01 / MS late shrinks are the `BETA_CLIP` floor; "standard recipe" = the HARNESS's AdamW + Adam; one cell |
| `caw2` levels | K01 **22.5240** (control collapsed); M `MS` **13.2440** / `ML` 68.2060 (**COLLAPSE**, +54.9620 pp); L `LS` **72.5673** / `LL` 68.1093 (NOGAP); **A `AS` 72.9493 / `AL` 67.8440, `G_A` −5.1053 pp = −9.75 SE (NOGAP, co-primary)**; X `XS` **73.2507** / `XL` 68.6840 (NOGAP) |
| `caw2` answer | **a CORNER CASE of the SGDm base at this cell**, not a hazard of the standard recipe (295.7) |
| `cgw1` verdict | **`AUDIT-UNDECIDED \| SCALAR-BEATS-BEST \| W1-SURVIVES+W2-SURVIVES+W4-UNDECIDED`** + 7 stamps |
| `cgw1` bounds first | ONE cell (CIFAR-10); three rungs; α-scaled decay only; **W4's realised shrink is 115–883× below a standard SGD recipe's**, so no reading settles whether the audit effect is a non-standard-decay artefact; "standard decay" in the licence = the VALUE 5e-4 applied α-scaled |
| `cgw1` numbers | D = chunk777 − nodewise **+0.3693 (W1, SURVIVES) / +0.4533 (W2, SURVIVES) / +0.2410 (W4, UNDECIDED, ±2 SE [−0.0120, +0.4940])**; at W4 scalar **90.4650** is above chunk777 by **+2.5590** and nodewise by **+2.8000** pp (+20.23 / +22.13 SE) |
| `cgw1` answer | the count-matched sign is **KEPT**, its size at 5e-4 **UNDECIDED**, its practical value **SHRINKS**: the headline is rewritten, not dropped (296.7) |
| gates | RULE 20 full coverage PASS on both (27/27, 40/40; three passes each); scorers `bdeb1321…` / `8bf0aaf7…` UNEDITED exit 0 on both hosts, re-run post-ingest (only the corpus disclosure line moves); independent parsers 267/267 and 15 PASS / 0 FAIL, byte-identical on both hosts; both refute passes **hold** (8 + 7 wording fixes, 0 refutations of a verdict claim; one latent RULE 16 defect F1 in `cgw1`, reported) |
| ingest | `477a853`: 3,316 → **3,383** (added **67**, changed 0, removed 0, 0 of 126,008 cells changed, no proof job); `CORPUS-EXCLUSIONS.tsv` 222 → **268** (caw2 12 `ARGS_MOMENTUM_BASE` + **6 TWO-ARGS, the corpus's first**; cgw1 28 `ARGS_WD_BASE`); `--check` exit 0 PASS (multi-ARGS 6 True); `c98b` exit 0, 618/618; noise-floor demo moved to **0.636566 (df 274)** — the next registration must quote it |
| **THE PATH DECISION (297)** | **C1 (ICML vote capture) CLOSED** — Step 0 not met (289), Step 1 does not fire (295).  **ONE paper, TMLR**: the count-matched audit with its headline rewritten by `cgw1`, the denominator, and the collapse as a bounded configuration-conditional section (C2 merged in).  No ICML submission.  TMLR ≈ 0.6–0.7 (judgement) |
| next (ranked, none submitted) | **1** G3b: the audit's CIFAR-100 cell at 0.1 / 5e-4, four grains, 32 runs ≈ 26 GPU-h (also answers 1.16); **2** retune at 5e-4 on the core cell, 27 runs ≈ 20 GPU-h (can refute `SCALAR-BEATS-BEST`); **3** 1.20 short-horizon γ control, 12 runs ≈ 8.4 GPU-h |
| docs | MASTER-TABLE rows **239–240** appended, header 3,383 / 3458.9, tally 175 → **177**, `c73` exit 0; `ICML-PLAN` amended (297 paragraph, §0, §4 table + gate, **§4a live queue**); this table; Venue row below amended |
| owed to the draft (`paper/` NOT touched) | the abstract + audit headline (296.7), the scalar-beats-both row + qualifier, the W4 bound, the collapse section's first sentences (285.8, 295.8), the harness-AdamW disclosure; the abstract gate |
| discipline | ZERO GPU; one read-only `sacct` / `squeue` on `alice2`; **`alice` NOT contacted**; nothing under `paper/` read; no notebook site or Vercel URL opened; nothing downloaded |

## CYCLE 159 (LANDING) — **CORRECTIONS 285: `cwd5` LANDS 27/27 — `THRESHOLD-W1-W2 | W1-COLLAPSE+W2-NOGAP+W3-NOGAP+W4-NOGAP+CAR-UNREADABLE`.  THE WEIGHT-DECAY LADDER, THE ONE EXPERIMENT §10 SAID DECIDES WHETHER THE MECHANISM LINE IS PUBLISHABLE AT ALL, IS RUN — AND IT IS THE ADVERSE, PRE-REGISTERED OUTCOME.  THE COLLAPSE IS PRESENT AT THE CAMPAIGN'S α-SCALED wd 0.1 AND ABSENT AT 1e-2, 1e-3 AND 5e-4.  THE REFEREE'S CORNER-CASE CHARGE LANDS; `WRITEUP-mechanism`'s O-14 CLOSES AGAINST THE GENERALITY OF THE RESULT.  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,316 / 3407.9 GPU-h.**

| item | value |
|---|---|
| verdict | **`THRESHOLD-W1-W2` + `W1-COLLAPSE+W2-NOGAP+W3-NOGAP+W4-NOGAP+CAR-UNREADABLE`** + 19 stamps, `LADDER-IS-FOUR-POINTS`, `DECOUPLED-NOT-TESTED`, `BOTH-GRAINS-AT-EVERY-RUNG`, `FLOOR-READINGS-ARE-BOUNDS` and `HW-UNIFORM-NVIDIA_L4` among them |
| the question it answers | **HAZARD OR CORNER CASE?**  Four rungs, both grains IN BATCH at every rung.  `G_W1` = +46.0700 pp = +87.46 SE (`COLLAPSE`); `G_W2` −1.0893, `G_W3` −4.1607, **CO-PRIMARY `G_W4` = −4.5267 pp = −8.59 SE at the standard CIFAR 5e-4** — all three `NOGAP`.  **A CORNER CASE, and not close** |
| the BOUNDS (lead with these) | **FOUR rungs BRACKET the transition between 1e-2 and 0.1 and LOCATE NOTHING inside it** — *"exists only at 0.1"* is forbidden.  `G_W1` is a floor LOCATION, so its SIZE is where the arm landed (164.6).  **`NOGAP` is a bound**, never *"the grains are equal"*.  **`G_W2` is NOT resolved** (−2.07 SE, ±2 SE [−2.1428, −0.0358], upper end 0.04 pp from zero) and may not be pooled with W3/W4.  **α-INDEPENDENT decay untested in either direction** |
| levels | scalar **23.2240 / 69.3513 / 72.4980 / 72.4080**, layerwise **69.2940 / 68.2620 / 68.3373 / 67.8813**, `CARW2` **71.8327** (a LEVEL only).  No state near-bar: W1 collapses by 11.4230 pp, W2/W3/W4 sit **35.2203 / 38.3293 / 38.4673 pp ABOVE** their own rungs' bars; largest seed range 1.388 against a 5.0 bar; **the account replicates on every individual seed**.  `SIGMA_USED` **0.645141** (frozen prior over in-batch 0.485817), `SE_ARM_DIFF` **0.526756** |
| which side moves | **the layerwise ladder is nearly FLAT — 1.4127 pp across a 200× change — while the scalar arm swings 49.2740 pp.**  At W1 the collapsed scalar arm reaches only **23.17 % TRAIN** against 97.9–99.8 % everywhere else: **it never learns** |
| anchor | `k01W1` 23.2240 against `cwd3`'s landed `k01` 22.9853 = **+0.2387 pp**, inside the 5 pp `MATCH` bar — no `ANCHOR-DIFFERS-FROM-CWD3` stamp, so the threshold is not an artefact of a failed anchor |
| the carrier companion | **`CAR-UNREADABLE`, and it is read as NOTHING.**  W2 is `NOGAP`, so there is no collapse for the mask to remove; `P_CARW2` / `D_CARW2` never computed.  **The mask DID bite** (positive `dm_wdterm` on 1,500 of 1,500 masked records), so this is a **design-scope limit, not a patch failure** — the 256-vector audit shows `THRESHOLD-W1-W2` can NEVER carry a read carrier word |
| gates | RULE 20 **27/27** full coverage — **four independent passes** (registration 13:32Z, landing 17:46Z, Track C 18:00Z, refute 18:30Z), all PASS and agreeing — `ENV AUDIT PASS`, **27/27 `NVIDIA L4`**, `argsline_guard.py` `81cea8b5…` UNEDITED; scorer `51c28608…` UNEDITED exit 0, 401 lines, identical on both hosts bar two path echoes and **re-run post-ingest BYTE-IDENTICAL (`29624a5a…`)**; independent parser **401/401** (398 verbatim), MISMATCHES 0, VIOLATIONS 0, byte-identical on both hosts; `G-BITE` 27/27 with **240 tie-skips stated beside the zero** (20 per layerwise run, 0 per scalar) |
| ingest | `91fcd57`: 3,289 → **3,316** (added **27**, changed 0, removed 0, 0 of 124,982 pre-existing field-cells changed; **no proof job** — 5052151 has no row); `CORPUS-EXCLUSIONS.tsv` 201 → **222** (21 `ARGS_WD_BASE` rows, prefix-exact, 222 distinct keys, **the 3 `CARW2` rows the corpus's first TWO-AXIS rows in 284's shape**); `--check` exit 0 `VERDICT: PASS`, `c98b` exit 0 with 283's verdict byte for byte |
| cost | **18.5092 GPU-h** (`sacct`; 18.2167 by the runs' own minutes), mean 0.6855/run.  **ZERO GPU submitted by this cycle**; one read-only `sacct`/`squeue` poll on the `alice2` login node |
| docs | MASTER-TABLE **row 238** appended at the end (no line moved), header **3,316 / 3407.9** and the tally 174 → **175**, old figures bracketed, `c73` UNEDITED **exit 0**.  `WRITEUP-mechanism` amended (§0 item 15 rewritten, **§1 A5 NEW**, §2's claim paragraph, **§3.3d + E28–E30 NEW**, T22, **O-14 CLOSED**, §8, §9, §10.1/10.2/10.4/**10.5 outline**/10.6 R1/10.7), artefact-path checker **exit 0**.  `LIMITS-PREP` §1.1b gains **S1e**.  **The site is NOT touched — rows 237 and 238 import together in a later stage** |
| RULE 16 | held; **one defect REPORTED and NOT fixed (281/285 F1)** — the `cwd5` scorer's gap-monotonicity guard tests `"UNREADABLE" not in word`, whose intended RUNG reading is DEAD CODE, so the only string that can trip it is the CARRIER word; `GAP-MONOTONE-IN-WD` WOULD have fired and is suppressed.  Cosmetic; the monotonicity is re-derived from the four `G` values instead and **no stamp is quoted**.  The eight older open defects stand |
| the refute pass | **HOLDS — the verdict could not be refuted.**  It refuted exactly ONE claim, the SCORER STAGE's provenance (285.6 **W7**): the stage was CODE from `1d2a8b76` plus the two corpus DATA files from HEAD `8b9fbd2`, not a pure `git archive`, which is why the committed logs read 3,088 where a pure archive stage prints 3,085.  **Touches no scored quantity** — the corpus line is disclosure only and all six code shas are the registered ones.  Eight further wording fixes applied (W1–W6, W8, W9), none moving a level, contrast, bar, state, branch word or stamp |
| `cwd4`'s two carried obligations | **(a) DISCHARGED, verified by me** — `CTL2-PARTIAL` appears **0 times** in both committed `cwd4` scorer outputs and its FINAL reads `CTL2-NULL`, so `cwd4`'s three licence paragraphs may be quoted verbatim.  **(b) HONOURED** — no sentence anywhere here reads a `cwd4` result as tensor identity, and `cwd5` says nothing about carriers at all |
| still owed | **nothing from this batch.**  The campaign's own gate list: **O-14 CLOSED (adverse), O-12 CLOSED (283), O-1 CLOSED (278)** — the top live item is now **O-13, the magnitude arm**, and **§10.6 R2 (α-independent decay) remains UNANSWERED** by design (`DECOUPLED-NOT-TESTED`, descoped with reasons at 281.2).  **Q1 and Q2 of `WRITEUP-mechanism` §8 are now the live decisions and they are the professor's**, not this cycle's |

## CYCLE 159c (FINAL AUDIT, ZERO GPU) — **CORRECTIONS 287: THE 283 + 285 + 286 CYCLE RE-AUDITED END TO END BY A STAGE THAT WROTE NONE OF IT.  EVERY GATE RE-RUN AND PASSING, EVERY `cwd4` AND `cwd5` FIGURE RE-DERIVED FROM THE RAW `.out` FILES BY MY OWN READER, THE SITE IMPORT VERIFIED THROUGH `gh api` ONLY.  TWO DOC SLIPS FOUND AND FIXED, BOTH WORDING, NEITHER A NUMBER.  CORPUS 3,316 / 3407.9 GPU-h, UNCHANGED.**

| item | value |
|---|---|
| gates, all re-run here | `corpus_exclusions.py --check --runs ../runs ../runs_alice2` **exit 0 `VERDICT: PASS`** (222 rows / 17 batches; two-axis runs (284): 3, both axes True); `c98b_reproduce.py` **exit 0**, `science 618/618 PASS | drift 18 site(s), 10 differ (not gated) | guards fired 0 | declaration OK` — 283's verdict byte for byte; `c73_mastertable_check.py` UNEDITED **exit 0**; `writeup_artefact_paths.py` **exit 0** (re-run again after my edits) |
| numbers re-derived FROM RAW `.out`, not from any repo program | my own stdlib reader over the 27 `cwd5` and 21 `cwd4` files in `alice-backup/runs_alice2`, `plateau5` = mean TEST over epochs 95–99: **all 16 arm levels reproduce to 4 d.p.**  `G_W1` **+46.0700** / `G_W2` **−1.0893** / `G_W3` **−4.1607** / co-primary `G_W4` **−4.5267 pp = −8.59 SE**; collapse-bar margins **−11.4230 / +35.2203 / +38.3293 / +38.4673**; `G_W2` ±2 SE **[−2.1428, −0.0358]**; layerwise spread **1.4127**, scalar swing **49.2740**; anchor **+0.2387** vs `cwd3`.  `cwd4`: `P_2SPEC` **+44.2167 = +83.94 SE**, `P_CTL2` **+0.0333 = +0.06 SE** ±2 SE **[−1.0202, +1.0868]**, `ONE50` margin **+0.2133 = +0.40 SE**, `ONE59`−`TWOWD0` **+0.8853 = +1.68 SE**, `D_TWO` **+2.9387**.  **Zero discrepancies** |
| corpus and ordering | `results/all_runs.csv` **3,316 rows / 38 fields**, `cwd4` 21 + `cwd5` 27; `CORPUS-EXCLUSIONS.tsv` **222 data rows**; ingests `8b9fbd2` (+21/+18) and `91fcd57` (+27/+21) touched those two files and nothing else.  `MASTER-TABLE.md` **238 lines**: across `ed8d954..HEAD` **exactly two lines differ** — the header (in-place, old figures bracketed) and the append — so no existing row moved.  `CORRECTIONS.md` headings end 280…287 strictly increasing; **43 non-increasing pairs in the whole file, all pre-existing (≤ 167)** |
| site | GitHub `main` = **`429e0c2`**, its Production deployment **`success`** (2026-09-21T00:33:06Z).  175 experiments, outcomes **56/41/37/23**, 3,316 runs, 497 sources, 235 warnings; **MT237 and MT238 both `mixed`**, with **21** and **27** runs linked and 2 warnings each.  Verified through `gh api` and the stage clone only — **no notebook site and no Vercel URL opened** |
| slip 1, FIXED | `WRITEUP-mechanism` §5 **T22** still read *"a granularity measurement **below about 1e-2**"* — the **third** instance of 285.6 **W1**'s error class, and the one 286 did not sweep for after fixing the other two.  It excludes the tested 1e-2 rung (a measured `NOGAP` rung) and softens a BRACKETING PAIR into an approximate boundary inside the **unrun** decade.  Now reads ***"at 1e-2 and below — the three rungs we ran"*** |
| slip 2, FIXED | 286 recorded its edit as landing in **§1 A4(b)**.  §1 A4 is the **GroupNorm** row (`cgn2` / `cgn3`, `MAGNITUDE-NOT-SEPARATED`) and has no `(b)` and no weight-decay clause; the sentence 286 actually changed is in **§0 item 15**.  The label is corrected in `WRITEUP-mechanism`'s header and here, each with the old label bracketed; **CORRECTIONS 286's own fix table is NOT rewritten** and is superseded by 287 |
| what I did NOT change | no number, bar, state, contrast, branch word, stamp or licence sentence; no scorer, guard, patch, launcher, `results/*.csv` or `results/*.tsv`; **RULE 16 held**.  The site was not touched — rows 237 and 238 are already imported and deployed |
| disclosure carried forward | `--check`'s noise-floor DEMONSTRATION moved again with the corpus: `SIGMA_R18ALL` **0.641573 (df 267)** at this HEAD, against the 0.645653 (df 261) 283.9 told the next registration to quote and the 0.645141 `cwd5` froze at 281.  **No bar reads that line**, and freezing the prior at registration is what makes `cwd5`'s SE stable — but the NEXT registration must quote **0.641573**, not 283.9's figure |
| still owed, unchanged by this entry | **O-13** (magnitude) is the top live item; **§10.6 R2 (α-independent decay) UNANSWERED by design**; the threshold inside the 1e-2 … 0.1 decade is **not located** and no cheap run locates it from four points; RULE 16 defects `cwd5` F1 and `cwd4` F1 stay **REPORTED-NOT-FIXED** with the eight older ones; §8 Q1 / Q2 and §10.6 R9 are the professor's |
| discipline | **ZERO GPU. No Slurm job submitted or cancelled and no cluster command of any kind** — everything ran on the Mac against the already-synced `../runs_alice2` tree.  `alice` (Saber's shared account) **NOT contacted**.  **Nothing under `paper/` read, opened or touched.**  No notebook site or Vercel URL opened; nothing downloaded; no `.pdf` fetched |


## CYCLE 159b (VERIFICATION, ZERO GPU) — **CORRECTIONS 286: 285's LANDING RE-VERIFIED BY A STAGE THAT DID NOT WRITE IT.  EVERY GATE RE-RUN AND PASSING, THE INGEST RE-DERIVED WITH AN INDEPENDENT READER, AND **ONE WORDING DEFECT** FOUND AND FIXED IN `WRITEUP-mechanism`.  NOTHING INGESTED; CORPUS UNCHANGED AT 3,316 / 3407.9 GPU-h / 222 EXCLUSIONS.**

| item | value |
|---|---|
| gates re-run | `corpus_exclusions.py --check --runs ../runs ../runs_alice2` **exit 0 `VERDICT: PASS`** (222 rows / 17 batches; **two-axis runs (284): 3, both axes verified True**); `c98b_reproduce.py` **exit 0**, 806 lines, 283's verdict byte for byte; `c73_mastertable_check.py` UNEDITED **exit 0** (175 rows, 3,316 / 3407.9); `writeup_artefact_paths.py` **exit 0**, re-run again after the edit |
| the scorer, POST-ingest | `cWD5_wdladder_score.py` `51c28608…` and `argsline_guard.py` `81cea8b5…` **UNEDITED at HEAD**; re-run **exit 0, 401 lines**, differing from the committed Mac log on **exactly the two runsdir path echoes**; corpus disclosure still **3,088** because the scorer excludes `cwd5-` by name |
| ingest re-derived | keyed on `(run, job_id)`: 3,289 → **3,316**, **added 27 exactly, removed 0, 0 duplicate keys, 0 of 124,982 pre-existing field-cells changed**, 9 arms × 3 seeds, jobs 5052142–5052169, all 100/100 `complete=1 superseded=0`, **no proof job**, **+18.2167 GPU-h**.  Exclusions 201 → **222**, append-only; **all 21 witnesses re-derived from each run's OWN `ARGS:` line — 21/21 match, 0 mismatches**; the 6 anchors carry `0.1` and are correctly unlisted |
| RULE 16 re-checked | `GAP-MONOTONE-IN-WD` / `GAP-NOT-MONOTONE-IN-WD` appear **0 times** in both committed `cwd5` logs — defect **F1** is exactly as reported, still **NOT fixed**; `CTL2-PARTIAL` **0 times** in both `cwd4` logs, so `cwd4`'s floor clause holds |
| the one fix | **WORDING, not a number.**  §0 item 15 **[SUPERSEDED LABEL: §1 A4(b), corrected at **287** — §1 A4 is the GroupNorm row and holds no such clause]** and §2's claim paragraph read *"below ~1e-2"* / *"below about 1e-2"*; both now read ***"at 1e-2 and below"***.  The old form excluded the tested 1e-2 rung AND softened a BRACKETING PAIR into an approximate boundary inside the **unrun** decade — the same error class as 285.6 **W1**.  285.7's copy of the phrase stands as written and is **superseded**, since CORRECTIONS entries are not rewritten |
| still owed | unchanged by this entry: the **site import of rows 237 + 238 together**; **O-13** (magnitude) is the top live item; **§10.6 R2 (α-independent decay) UNANSWERED by design**; `WRITEUP-mechanism` §8 **Q1 / Q2 are the professor's decisions** |


## CYCLE 158 (LANDING) — **CORRECTIONS 283: `cwd4` LANDS 21/21 — `ONE-SUFFICES-PARTIAL | TWO-REC+CTL2-NULL+ONE50-REC+ONE53-PART+ONE59-REC`.  AT MATCHED COUNT TWO THE CARRIER PAIR RECOVERS AND THE CARRIER-FREE PAIR STAYS ON THE FLOOR, AND ONE CARRIER SCALE ALONE CAN SUFFICE — SO 278's COUNT / DOSE RIVAL IS REFUTED AT THIS CELL AND `WRITEUP-mechanism`'s O-12 CLOSES.  THE OTHER TWO CONFOUNDS — POSITION CLASS AND TERM MAGNITUDE — ARE UNMOVED AND BOTH GET SHARPER.**

| item | value |
|---|---|
| verdict | **`ONE-SUFFICES-PARTIAL` + `TWO-REC+CTL2-NULL+ONE50-REC+ONE53-PART+ONE59-REC`** + 19 stamps, `TWO-SPECIFIC`, `MAGNITUDE-NOT-SEPARATED`, `POSITION-CLASS-NOT-SEPARATED`, `FLOOR-READINGS-ARE-BOUNDS` and `HW-UNIFORM-NVIDIA_L4` among them |
| the question it answers | **COUNT vs IDENTITY.** `P_2SPEC` = `TWOWD0` − `CTL2WD0` = **67.0713 − 22.8547 = +44.2167 pp = +83.94 SE** at matched count 2; a single carrier reaches `REC` (`ONE59` **67.9567**, `P_ONE59` +45.1353 = +85.69 SE) |
| the BOUNDS (lead with these) | `CTL2WD0`'s `NULL` is a **floor LOCATION**: `P_CTL2` +0.0333 pp = +0.06 SE, ±2 SE **[−1.0202, +1.0868] spanning zero**, per-seed signs flipping — **never "does nothing"**.  `ONE50`'s `REC` is **+0.40 SE** past the bar (UNSURE).  `ONE59` − `TWOWD0` = **+0.8853 pp = +1.68 SE, not resolved** — the ladder is not cleanly monotone |
| the two confounds it does NOT touch, both now sharper | **position class** — the two `REC` singles {50, 59} are both Kim γ_last, the `PART` single {53} is γ_down, so it bites at count ONE as well as two — and **term magnitude**: the singles land in exactly the in-batch mean-`\|L\|` rank order (59 3.1188e-01 > 50 2.7474e-01 > 53 1.8077e-01, ranks 1/2/3 of 62 against controls at 33 and 38, gap **140.1×**) with BOTH gaps resolved and `SINGLES-SATURATED` absent, which is evidence **FOR** the rival |
| levels | `k01` **22.8213** / `CARWD0` **70.0100** / `TWOWD0` **67.0713** / `CTL2WD0` **22.8547** / `ONE50` **65.2233** / `ONE53` **58.5780** / `ONE59` **67.9567**; largest seed range 1.498 against a 5.0 divergence bar; `SE_ARM_DIFF` **0.526756** (frozen prior 0.645141 over in-batch 0.478878); every contrast WITHIN batch, recovery against the in-batch `CARWD0` |
| anchors | `CARWD0` −0.2540, `CTL2WD0` −0.0786, `k01` −0.1640 against `cwd3`'s — no `DIFFERS-FROM-CWD3` stamp; no `cwd3` level enters any bar |
| gates | RULE 20 **21/21** full coverage — **four independent passes**, 280.8's at 11:27:23Z BEFORE any accuracy line, the scoring track's at 12:12:33Z, the refute's and this entry's (ordering of the last one disclosed at 283.1) — `ENV AUDIT PASS`, **21/21 `NVIDIA L4`**; scorer `475e1278…` UNEDITED exit 0, 359 lines, selftest 133/0/0, identical on both hosts and **re-run post-ingest byte-identical**; independent parser 359/359 (356 verbatim), MISMATCHES 0, VIOLATIONS 0, stdout `07883dda…` identical across hosts; `G-BITE` 21/21 with **9,000 of 9,000 masked records carrying a positive `dm_wdterm`** and 0 `dm_*` keys on `k01`'s 1,500 |
| ingest | `8b9fbd2` (committed by the interrupted stage, **re-verified field by field here**): 3,268 → **3,289** (added **21**, changed 0, removed 0, 0 of 124,184 pre-existing field-cells changed); `CORPUS-EXCLUSIONS.tsv` 183 → **201** (18 `DECAY_MASK` rows, prefix-exact, 201 distinct keys); `--check` exit 0 `VERDICT: PASS`, `c98b` exit 0 with 278's verdict byte for byte |
| cost | **14.9942 GPU-h** (`sacct`; 14.7667 by the runs' own minutes), mean 0.7140/run, against ≈15.0 expected — **right to 0.04 %** — and a 63 h hard bound.  **ZERO GPU submitted by this cycle** |
| docs | MASTER-TABLE **row 237** appended at the end (no line moved), header **3,289 / 3389.7** and the tally 173 → **174**, old figures bracketed, `c73` UNEDITED **exit 0**.  `WRITEUP-mechanism` amended (§0 item 11, §3.3c + E25–E27, §5 T19, §8, §10), artefact-path checker exit 0.  `LIMITS-PREP` §1.1b gains **S1d**.  **The site is NOT touched — rows 237 and 238 import together in a later stage** |
| RULE 16 | held; **one defect REPORTED and NOT fixed (283.6 F1)** — the scorer prints a literal `100 %%` from two bare `print()` calls; cosmetic, in the NOT-LICENSED block, read by no bar or stamp.  The seven older open defects stand |
| the launch refuter's two obligations | **(a) DISCHARGED** — `CTL2-PARTIAL` did **not** fire (`CTL2WD0` is `NULL`, verified three ways here), and the fired branch quotes no floor clause at all.  **(b) CARRIED** — the batch has no power against the magnitude rival by construction, and no sentence anywhere reads the result as tensor identity |
| still owed | nothing from this batch.  **`cwd5` (281, 27 runs, jobs 5052142–5052169) has since been SCORED by a concurrent track at `bfda844` — `THRESHOLD-W1-W2 \| W1-COLLAPSE+W2-NOGAP+W3-NOGAP+W4-NOGAP+CAR-UNREADABLE`, the registered ADVERSE outcome — and is NOT yet landed and NOT yet ingested**; its ingest is **BLOCKED by 281.12's RULE 16 defect** (`CARW2` both prints a `DECAY_MASK: on` witness and deviates on an ARGS value, and `corpus_exclusions.py` lists one witness per run), whose verified fix is **owed under its own CORRECTIONS number BEFORE that ingest** and which a concurrent track is writing now |
| what `cwd5`'s verdict does to THIS row, said rather than left implicit | **it does not move a single `cwd4` number** — no `cwd5` level enters any `cwd4` bar, sigma or contrast, and `cwd4` was scored and ingested before `cwd5` was read.  What it does is **bind the SCOPE sentence**: `cwd4`'s result is at **α-scaled wd 0.1**, and `cwd5` reads the collapse as existing at that rung and **not at 1e-2 / 1e-3 / 5e-4**.  Reading the two together is that landing's job, not this one's; `WRITEUP-mechanism` §10 already carries the wd-0.1 scope INLINE (283.6 **W7**) |

## CYCLE 157 (LANDING) — **CORRECTIONS 278: `cwd3` LANDS 15/15 — `CARRIER-DECAY-SUFFICES | CAR-REC+CTL-NULL+CTL2-NULL`.  THE ResNet CARRIER-ONLY DECAY MASK, WHICH 271 / 274 AND THE WRITE-UP'S OWN T2 / O-1 NAMED AS THE GAP, IS RUN AND THE GAP IS CLOSED AT THIS CELL.  `alice2` QUEUE EMPTY; `alice` NOT CONTACTED; ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,268 / 3375.0 GPU-h.** *(Authority is `docs/CORRECTIONS.md`, now 278.)*

| item | value |
|---|---|
| verdict | **`CARRIER-DECAY-SUFFICES` + `CAR-REC+CTL-NULL+CTL2-NULL`** + 15 stamps, `FLOOR-READINGS-ARE-BOUNDS` and `HW-UNIFORM-NVIDIA_L4` among them |
| the one measured positive | `P_CAR` = `CARWD0` − `k01` = **70.2640 − 22.9853 = +47.2787 pp = +89.34 SE**; `CARWD0` is **`REC`**, +4.5413 pp inside `NWD`'s band.  `P_SPEC` = `CARWD0` − `CTLWD0` = **+47.2627 pp = +89.31 SE** |
| the BOUNDS (lead with these) | both controls are LOCATIONS at `k01`'s floor: `P_CTL` +0.0160 pp, `P_CTL2` −0.0520 pp → **\|effect\| ≤ 1.0744 / 1.1104 pp at ±2 SE** (2 SE = 1.058364 is the HALF-WIDTH), ratio **42.6 : 1**.  `P_SET` = `NWD` − `CARWD0` = **+0.4587 pp = +0.87 SE, a bound ≤ +1.5171 at 2 SE, and its SIGN is HORIZON-DEPENDENT** (crosses zero near epoch 80, still moving at 99).  **`F_CAR` 0.9904 is DESCRIPTIVE and drifts through 1 — no share may be read off it** |
| two confounds, neither excluded | **position class** (only five 512-wide BN scales exist, three are carriers — 275.1, Kim et al. arXiv:2205.07260) and, separately, **count / dose** (`CTLWD0`'s idx 48 is a BN shift with ‖w‖ 1.3e-10, so its effective set is TWO scales) |
| levels | `k01` **22.9853** / `CARWD0` **70.2640** / `CTLWD0` **23.0013** / `CTL2WD0` **22.9333** / `NWD` **70.7227**; largest seed range 1.1460 against a 5.0 divergence bar; `SE_ARM_DIFF` 0.529182 (frozen prior over in-batch 0.408720) |
| anchors | `k01` vs `cwd1` 22.9513 and the corpus census 22.96; `NWD` vs `cwd1`'s `k01NWD` 70.7760 (Δ 0.0533, inside `MATCH`) |
| gates | RULE 20 **15/15** full coverage (three independent passes; **this entry's own ordering slip DISCLOSED at 278.1**), `ENV AUDIT PASS`, **15/15 `NVIDIA L4`**; scorer `bade3827…` UNEDITED exit 0, selftest 111/0/0, 259 lines identical on both hosts and re-run after the ingest with no line moved; independent parser **259/259**, VIOLATIONS 0, byte-identical across hosts; **`G-BITE` 15/15 with a positive `dm_wdterm` on 6,000/6,000 masked records and a CROSS-READ that refuses every wrong k** |
| ingest | `064dff6`: 3,253 → **3,268** (added **15**, changed 0, removed 0, 0 of 123,614 pre-existing field-cells changed); `CORPUS-EXCLUSIONS.tsv` 171 → **183** (12 `DECAY_MASK` rows, witnesses GENERATED, Track A's handover file verified not trusted, 0 differing cells); `--check` PASS, `c98b` exit 0, same verdict as 273 / 266 / 253 / 247.  **Disclosed:** the `--check` demonstration floor moved, `SIGMA_R18ALL` 0.648113 (df 255) → **0.645141 (df 258)**; no bar reads it |
| cost | **10.7111 GPU-h** (`sacct`; 10.60 by the runs' own minutes), mean 0.7141/run, against ≈12.6 expected — **18 % under** — and a 45 h hard bound.  **ZERO GPU submitted by this cycle** |
| docs | MASTER-TABLE **row 236** appended at the end (no line moved), header **3,268 / 3375.0** and the tally 172 → **173**, old figures bracketed, `c73` UNEDITED **exit 0**.  `LIMITS-PREP` §1.1b gains **S1c**.  **`docs/WRITEUP-mechanism.md` AMENDED in the same cycle**: §0 bound 3 retired, three new bounds, §1's claim paragraph, new §1.3 and §2.3b (**E19–E21**), §3.1(4), **T2 rewritten, T9 updated, T18 + T19 added**, **O-1 CLOSED and O-12 added** |
| RULE 16 | held; **one defect REPORTED and NOT fixed** — the scorer's DESCRIPTIVE `dm` median takes the upper middle (prints 16, true 13.6569); non-gating.  The six older open defects stand |
| still owed | nothing from this batch.  **O-12** (a TWO-carrier mask {50, 53}, ≈5 GPU-h) is the arm that would separate the count account from the identity account — **NOT registered, NOT launched** |

## CYCLE 156 (TRACK A, REGISTRATION + SUBMISSION) — **CORRECTIONS 275: `cwd3` REGISTERED, PROVED AND SUBMITTED — THE CONTRAST THE 274 AUDIT NAMED.  15 JOBS IN FLIGHT ON `alice2`, 15/15 STARTED, 0/15 `RUN_DONE`, NOT ONE NUMBER QUOTABLE.  CORPUS 3,253, UNCHANGED.** *(Authority is `docs/CORRECTIONS.md`, now 276; this entry is 275.)*

| item | value |
|---|---|
| question | on `ResNet18_c100` at `ciso1`'s cell, is the scalar collapse's weight-decay precondition carried by the THREE CARRIERS' OWN decay? (`cwd1` masked all 20 BN scales and could not say which) |
| arms × seeds | `k01` / `CARWD0` {50,53,59} / `CTLWD0` {47,48,56} / `CTL2WD0` {47,56} / `NWD` `normscale`, **every arm scalar**, seeds {140,141,142} = **15 runs**, ≈12.6 GPU-h expected (hard bound 45) |
| new harness code | **NONE** — `PATCH_DECAYMASK`'s registered name-list grammar on `cwd1`'s tree (HF `94aedc33…`) with `cwd1`'s runner (`34a8c90e…`), both UNCHANGED; `cwd_design.py` / `cwd_common.py` imported UNEDITED |
| prior art | `WORTH-TESTING`; **Kim et al. arXiv:2205.07260 names the three carriers exactly** (γ_last, γ_down, γ_last) and the class-pure control (γ_others), so identity vs POSITION-class is **registered as inseparable here** |
| proof job | `cwd3bite` **5047831**, node887 (L4), 6m26s, COMPLETED, `ALL PASS`, **53 PASS / 0 FAIL**; all four name-list masks bite exactly their tensors at every one of 300 real steps, each is told apart from the set it must be distinguished from, and the grammar is loud on five malformed values |
| registration | selftest **111 PASS / 0 FAIL / 0 SKIP** on the Mac and alice2; dry run exit 0, **0 guard failures, 138 lines read**; **ONE submission, 15 accepted / 0 rejected**, jobs 5047894–5047908 |
| RULE 21 | commit `3c22eda` pushed 13:01:53Z, submitted 13:11:16Z — **margin +563 s against the push** |
| RULE 20 | **FULL coverage 15/15, 0 violations**, one distinct ENV line, one `PROBE_TENSOR … type=scalar tensors=62` line, every `DECAY_MASK` witness equal to its arm's registered string |
| hardware | **every job `--constraint=L4`** — 271.4(5) / 273's ungated-hardware defect fixed at the LAUNCHER; the scorer only DISCLOSES the device (`G-HW`), never gates on it |
| owed at landing | **12 `DECAY_MASK` exclusion rows** (the 4 masked arms × 3 seeds; the kind exists since 269, `corpus_exclusions.py` NOT edited); the scorer run unedited on both hosts; an independent parser |
| noise floor | re-derived and frozen for this batch: `SIGMA_R18ALL` **0.6481128684085689** (df 255, `filter_rows`, 3,253 rows; NAIVE 11.390185) — the value 273 said the next registration must quote |

**NOTHING LANDED. NO ACCURACY NUMBER OF THIS BATCH HAS BEEN OPENED.**  `alice` — Saber's shared account — NOT contacted; nothing downloaded; no `.pdf` fetched; no website opened; `paper/` untouched; RULE 16 held (no registered scorer, launcher, patch, tree, runner, `argsline_guard.py`, `corpus_exclusions.py`, `results/*.csv`, `results/*.tsv` edited).

## CYCLE 155 (LANDING) — **CORRECTIONS 270 (`cvt10`) + 271 (`cwd1`) + 272 (`csv1`) + 273 (`cwd2`, the ingest of all four, and THE SYNTHESIS). THE LAST FOUR MUST-TIER BATCHES LANDED, FOUR VERDICTS, ALL CLEAN; `alice` NOT CONTACTED; ZERO GPU SUBMITTED BY THIS CYCLE. CORPUS 3,253 / 3364.4 GPU-h.** *(Authority is `docs/CORRECTIONS.md`, now 273.)*

| # | batch | runs | verdict (registered FINAL, first tokens) | what bounds it | GPU-h |
|---|---|---|---|---|---|
| 270 | `cvt10` `ResNet18_c100` at `ciso1`'s cell, ONE carrier vs THREE at PlainNet's dose (R1) | 30 | `ONE-SUFFICES+ONE50-STALLS+ONE59-STALLS+ONE53-STALLS+SPLIT-NO-EFFECT`: each carrier alone, held on `tri:9428`, stalls (`P_ONE50` +43.66, `P_ONE59` +46.61, `P_ONE53` +35.21 pp) while `ISOSPLIT` 67.0893 stays at ISO 70.1660 — **the one-tensor stall needs the other carriers voting in the shared sum** | sufficiency only, ONE dose, holds from init; all four stalled arms sit BELOW `k01` (LOCATIONS); **`SPLIT-NO-EFFECT` is the tightest word at 1.9233 pp = 3.46 SE**; no free `[59,1,2]` control; **no GPU-hardware census was made** | 25.47 |
| 271 | `cwd1` `ResNet18_c100`, α-scaled WD off on the 20 BatchNorm scales (S1b) | 9 | `COLLAPSE-VANISHES`: `k01NWD` **70.7760** vs `k01` 22.9513, `P_NWD` **+47.8247 pp = +85.99 SE**, `G_NWD` −1.4340 pp — the scalar arm is ABOVE its own masked layerwise reference, `RATIO` 1.0207 | the mask is ONE SET and BOTH ROUTES at once, so *which tensors* not *which route*; the reference arm is also masked; no ISO arm; 7 × L4 / 2 × 2080 Ti, ungated; **no bar is near (6.43 pp = 11.57 SE)** | 7.53 |
| 272 | `csv1` `PlainNet18_c100`, idx 50's applied step vs its vote (N1) | 18 | `BOTH-ROUTES`: `SHADOWLOW` 49.8790 BETWEEN, `NAIVELOW` 65.2420 at HEAD 64.4293; `P_VOTE` **+15.3630 pp**, `P_APPLIED` **+37.7610 pp**, share premise met (0.5271 / 0.3592) | **only the CLAMP FLOOR was tested, so the sentence is "above the floor", not "large" — a RULE 16 defect against the registered licence string (272.6 F1)**; the shadow vote is COUNTERFACTUAL; `INERT` n = 1 and **not bitwise `k01` over the landed run**, though inside its ±2.0 bar | 15.18 |
| 273 | `cwd2` `PlainNet18_c100`, α-scaled WD off on idx 50 under a held large step (N4) | 15 | `WD-ROUTE` + `SCALAR-NEEDS-CARRIER-WD`: `P_WD` **+54.9133 pp = +98.73 SE** against `R_HIGH` +54.2880 (**`F_WD` 1.012**), `P_SC` **+53.6827 pp**, `P_LEFT` −0.6253 pp inside the null bar | three accelerators (A100 / L4 / 2080 Ti), device effect ≤ 0.073 pp; **the mechanism is not watched where it acts — the stalling arms carry no weight-norm record**; `HIGHHEADPATH` 1.3547 pp below `k01`; `k01WD0` changes both routes; complement forced onto a replay | 12.22 |

- RULE 20 at FULL coverage before any number on all four (30/30, 9/9, 18/18, 15/15), 0 violations, on both hosts; `csv1`'s **closes the half CORRECTIONS 267.2 left owed by Track F**. All four registered scorers run **UNEDITED**, exit 0; four independent parsers, VIOLATIONS 0, byte-identical across hosts, `cwd2`'s with a working NEGATIVE CONTROL. Every declared null — BROKEN-SINGLETON-HOLD, BROKEN-MASK ×2, BROKEN-SHADOW — was excluded **on real records before any level was read**.
- **All four refute stages returned `holds = true`** with descriptive wording fixes only. **Their numbered lists are NOT in the repository** (no refute commit, no `results/` file), so each fix was RECONSTRUCTED from the artefact it names and RE-DERIVED before being applied; the numbering `F1…Fn` is the landing's own and is stated as such at 270.6 / 271.6 / 272.6 / 273.6.
- Reported under RULE 16, **none fixed**: `cVT10`'s host-dependent DESCRIPTIVE rounding (`HOLDBIG3` epoch line 18, exact mean 14.05 → 14.1 on alice2 / 14.0 on the Mac); `cwd1`'s mistyped `REGISTERED_COMMIT` (32 hex chars, a 7-character valid prefix — correcting 269.9's "12"); `cSV1`'s `BOTH-ROUTES` licence saying **large** where only the floor was tested, plus seven display ties; `cWD2`'s pre-launch RULE 21 selftest premise; and **269.8's assertion in both `cWD` scorers, for which option (b) was taken and MEASURED — 74/4 and 77/4, every FAIL a stale premise**.
- Ingest `66a19fb`: 3,181 → **3,253** (added 72, changed 0, removed 0; 0 of 120,878 pre-existing field-cells changed; 60.4 GPU-h); `CORPUS-EXCLUSIONS.tsv` 126 → **171**, every witness GENERATED from the run's own `.out`, Track F's proposed rows verified not trusted (0 differing cells). `--check` PASS and `c98b` exit 0, both **re-run by this landing on the committed tree**; 269.6's completeness (108/153 → **153/153**) and multi-kind (30+6 → **33+12**) items CLOSED. **Disclosed:** the `--check` noise-floor demonstration moved, `SIGMA_R18ALL` 0.663166 → **0.648113** (df 255) and `SIGMA_PLAIN` 0.459529 → **0.460632** — no bar reads it, but the next registration must quote it.
- MASTER-TABLE rows **232–235** appended at the end (no line moved), header **3,253 / 3364.4** and the appended-row tally 168 → **172**, old figures bracketed; the stale wallclock-carrying count 3,112 corrected to **3,238**; **row 229 AMENDED IN PLACE** with `cST2`'s verdict — the supersession CORRECTIONS 268.11 OWED. **Line 5 deliberately NOT amended** (no verdict changes it). `c73` UNEDITED **exit 0**, header now consistent with both the table and the CSV. `docs/LIMITS-PREP.md` §1.1b added.
- **THE SYNTHESIS, in one line (273.10): α-SCALED WEIGHT DECAY ON NORMALISATION SCALES IS A PRECONDITION OF THE SCALAR COLLAPSE ON BOTH NETWORKS** — every tensor + the meta trace on ResNet (+48.77), the 20 BN scales on ResNet (+47.82), ONE BN scale on PlainNet (+53.68) — **and the held-step damage runs through the same factor** (`F_WD` 1.012). One carrier suffices on ResNet, but only while the others vote; a step above the floor is necessary for the full stall and the vote carries a partial loss; the isolation rescue transfers to a second meta step size while the nomination does not. **What is NOT established: the mechanism.** The weight-norm readout exists only where the decay is OFF, and there the scales GROW.
- **Judgement (273.12): CONSOLIDATE — WRITE UP NOW.** The headline is a **configuration precondition that common practice avoids** (He et al. arXiv:1812.01187, *no bias decay*), so more GPU on widening the cell cannot raise the ceiling, and the mechanism cannot be reached with this instrument. Ranked: 1 write the mechanism line up as a bounded, configuration-conditional section with the denominator + partition audit keeping the headline (**ZERO GPU**); 2 GPU-hardware census for `cvt10` / `csv1` (**ZERO**); 3 pin a `--constraint` or record the device in `PROVENANCE` (**ZERO**); 4 close 264.6 W1's fix-track debt (**ZERO**); 5 the `cWD` successor question, decided and recorded here (**ZERO**); 6 S4 CTL-at-M9 (~5 h) and 7 N2 (~13 h) and 8 an applied-step dose ladder (~10 h) **only if the professor wants the mechanism widened**; 9 **NOT recommended**: further L1 widening by network or dataset. **Nothing registered, nothing launched.**


## CYCLE 154 (LANDING) — **CORRECTIONS 264 (`cmo1`) + 265 (`cst1` and `cct1`) + 266 (`cmg1`, the ingest of all four, and the LIMITS-PREP consequence). FOUR MUST-TIER BATCHES LANDED, THREE VERDICTS AND ONE GATE; `alice` NOT CONTACTED; ZERO GPU SUBMITTED BY THIS CYCLE. CORPUS 3,181.** *(Authority is `docs/CORRECTIONS.md`, now 266.)*

| # | batch | runs | verdict (registered FINAL, first tokens) | what bounds it | GPU-h |
|---|---|---|---|---|---|
| 264 | `cmo1` `ResNet18_c100`, momentum 0.9 leg and base-WD 0 leg off the mechanism cell (S1a) | 27 | `M9:COLLAPSE-PERSISTS/ISO-RESCUES + W0:COLLAPSE-IS-CONFIG/ISO-UNREADABLE`: `M9k01` 24.58 vs `M9kL` 69.39 (ratio 0.354), `M9ISO` 70.90 clears the one-sided bar by 6.51 pp; `W0k01` **71.56** is the BEST arm, +3.39 pp ABOVE `W0kL` 68.17 and +48.77 pp above the anchor | one value per factor, one factor at a time, no interaction arm; no CTL at either new config (so M9's ISO word is rescue, not specificity); the W0 flag removes WD from the weights AND the meta trace together; **heterogeneous GPU hardware (10 × 2080 Ti / 17 × L4), unregistered — both deciding contrasts are matched seed for seed, but `DI_M9` is confounded** | 25.93 |
| 265 | `cst1` `ResNet18_c100` at ms 3e-4, carriers / ISO / CTL (S2) | 9 | **`UNRESOLVED-DECOMPOSITION` — a GATE, no branch, NO licence sentence.** Scorer exit 1 on a tolerance defect: an absolute 1e-3 bar on a quotient whose float32 noise is 1.59e-03 at ms 3e-4. The harness update is bit-exact on 6,876/6,876 unclamped coordinates | nothing quotable; a FROZEN SUCCESSOR SCORER must be registered first, with the 256.5 bars untouched — the blocked reading misses its bars by 41 and **six** records of 1,500   **[SUPERSEDED at cycle 155, CORRECTIONS 268 then 273: the FROZEN SUCCESSOR `cST2` (`8300f757...`, registered and pushed as `9581897` before it read a record, its ONE change the ms-aware `G-DECOMP` tolerance) reads the same nine runs as `NOMINATION-PARTIAL+ISO-RESCUES+CTL-NULL`, exit 0 -- `k01` 28.5173, ISO 69.6740, CTL 28.6980, `D_ISO` +41.1567 pp = +74.00 SE, `D_CTL` +0.1807 pp, `P` +40.9760 pp. THE ISOLATION RESCUE TRANSFERS TO ms 3e-4; THE VOTE-DOMINANCE NOMINATION DOES NOT -- `DOM_C` 0.4727 = 709 of 1,500 records against a 750-record bar, missing by 41, and `TOP3_C` by SIX. The 256.5 bars were proved unmoved. MASTER-TABLE row 229 amended in place at CORRECTIONS 273]** | 8.85 |
| 265 | `cct1` `ResNet18` / CIFAR-10, dominance where nothing collapses (S3) | 6 | `NOT-COLLAPSED+CARRIERS-DO-NOT-DOMINATE`: ratio 0.9641, gap +3.2607 pp; `DOM_C` **0 of 1,500** records against 0.8069 at the collapsing CIFAR-100 cell, yet the carriers are still the top-3 on 69 % and tensor 59 the argmax on 83 % | two cells only, and **dataset and head width co-vary**; the CIFAR-100 side is a registration calibration, not in-batch. Association, not causation | 7.17 |
| 266 | `cmg1` `ResNet18_c100`, merge the carriers into one group inside healthy layerwise (N3) | 12 | `NO-MERGE-HARMS`: `KLS` 69.5485, `MCAR` 65.4335, `MCTL` 68.4560; `DELTA_ID` +3.0225 pp, `D_CAR` +4.1150 pp, `D_CTL` +1.0925 pp | **the near bar** — `D_CAR` is only 0.885 pp (1.84 SE) inside the 5 pp margin while being +8.54 SE from zero in all four seeds, so it is a BOUND ("costs < 5 pp"), and "harmless" is the wrong gloss; one merge, one cell, pinned-G\* regime, control not role-matched | 16.42 |

- RULE 20 at FULL coverage before any number on all four batches (27/27, 9/9, 6/6, 12/12), 0 violations; `cmg1`'s closed 259.11's open item with a new audit script. All four registered scorers run **UNEDITED** on both hosts; four new independent parsers, VIOLATIONS 0, byte-identical across hosts, `cct1`'s and `cmg1`'s reproducing the scorers' lines verbatim.
- **Two verifiers could not refute; the third returned `holds = false` on ONE descriptive `cmg1` claim, which was re-checked here from the raw probe records and CONFIRMED** — four of N's nine members DO pin late (`t51` and `t54` at 1.000 in every seed), the registered scorer prints only N's four convs, and 259.4 had it right. Non-gating; corrected at 266.6/266.7. All 13 numbered wording fixes applied or answered (264.6, 265.6, 266.6).
- Reported under RULE 16, **not edited**: `cST1`'s `G-DECOMP` tolerance (needs a frozen successor; forward bound corrected — the 1e-3 bar is safe for ms ≥ 4.77e-04, not ms ≥ 1e-3). One delivered-report claim refuted: **no run prints its `Epoch 99` line twice** (264.6 W1) — the false sentence is still baked into `cmo1_attack_indep.py`'s output and is an open fix-track debt.
- Ingest `77c6de9`: 3,127 → **3,181** (added 54, changed 0, removed 0; 58.37 GPU-h); `CORPUS-EXCLUSIONS.tsv` 108 → **126** with `cmo1`'s 18 M9/W0 rows under the CORRECTIONS 263 ARGS-value kinds, every witness GENERATED from the run's own `.out`; `--check` PASS, `c98b` exit 0, cell-mixing clean on 647 cells. **Disclosed:** the `--check` noise-floor demonstration moved, `SIGMA_R18ALL` 0.681198 (df 221) → 0.663166 (df 239); registered floors were frozen at their registrations and are untouched.
- MASTER-TABLE rows **228–231** appended at the end (no line moved), header 3,181 / 3304.0 and the appended-row tally 164 → 168, old figures bracketed; **line 5 deliberately NOT amended** — no verdict changes it (`W0k01` 71.56 is still 6.03 pp under that cell's tuned SGD peak 77.5927). `c73` UNEDITED **exit 0**. `docs/LIMITS-PREP.md` §1.1 "RESULTS SO FAR" added.
- **The headline, in one line: the carrier account survives momentum but is CONDITIONAL ON α-SCALED WEIGHT DECAY**, and dominance travels with collapse across the one non-collapsing cell we can reach. LIMITS-PREP §6's conditional has fired.
- **Judgement (266.11): CONTINUE, narrowly, in this order** — 1 register the frozen `cst1` successor scorer (**ZERO GPU**, unblocks ~41 pp already paid for) **[DONE at CORRECTIONS 268 -- `cST2` registered, `cst1` now reads `NOMINATION-PARTIAL+ISO-RESCUES+CTL-NULL`]**; 2 `cwd1` (S1b) to separate the WD routes `cmo1` opened; 3 `csv1` (N1) for carrier necessity; 4 `cwd2` (N4); 5 `cvt10` (R1, L3 unmoved); 6 the newly live S4 CTL-at-M9 arm. **Still in flight, not landed:** `cwd1` / `cwd2` (0 of 24 started at 260.11 / 261.11), `csv1` (18 jobs, RULE 20 owed), `cvt10` (18 of 30 queued, RULE 20 owed). *[SUPERSEDED at cycle 154's close, CORRECTIONS **267**, state re-read from `sacct` at 2026-09-18T21:46Z: the queue drained while 264–266 were being written. `cvt10` **30/30 COMPLETED** (25.76 GPU-h) and `cwd1` **9/9 COMPLETED** (7.64); `csv1` **14/18 COMPLETED + 4 RUNNING**; `cwd2` **15 RUNNING**, none finished. Nothing cancelled or failed. **RULE 20 at FULL coverage is now CLOSED and PASSING for `cvt10` (30/30), `cwd1` (9/9) and `cwd2` (15/15)** with `argsline_guard.py` `81cea8b5…` unedited; `cwd2`'s first armed-helper log ends `FAIL` on **one** run and that FAIL is a **33-second race**, withdrawn and re-run PASS at 267.2 — do not read it out of context. `csv1`'s batch-consistency half is PASS at 18/18; its registered witness-value audit is **still owed by Track F before any `csv1` number is read**. No batch has been scored and no number of any of the four has been read.]*
- **The cancelled proof jobs, answered (266.12):** `cwdinert1` 5026044 and `cwdinert2` 5026045 were `CANCELLED by 3263` = `s5014158`, **our own account**, 8 s after submission and still PENDING (`Start` None, Elapsed 00:00:00, zero GPU), and resubmitted 11 s later as 5026046/5026047, both `COMPLETED`; an earlier `COMPLETED` pair 5025973/5025974 also exists. Self-cancelled in the queue and immediately relaunched. **WHICH command issued the `scancel` is UNSURE** — no reachable log names those job ids.

## CYCLE 153 (LANDING) — **CORRECTIONS 252 (`cvt8`) + 253 (`cvt9` and the ingest of both). BOTH BATCHES LANDED; NOTHING RUNNING; `alice2` QUEUE EMPTY; `alice` NOT CONTACTED. CORPUS 3,127.** *(Authority is `docs/CORRECTIONS.md`, now 253.)*

| # | batch | runs | verdict (registered FINAL, first tokens) | what bounds it | GPU-h (`sacct`) |
|---|---|---|---|---|---|
| 252 | `cvt8` ResNet18_c100, dose × route on the carrier group (`PATCH_RESTHOLD`) | 21 | `DOSE-FULL+ROUTE-PARTIAL+BIGROUTE-DIRECT`: HOLDBIG 19.38 / BIGISOPATH 19.42 at (3.68 / 3.63 pp below) `k01` 23.06 *[final audit, after 253.15: was "(3.7 pp below)"; HOLDBIG is 3.68 and BIGISOPATH 3.63 pp below, 252.3]*; HOLDHIGH 49.63, HIGHISOPATH 58.19 `BETWEEN`; LOWISOPATH 70.47 at ISO 70.26; `P_DOSE` +30.25, `P_ROUTE` +8.56, `P_ROUTE_BIG` +0.05 pp | big-dose arms below `k01` (a location; BIGROUTE can't see < 5 pp); ROUTE 3.56 pp from its bar; held set (3 vs 1) remains; open-loop | 17.30 |
| 253 | `cvt9` PlainNet18_c100, dose ladder + window pair on 50, complement on HEADPATH (`PATCH_WINDOWHOLD`) | 21 | `DOSE-GRADED \| WINDOW-GRADED`: MIDDOSE 55.86, RESDOSE 21.84 `BETWEEN`; HIGHHEADPATH 11.21 at `k01` 12.13; EARLY 18.37, LATE 27.71 `PARTIAL`; CTL 65.21; `P_WIN` +9.34 pp | FINAL one 2.92 pp move from REPLICATE-FAILED; dose = triangle family; one window pair, open-loop, cut located to 9403–9501 | 17.16 |

- RULE 20 at full coverage (21/21 each) before either scorer; both scorers UNEDITED, exit 0; independent parsers 276/276 and 252/252 lines, byte-identical Mac ↔ alice2; both verifiers could not refute; their 8 + 8 wording fixes applied (252.6, 253.6); 249.3's LATE integral corrected in place.
- Reported under RULE 16, not edited: `cVT8`'s printed BIGROUTE-DIRECT licence ("the partial loss") and its ACCOUNTS coding of the broken-REST_HOLD null; `cVT9` counts non-finite records toward 500 (none here).
- Ingest `1cb52f7`: 3,085 → 3,127 (added 42, changed 0); `CORPUS-EXCLUSIONS.tsv` 75 → 108, `--check` PASS (completeness 108/108, multi-kind 30 + 6); `c98b` exit 0; MASTER-TABLE rows 226–227, header 3,127 / 3245.6, line 5 amended; `c73` exit 0.
- **Judgement (253.13): CONSOLIDATE** — write the mechanism section; five landings running came back graded / partial. **If one more batch (not registered):** R1 `cvt10` ResNet one carrier (`ONE` grouping) vs three at PlainNet's dose; R2 `cvt11` PlainNet peak-neighbourhood window; R3 `cvt12` ResNet dose ladder + window mirror.

## CYCLE 152 (FIX TRACK) — **CORRECTIONS 250. ZERO GPU; NOTHING SUBMITTED; `alice` NOT CONTACTED.**

- `jobs/run_cifar_cvt6.sh` (`9aa2ed5c…`) and `jobs/run_cifar_cvt7.sh` (`805267e4…`) archived from alice2, equal to the runners registered at 242 / 243.
- Both parser outputs belong to the pre-ingest corpus at `5309a96`. `cvt7_attack_indep.txt` also needs the log that matches the runs directory (on the Mac, `results/cvt7_grouphold_score_mac.txt`). Invocations at 250.2.

## CYCLE 152 (LANDING) — **CORRECTIONS 246 (`cvt6`) + 247 (`cvt7` and the ingest of both). BOTH BATCHES LANDED; NOTHING RUNNING; `alice2` QUEUE EMPTY; `alice` NOT CONTACTED. CORPUS 3,085.** *(Authority is `docs/CORRECTIONS.md`, now 247.)*

| # | batch | runs | verdict (registered FINAL, first tokens) | what bounds it | GPU-h (`sacct`) |
|---|---|---|---|---|---|
| 246 | `cvt6` PlainNet18_c100, 50's step size × the complement's path (`PATCH_COMPHOLD`) | 21 | `GRADED`: HIGHHEADPATH 11.49 `AT-K01` (k01 12.07); LOWMUTEPATH 41.56 `BETWEEN`; LOWHEADPATH 65.02 / HOLDLOW 65.61 at HEAD 64.90; `P_50` +53.53, `P_C` +23.47 pp; no single-route sentence | forced arms open-loop; one shape per factor, no dose curve, no time gate; `k01` readings are locations | 14.02 |
| 247 | `cvt7` ResNet18_c100, hold ISO's carrier group (`PATCH_GROUPHOLD`) | 15 | `TRANSFERS-GRADED`: HOLDHIGH 50.23 `BETWEEN` (k01 22.95, ISO 70.04); HOLDLOW 70.30 / HOLDISO 70.06 `AT-ISO`; `P_HIGH` +19.81 pp | dose (−5.21 vs PlainNet's −4.39) and held set (3 vs 1) confounded with network; complement not ISO-like (pin ~53 vs ~96) | 13.86 |

- RULE 20 at full coverage (21/21, 15/15) before either scorer; both scorers UNEDITED, exit 0; independent parsers 207/207 and 165/165 lines, byte-identical Mac ↔ alice2; both verifiers could not refute; their 14 + 7 wording fixes applied (246.6, 247.6).
- In place: 242's RULE H "12 runs" (the 15-run median differs by ≤ 0.1601, mean 0.0100), 242.14 (8 runs had ended, not "the first"), 242.3(2) (18.07), 243.4 (→ 243.13).
- Ingest `45eac02`: 3,049 → 3,085 (added 36, changed 0); `CORPUS-EXCLUSIONS.tsv` 51 → 75, `--check` PASS (completeness 75/75, two-kind 9/9); `c98b` exit 0; MASTER-TABLE rows 224–225, header 3,085 / 3211.6, line 5 amended; `c73` exit 0.
- **Next (247.13, not registered):** R1 `cvt8` ResNet dose × route on the carrier group; R2 `cvt9` PlainNet window and dose on 50 with the complement on HEADPATH; R3 `cvt10` the complement route's window.
...
| # | batch | runs | verdict (registered FINAL, first tokens) | what bounds it | GPU-h (`sacct`) |
|---|---|---|---|---|---|
| 240 | `cvt4` PlainNet18_c100, hold 50's own beta (`PATCH_BETAHOLD`) | 18 | `OWN-STEP-MAGNITUDE`: HOLDHIGH 11.13 / HOLDSHARED 11.39 at `k01` 11.69; HOLDLOW 65.46 / HOLDHEAD 64.67 at HEAD 64.73; `P_HIGH` +53.61 pp | HOLDHIGH's complement collapsed too (pin 40.4 vs HEAD ~99), so direct vs via-complement is not separated; no time gate; no dose curve | 14.96 |
| 241 | `cvt5` PlainNet18_c100 at 300 epochs | 12 | `K-DEPENDENT-EQUILIBRIUM` \| `RESCUE-SURVIVES` \| `K33-HOLDS-PINNED`: K13 53.08 vs K33 37.13 at 300, DROP13 +0.42; RHO 1.0012 | stamps only 0.96–1.34 pp inside READ_BAR; K13's pin is a median hover (s94 ends at r 2.59); level settled before the pin | 25.15 |

- RULE 20 at full coverage (18/18, 12/12) before either scorer; both scorers UNEDITED, exit 0; independent parsers 154/154 and 115/115 lines, byte-identical Mac ↔ alice2; verifiers' wording slips applied (240.6, 241.7).
- Ingest `0f4d006`: 3,019 → 3,049 (added 30, changed 0); `CORPUS-EXCLUSIONS.tsv` 33 → 51, `--check` PASS with completeness 51/51; `c98b` exit 0; MASTER-TABLE rows 222–223, header 3,049 / 3184.1, line 5 amended; `c73` exit 0.
- **Next (241.12, not registered):** R1 `cvt6` 50's step size × complement path factorial; R2 `cvt7` the hold on ResNet18_c100; R3 `cvt8` window and dose on 50.

## CYCLE 151 (REGISTRATION, CONSOLIDATED) — **CORRECTIONS 211–215. FOUR BATCHES REGISTERED AND DRY-RUN ON `alice2`; MASTER-TABLE BROUGHT CURRENT. NOTHING SUBMITTED. NOTHING PUSHED. CORPUS 2,863, UNCHANGED. `alice2` QUEUE EMPTY; `alice` NOT CONTACTED.**

| # | batch | seeds | jobs | GPU-h (expected / bound) | launch (operator, on `alice2`) | commit |
|---|---|---|---|---|---|---|
| 211 | MASTER-TABLE §10 (count-matched partition audit, 37 rows) | — | 0 | 0 | — | `62977fb` |
| 212 | `cgn1` ResNet18_gn_c100 (BN → GN(32)), k01 / kL | 60–62 | 6 | ≈4.5 | `bash bin/cGN1_gn_gap.sh --submit` | `e90ac8a` |
| 213 | `cvh1` VGG11_bn_c100 isolation rescue at 328 ep, k01 / kL / ISO / CTL | 63–65 | 12 | 17.8 / 84 (36.7 at the 7-day pool's worst) | `bash bin/cVH1_vgg_horizon.sh --submit` | `4c1aa60` |
| 214 | `cuc1` CIFAR-100 AUGMENT=0 denominator, 5-rung SGD ladder vs 5-cell meta plus | 66–68 | 30 | 20.0 / 105 (46.3 worst observed) | `bash bin/cUC1_unaug_c100_denominator.sh --submit`, then start the `SGD_*` witness poller at once (214.7); `--witness` at 30/30 | `acaf8f3` |
| 215 | `cpl1` PlainNet18_c100 (residual additions removed), k01 / kL / ISO / CTL / ONE | 69–71 | 15 | ≈11 / 45 | `CPL1_REGISTERED_COMMIT=ea51295d8056103947686ff9aef5c2f98c52c349 bash bin/cPL1_plainnet_residual.sh --submit` | `ea51295` |

- **Total if all four launch:** 63 jobs, ≈53.3 GPU-h expected. **Each launch is the operator's to confirm; none was made.**
- **Launch from a tree whose files sha-match these commits** (each launcher's guard 1a prints the sha prefixes). `cgn1` and `cpl1` run from their own isolated trees (`$WS/harness_cgn1`, `$WS/harness_cpl1`), already built on `alice2`. **Apply neither patch to the live harness:** the GN patch breaks `tests/test_vggbn2.py` V0f (212.2), and the two patches cannot share one tree (215.3).
- **Mac re-check on the committed bytes:** T selftest 18/18, `--check` 53 / 0 mismatches, `c73_mastertable_check.py` exit 0 (148 rows), its selftest 6/6. `cgn1` 61/0/0. `cvh1` 72/0/1. `cuc1` 76/0/1. `cpl1` 82/0/0. Patch tests: `test_resnet_gn_c100` 51/0, `test_plainnet` 77/0, `test_probe_tensor_blockwise_cpl1` 45/0. Each SKIP is a section whose records exist only on `alice2`.
- **`alice2`, read-only, at consolidation:** squeue 0 jobs; 0 `cgn1|cvh1|cuc1|cpl1` jobs in sacct since 2026-09-01; 0 such `.out`; live `build_network.py` `c7998883…`.
- **Open handoff (211.6):** the portal's `export_research.py` still hard-codes 111 experiments and links "Partition tests" to MT098 only.

## CYCLE 148 RECONCILIATION — **THREE TRACKS, THREE ENTRIES (`188`, `189`, `190`), NO COLLISION. CORRECTIONS 191. 36 JOBS IN FLIGHT ON `alice2`, 9 STARTED, 0/36 `RUN_DONE`, 0 TRACEBACKS — **NOT ONE NUMBER QUOTABLE**. ZERO GPU SPENT BY THIS ENTRY; CORPUS 2,761, UNCHANGED; NOTHING SCORED, NOTHING INGESTED.**

* **No collision, nothing renumbered.** Linear history, 8 commits, no merge; entries disjoint by file — Track A `654497f`+`6f90afe` (188, the only track that wrote `STATUS.md`), Track C `77b499d`+`a56083e` (189), Track B `0a60f05`+`d593069`+`ca8926e`+`8594975` (190). `grep -cE '^## <n>\.'` = **exactly 1** for each of 188/189/190, **0** for 191; each entry names the next free number correctly. `bin/PROTECTED.txt` carries all three prefixes (`cvg1-`, `cdep1-`, `ciso2-`).
* **RULE 20 + ENV, audited `2026-09-09T14:13:27+0200` with the UNEDITED guard, coverage 9 OF 36:** `cdep1` **9/18 started** → `batch-consistency` *"every non-axis flag is identical across 9 runs"*, **`9 clean, 0 WITH REPEATED FLAGS OR DESIGN MISMATCH, 0 without an ARGS line / VERDICT: PASS`**; ENV **one line ×9** modulo `PROBE_DIR`; `PROBE_TENSOR` 4 blockwise / 2 layerwise / 2 scalar. `ciso2` **0/12** and `cvg1` **0/6** → *"no candidate files … with prefix"* = **UNVERIFIED, NOT FAILED**. **Disclosed discrepancy:** 9 ARGS/ENV lines but only **8** `PROBE_TENSOR` lines — `cdep1-ISO-s25-4929657` started 14:09 and is 0 epochs in, so the line is not emitted yet; a mid-flight timing artefact, not a provenance defect. **No mismatch → nothing cancelled; no `scancel` issued against any prefix; `alice` not accessed.** **RULE 20 + ENV stay OPEN OBLIGATIONS until 36/36.**
* **RULE 21 re-derived by wall clock (`git log -1 --format=%ct` vs `sacct Submit`), all positive:** A `cdep1` `654497f` 13:40:09 vs 13:44:00 = **+231 s**; B `ciso2` scorer `0a60f05` 13:39:25 vs 13:41:57 = **+152 s** (launcher `d593069` 13:40:48 = **+69 s**); C `cvg1` `77b499d` 13:39:25 vs 13:46:20 = **+415 s**. Every registered artefact **unedited since its registration commit** (`git log <reg>..HEAD -- <path>` empty, ×8 files); three scorers sha-identical Mac ↔ `alice2`.
* **CROSS-TRACK FACT NO TRACK COULD SEE (191.4), re-derived by an independent parser from both live manifests:** `cdep1`'s and `ciso2`'s **`ISO` and `ONE` spec strings are BYTE-IDENTICAL to each other and to `ciso1`'s**, at disjoint seeds ({24,25,26} @100 ep vs {31,32,33} @250 ep with a free in-run 100-ep control). The `187` rescue therefore acquires **three independent batch-level replications at nine seeds**. Registered in both directions: concordance is the strongest provenance this campaign has had; **discordance fires each batch's own gate** (`UNRESOLVED-NO-POSITIVE-CONTROL` / `DIFFERS-FROM-CISO1`) and is **itself the finding** — it would put the batch-as-unit-of-replication claim under test, not the isolation claim. Manifests re-composed from their own `sets:` strings: all six `sets:` arms have **exact complements**, correct sizes and numels, `ISO ≡ DEPTH` at **1,536 isolated params exactly**.
* **SEEDS (191.5):** corpus max seed **23**; 24/25/26 and 31/32/33 all carry **0 rows** — both freshness claims correct. **But `cvg1` ALSO uses {31,32,33}**, which no track stated because each checked only the corpus and the one concurrent batch it knew of. **Not a defect** (different architectures, different prefixes, every contrast within batch), **recorded because a future agent reading "31–33 are fresh" in `190` would be wrong**. Next free triple **{34,35,36}**.
* **WHAT H-ISOLATE TURNS ON (191.6):** `cdep1` decides what the rescue is **about** (`DELTA_ID`; `IDENTITY-` vs `DEPTH-OPERATIVE`, with `188.3`'s ×225 magnitude confound unfixable on this architecture); `ciso2` decides whether it is a **regime or a ≈68-epoch delay** (`RHO`; `RESCUE-COLLAPSES` vs `-SURVIVES`, threshold corpus-anchored above the best m=2 rescue on record); `cvg1` decides whether the phenomenon **exists off ResNet** (`GAP-ABSENT` vs `GAP-REPLICATES`). Joint readings registered in advance, including `IDENTITY-OPERATIVE × RESCUE-COLLAPSES` → the entitled sentence is about **timing**, not regime.
* **LEDGER UNCHANGED — SEVENTEEN REGISTERED, FIFTEEN DEAD, TWO LIVE, NONE PENDING.** Checked, not assumed: none of 188/189/190 registers an eighteenth candidate — `cdep1` is a **control** on H-ISOLATE, `ciso2` a **horizon test** of it, `cvg1` a test of the **headline gap's external validity**. Live: `crn1`'s composition; **H-ISOLATE at `UNRESOLVED-PARTIAL`**. H-DOMINATE as named stays dead.
* **SCOPE CAVEAT — TRACK C DOES *NOT* CHANGE IT YET (191.8).** Corpus is **100 % ResNet** (2,761 rows, eleven `network` values, **zero** fail `^[Rr]es[Nn]et`), and `cvg1` has **0/6 started**, so **every finding still carries the ResNet caveat verbatim**. What Track C changed *now*: the caveat is **measurable** (the harness is architecture-agnostic, so it was never an infrastructure limit and cannot be excused as one again) and **sharper** (`189.2`: the carriers are *exactly* the 512-wide BN scales feeding a residual add, the non-carriers *exactly* the mid-branch `bn1` scales — 3/2, no exceptions). What it will **never** buy: VGG is not a one-variable ablation, so `GAP-ABSENT` ⇏ *"residuals cause the gap"* and `GAP-REPLICATES` ⇏ *"residuals are irrelevant"*; `PlainNet18` was **priced (~1 h, ~9 GPU-h) and refused**. `ResNet18_gn`, the nearest prior art, is **17 CIFAR-10 `chunk777`/`nodewise` rows with no `scalar` and no `layerwise` arm** — BN→GN has never been tested at this contrast either.
* **COST:** `cdep1` ≈**15.6** + `ciso2` ≈**21.3** + `cvg1` ≈**9.0** = **≈45.9 GPU-h planned** (walltime bound 132); consumed at the audit **3.373 GPU-h** (`cdep1` only) + 0.071 h for Track C's never-ingested `vggsmoke-` smoke. **JUDGEMENT CALL FLAGGED: each track honoured "~30 GPU-h per batch"; the ACCOUNT total is ≈46 h and no track could see the sum. Nothing is over-committed, but do not launch a fourth concurrent batch on `alice2` before these drain.**
* **`c98_reproduce.py` exit 1, the same 10 checks, no new failing check** (nothing ingested). Corpus **2,761**, **zero** rows under any of the four new prefixes. `alice` not accessed; nothing submitted, nothing cancelled, zero GPU by this entry; `paper/` untouched. Next free number **192**.

## CYCLE 148 (TRACK A) — **`cdep1` REGISTERED AND LAUNCHED: THE IDENTITY-vs-DEPTH CONTROL `187.4` NAMED AS `ciso1`'s BINDING LIMITATION. CORRECTIONS 188. 18 JOBS, ONE SUBMISSION, ≈13–16 GPU-h, RULE 21 MARGIN +231 s. NOTHING LANDED, SCORED OR INGESTED; CORPUS 2,761 (187), UNCHANGED HERE.**

* **Design:** 6 arms × seeds {24,25,26} = 18 jobs, standard cell, `PROBE=100 PROBE_TENSOR=1` on every arm. `k01` scalar / `k62` layerwise anchors **in batch**; `ISO` (`…/50,53,59`, [59,3]/1,536) **re-run in batch at fresh seeds** because BATCH is the unit of replication (F(62,85)=5.47) and SEED is null — every contrast is WITHIN batch, at a cost of 3 jobs ≈ 2.2 GPU-h; **`DEPTH`** (`sets:1-46,49-55,57-62/layer4.0.bn1.weight,layer4.0.bn1.bias,layer4.1.bn1.weight`, **[59,3]/1,536**) — matched to `ISO` on group sizes, **isolated numel exactly**, per-tensor width (512) and depth (all `layer4`), containing **NO carrier**; `DEPTH2` (`…/47,56`, [60,2]/1,024) the class-pure variant; `ONE` (`…/50`, [61,1]/512) to kill H-BROKEN in batch.
* **Every index verified BY NAME on the live model, nothing inherited** (`187.4`'s string was wrong as first written): **48 = `layer4.0.bn1.bias`, a BatchNorm *bias*, 512 params, NOT a scale**. `layer4` has exactly five BN scales (47,50,53,56,59) and every 512-wide BN scale in the model is one of them → a class+depth+numel-matched **carrier-free triple does not exist**; that is what the match costs, and `DEPTH2` makes it testable rather than merely disclosed. Guard 4h re-derives each coarse group **as the complement** of the isolated set on the live model.
* **NEW SCOPE LIMIT, ESTABLISHED BEFORE ANY RESULT (188.3):** the arm **cannot** be matched on **dynamical magnitude**, and no member choice fixes it. On `ciso1`'s own scalar records the carriers are ranks **1-2-3** of 62 by mean |L| (3.19e-01 / 2.81e-01 / 1.83e-01) while **every** 512-param `layer4` non-carrier sits at 1.73e-06…1.29e-03, *below* the median 1.94e-03; 48 is **rank 62 of 62**. Isolated-set totals `ISO` 7.83e-01 vs `DEPTH` 2.25e-03 (**×348**); the best possible carrier-free triple {47,56,60} still **×225**. **An `IDENTITY-OPERATIVE` reading means "not any three matched `layer4` BN parameters" and does NOT separate identity from magnitude** — printed in the scorer's `FINAL` block with every verdict.
* **Registered** `analysis/cDP1_cdep1_depth_score.py` (`4ba6c888f36e8c51…`) + `bin/cDP1_depth_control.sh` (`48ae4b2073196958…`) + `tests/test_probe_tensor_blockwise_cdep1.py` (`1ec724cee58a1690…`) at **`654497f`** `2026-09-09T13:40:09+02:00`, earliest `sacct` Submit `13:44:00` → **margin +231 s**. `--selftest` **100 PASS / 0 FAIL / 1 SKIP** on Mac, **110 / 0 / 0** on `alice2` (guard 1c2 aborts if the ex-ante premise section is skipped).
* **Noise floor RE-DERIVED at 2,761 rows, `cdep1-` excluded from every reader:** `SIGMA_NARROW` **0.925518** (df 60, 30 cells) unchanged; `SIGMA_WIDE` **0.863472** (df 84, 42 cells) **DRIFTED** from `ciso1`'s 0.889041 with its five `sets:` cells ingested; `SIGMA_W` = max = 0.925518, `SE_ARM_DIFF` **0.755682**. Bars deliberately identical to `ciso1`'s so the batches are commensurable: `SUPPORT +25.0 = 33.08 SE`, `REFUTE +5.0 = 6.62 SE`, `READ 1.511364 = 2 SE`, `DEAD 5.0`. Anchor bands `k01` [20.6306, 25.1976] (n 35), `k62` [67.3954, 71.4918] (n 29).
* **PRIMARY = `DELTA_ID` = plateau5(`ISO`) − plateau5(`DEPTH`), NOT `D_DEPTH` — `164.6` applied, not recited.** Under IDENTITY-OPERATIVE `D_DEPTH` is predicted **on the floor** (0.00 margin) and cannot disagree with the account predicting it; `DELTA_ID` is bounded in **neither** direction (**+47.2972 = +62.59 SE** vs **0.0000**). Four things registered in advance: the primary's ≥30 SE separation (selftest-enforced); a floored `DEPTH` entitles a **BOUND**, never a magnitude (`DEPTH-FLOOR-SATURATED`, with the forbidden phrasings printed in the run); the saturation band **[21.0940, 27.9520]** = 9.08 SE re-derived from the corpus **now**; and **H-BROKEN** (*"any partition other than `ISO` floors"*) named and killed in batch by `ONE`.
* **Ex-ante mechanism premise** on `ciso1`'s k01 records (1,500 determinate / 945 pinned), using `187.6`'s flip statistic and not domination: removing {50,53,59} flips the remainder's sign on 0.8260 / **1.0000**; {50} 0.8000 / **1.0000**; **{47,48,56} 0.0000 / 0.0000**; **{47,56} 0.0000 / 0.0000**. The sign account predicts `DEPTH` NULL ex ante. Re-derived by `--selftest`; re-measured in batch on `cdep1`'s own `k01` for `PREMISE-REPLICATES`.
* **Branches:** `DEPTH-OPERATIVE` | `IDENTITY-OPERATIVE` | `BOTH-OPERATIVE` | **`UNRESOLVED-PARTIAL-RESCUE` — registered to MEAN "both matter", a result and not a failure**, with `D_DEPTH`/`D_ISO` printed | `UNRESOLVED-NO-POSITIVE-CONTROL` (`D_ISO` < SUPPORT: the batch refuses to read a `DEPTH` contrast when `ISO` did not reproduce) | the three provenance gates.
* **RULE 16:** `git diff -- analysis/ patches/ tests/ bin/` empty; three NEW files, 0 deletions. `ciso1`'s registered `tests/test_probe_tensor_blockwise.py` (`22f6599e8c523963…`) **NOT edited** — the successor imports it and re-binds `SPECS` at runtime, so every B0–B5 check is its own code; guard 1b3 aborts if `git status` shows it modified. `argsline_guard.py` `81cea8b586e124a6…` unedited, sha-identical on both hosts.
* **Inertness:** `ALL PASS`, **58 PASS / 0 FAIL**, on the live sha `4732b74aa3a10508…` for all four `sets:` arms; guard 4e refuses to submit without it.
* **Launch:** jobs **4929648–4929653 / 4929655–4929660 / 4929661–4929666**; partitions `gpu-short,gpu-l4-24g,gpu-mig-40g,gpu-a100-80g` at `WALL=03:00:00` (source-time hook PASS). RULE 20 pre-submit 18/18 with 0 repeated flags; `guard_postlaunch` PASS on the first run's own ARGS line; `--batch-consistency` **PASS 6/6** on the unedited guard with `--vary stepsize-groups --vary seed --vary run-name`; ENV **one line ×6**; `PROBE_TENSOR` 4 blockwise / 1 layerwise / 1 scalar = exactly the seed-24 arm mix. **Coverage 6/18 — RULE 20 + ENV are OPEN OBLIGATIONS until 18/18; 0/18 `RUN_DONE`, 0 tracebacks; NO NUMBER QUOTABLE.**
* **Cost, priced from `ciso1`'s same-shape 13.02 GPU-h:** median-based **13.1**, mean-based **15.6** (the planning figure), worst case **28.5** GPU-h — inside the ~30 h share. **`alice2` is shared:** `ciso2` (12 jobs, seeds {31,32,33}, the 200-epoch `ISO` horizon) and a `vggsmoke` were submitted by other tracks in this same tree; **seed sets are disjoint**, no guard or `scancel` line touches another prefix, and `cdep1` includes `gpu-short` (an independent QOS pool) so the two do not compete. **`alice` not accessed at all.**
* **NOT established:** no `cdep1` result of any kind; the identity/depth question is unanswered; `187`'s `UNRESOLVED-PARTIAL` stands; H-DOMINATE remains DEAD as named. **Ledger unchanged — seventeen registered, fifteen dead, two live**: `cdep1` registers no eighteenth candidate, it is a **control on H-ISOLATE**. `cdep1-` in `bin/PROTECTED.txt`. `paper/` untouched. Next free number **189**.

## CYCLE 147 — **CORRECTIONS 187. `ciso1` LANDED, SCORED UNEDITED, ATTACKED FIVE WAYS, INGESTED. H-ISOLATE → `UNRESOLVED-PARTIAL`: ISOLATING THE THREE CARRIER BN SCALES RECOVERS THE *WHOLE* SCALAR→LAYERWISE GAP (+46.9307 pp, +62.10 SE); THE LAYER-2 HOMOLOGUES RECOVER NOTHING (−0.0633 pp). **THE DESIGN CANNOT SEPARATE IDENTITY FROM DEPTH**, AND "DOMINATION" IS NOT THE OPERATIVE MECHANISM. H-DOMINATE AS NAMED STILL DEAD. ZERO GPU SUBMITTED; 12.82 GPU-h LANDED. CORPUS 2,761.**

* **Verdict verbatim** (`cIS1_ciso1_isolate_score.py` `f931040533…`, unedited, documented invocation `python3 analysis/cIS1_ciso1_isolate_score.py ../runs_alice2`, post-ingest): `FINAL: UNRESOLVED-PARTIAL | ISO-59-NEITHER | ISO-3-PINS | CTRL-59-PINS | ONE-61-FREE | PREMISE-REPLICATES | ONE-RESCUES | SPECIFIC:CTRL-NULL | ISO-RESCUES-BUT-59-NOT-FREE`. Pre-ingest run identical but for the dropped `FLOOR-NOT-INGESTED`. **0 FAIL** in every gate: [G1] 6/6, [G0] 76/76, [A] 3/3 (0/7,500 · 0/7,500 · 0/88,241 unclamped, 15,259 clamped), [R] 19/19.
* **Levels, re-derived by an independent numpy parser, every digit equal** (`plateau5` = mean test acc over Epoch 95–99, TRAIN beside): `k01` **23.2807** (23.040/23.604/23.198, train 23.1693) · `k62` **69.0273** (68.276/69.444/69.362, train 98.8493) · `ISO` **70.2113** (69.946/70.206/70.482, train 94.2373) · `CTRL` **23.2173** (23.236/23.388/23.028, train 23.1767) · `ONE` **64.7500** (64.162/65.398/64.690, train 83.3127). All ranges ≤ 1.236 < `DEAD_BAR` 5.00. Contrasts (`SE_ARM_DIFF` 0.755682 re-multiplied): `D_ISO` **+46.9307 = +62.10 SE**, `D_CTRL` **−0.0633 = −0.08 SE**, `D_ONE` **+41.4693 = +54.88 SE**, `D_CEIL` **+45.7467 = +60.54 SE**; `ISO − k62` **+1.1840 = +1.57 SE**; `ISO − ONE` +5.4613. `ISO` is **20.2 pp above its registered point prediction 50.0 [45,56]**.
* **Floor dynamics** (occupancy ≤ −15+1e−6 over 500 records): `ISO` 59-group **0.0013**, terminal −14.688 → `NEITHER`; `ISO` 3-group 0.7693 / −15.000 → `PINS`; `CTRL` 59-group **0.6307**, terminal −15.000 → `PINS` (the scalar arm's own is **0.6300**, both first at the floor at record 185 = epoch 37); `CTRL` 3-group 0.0000 / −10.765 → `FREE`; `ONE` 61-group 0.0000 / −11.840 → `FREE`; `ONE` 1-group 0.7733 / −15.000 → `PINS`. Isolated groups pin at epoch 22–23 and never leave.
* **[P] premise replicates on this batch's own scalar arm:** determinate 1500, DISAGREE 1238, `R_T` **0.8253**, pinned 945; the three carriers at **1.0000** each, next share 0.0323, exactly three tensors above 0.5; pinned trio 0.9714, 50 alone 0.0000, {53,59} 0.0000, 59-rest votes UP 1.0000.
* **RULE 20 / 21 / 16 / ENV:** guard `81cea8b586…` unedited → `15 clean, 0 WITH REPEATED FLAGS OR DESIGN MISMATCH, 0 without an ARGS line / VERDICT: PASS`; ENV exactly **one** line ×15 modulo `PROBE_DIR`; `PROBE_TENSOR` 9 blockwise / 3 layerwise / 3 scalar; five specs ×3 jobs; 15/15 `RUN_DONE`, 0 tracebacks, `sacct` 15/15 `COMPLETED 0:0`. RULE 21 commit `21:27:23Z` vs `sacct` Submit `21:27:52Z` = **+29 s** (the batch's own `PROVENANCE.txt` says `21:27:51Z` → +28 s; both positive). `git diff -- analysis/ patches/ tests/ bin/` empty.
* **Attack 1 — IDENTITY vs DEPTH IS THE BINDING LIMITATION.** `ISO` and `CTRL` differ in identity **and** depth **and** isolated numel (1,536 vs 384) at once. The live model has exactly **five** BN scales in `layer4` (47, 50, 53, 56, 59) and every 512-wide BN scale in the model is one of them — so a three-element, class-matched, depth-matched, **non-carrier** arm **does not exist**. The arm that would decide it: `DEPTH` = isolate {47, 56, one non-carrier 512-param `layer4` BN bias} at **[59,3] / 1,536 params** — matched to `ISO` on size, numel and depth — ≈ **2.15 GPU-h** (×3 seeds), ≈ 4.30 with a fresh `k01` anchor. Adjacent, non-decisive: 47 and 56 carry **0.0000** of DISAGREE records and removing either flips the pinned remainder's sign on **0.0000** (vs 1.0000 for tensor 50); and the corpus's fixed-depth count-matched swap `[49,13]` 55.4165 → `sets:1-48,layer4.0.bn2.weight/…` **21.6773** (−33.74 pp) against the 48↔52 swap at **55.5013** (+0.08 pp). Neither is a control. **The entitled sentence must not claim identity-specificity.**
* **Attack 2 — `ISO` does NOT beat layerwise.** +1.1840 pp = 1.57 SE is **inside** the 2-SE `READ` bar (1.511363 pp). `D_ISO`/`D_CEIL` = 1.026. What is real and unstamped: `ISO` matches the ceiling on test while fitting the training set **4.612 pp less** (94.24 vs 98.85).
* **Attack 3 — the joint-domination paradox dissolves and NARROWS the claim.** Domination is sufficient, never necessary. On the 945 pinned records, removing {50} **alone** flips the remainder's sign on **1.0000** — identical to removing the trio — while dominating on 0.0000 (its mean |L| is 0.2283 of the rest's Σ|L|). Removing 47 or 56 flips **0.0000**. Over all 1,500 determinate records: trio 0.8260, {50} **0.8000**, 61-rest votes UP on **0.9740**. And the registered ladder reading is **refuted by `ONE`'s own records**: {53,59} dominates `ONE`'s 61-group on **0.0000**, removing it flips only 0.46–0.48, and that group's sum is near-balanced (UP 0.52–0.54) against the scalar arm's 0.9740. **`ciso1` licenses "the remainder's signed sum stops being one-sided", not "the trio dominates".**
* **Attack 4 — `ISO-59-NEITHER` is NOT a collapse.** `beta[0]` peaks at **−4.95/−5.01/−4.89 at epoch 17–18** (the whole batch's ascent peak), then descends; it first crosses −14.0 at **epoch 98/92/94**; only seed 22 ever touches −15 (2 records of 500) in its final ten records. Last-150 steps 113/111/116 down vs 34/35/31 up, net −4.0/−4.4/−4.4 nats, **still descending at −0.026…−0.036 nats/record at the horizon**. Against `k01`/`CTRL` at the floor from epoch 36–37 for the last 63 % of training. **Verdict stands, for a different reason:** the freedom bar is a terminal-value test and the horizon cuts the descent mid-flight; at the observed slope the group would pin within **2–4 more epochs**, so a 200-epoch `ISO` is genuinely needed. `ONE`'s 61-group ends **higher** (−11.840) and scores **lower** — a higher terminal `beta[0]` is not the good thing.
* **Attack 5 — provenance and power.** Manifest re-verified name-by-name against the live model and the harness's own `param_numels`: 62 tensors / 11,220,132 params; `ISO` (50,53,59) [59,3] 1,536 · `CTRL` (20,23,29) [59,3] 384 · `ONE` (50) [61,1] 512; `ISO △ CTRL` = exactly the six names, `ISO ∩ CTRL` = ∅, `ISO △ ONE` = {53,59}. **`184.8`'s "one trajectory" replicates for the FLOORED arm only** — `k01` aggregate `sign(ΣL)` agrees **500/500** on every seed pair, but `ISO` **364–378/500** and `ONE` **366–390/500**; `beta[0]` seed-pair max|diff| `ISO` 0.46–1.00 nats vs `CTRL` 0.006–0.108. Within-arm SDs 0.18–0.65, all ≤ `SIGMA_W` 0.925518. Even at n = 1 per arm, `D_ISO` = **35.9 SE**. Both anchors inside their corpus bands (scalar n 32 22.8797 ± 0.5812 ∋ 23.2807; layerwise n 26 69.4916 ± 0.4863 ∋ 69.0273, re-derived independently).
* **Ingest:** `aggregate.py … > results/all_runs.csv` then `args_repair.py --apply` (36 standing `dup_group` rows, 0 accuracy/config, 0 `superseded`). **2,746 → 2,761: ADDED 15, CHANGED 0, REMOVED 0, other-batch 0.** Selftest sweep **104 scorers pre/post: 0 exit codes moved, 0 `FINAL:`/`VERDICT` lines moved, 25 drifters**, all corpus-count / cell-mean / pooled-sigma re-derivations; four moved an internal check — `c68_window_blast` FAIL 5→4, `cH1_hb1_score` 10→8, `cJ1_cbl1_score` 9→7 (frozen literals landing back), `cX1_crn1_score` 6→8 (`SIGMA_100_WIDE` 0.883894 → 0.857264) — **none a batch verdict**. `c98_reproduce.py` exit 1, same 10 checks.
* **Entitled sentence and the overreach: 187.10.** **H-DOMINATE as named remains DEAD (184/186)** — `ciso1` tested the descriptive three-tensor residue, not the named hypothesis about the layer-4 convs and the classifier; a rescue does not revive it, and 187.6 narrows the residue further. **Ledger: seventeen registered, fifteen dead, TWO live (`crn1`; H-ISOLATE at `UNRESOLVED-PARTIAL`), NONE pending.** Three arms named for whoever runs next: `DEPTH`/`DEPTH2` (identity vs depth), `ISO` at 200 epochs, and an explicit freeze of 50/53/59 under `scalar`. **None was run here.** Next free number **188**.

## CYCLE 146 (RECONCILIATION) — **CORRECTIONS 186. TRACKS A (184, `b841cf7`) AND B (`be15a15` + 185, `d7e11a5`) DO NOT COLLIDE. H-DOMINATE AS NAMED IS DEAD AT (T), UNTESTABLE AT (E); THE THREE-BN-SCALE RESIDUE IS ESTABLISHED AND DESCRIPTIVE; `ciso1` DECIDES CAUSATION. 7/15 RUNNING, NO NUMBER QUOTABLE. LEDGER 17 / 15 DEAD / 1 LIVE / 1 PENDING. ZERO GPU, CORPUS 2,746.**

* **No collision:** linear history `9ef4ac3 → be15a15 (B) → b841cf7 (A) → d7e11a5 (B)`, `HEAD == origin/master`, tree clean; entries 184 / 185 disjoint, each heading once; only the two docs touched by both, sequentially. RULE 16 over `analysis/ patches/ tests/ bin/`: 5 new files, 0 deletions. `paper/` untouched.
* **Track A re-derived:** `cTD2` unedited on the mirror → identical `FINAL` (0 gate FAILs; `R_T` 0.8266 = 1,239/1,499; PINNED 944 / 1.0000 / agg<0 0.0000; NAMED-CARRIED 0.0000; three carriers 1,239/1,239). Ingest keyed `(run, job_id)` at `9ef4ac3` vs `HEAD`: 2,740 → 2,746, ADDED 6 / CHANGED 0 / REMOVED 0 / other-batch 0, 5.083 GPU-h. Anchors via unedited `cIS1 --selftest`: scalar n 32 22.8797 ± 0.5812, layerwise n 26 69.4916 ± 0.4863 (64 PASS / 4 FAIL, the four frozen anchors, non-gating). `c98` exit 1, same 10 checks.
* **Track B re-derived (23:40 CEST):** `argsline_guard --batch-consistency` PASS 7/7 (unedited guard); ENV one line ×7; `PROBE_TENSOR` blockwise ×3 / layerwise ×2 / scalar ×2; `sacct` 7 RUNNING since 23:28:49 + 8 PENDING; 0/15 `RUN_DONE`, 0 tracebacks, 11–28 epochs. **Coverage 7/15 — RULE 20 + ENV open obligations at 15/15; no accuracy / beta / per-tensor number quotable.** RULE 21 +29 s unchanged.
* **H-DOMINATE, honest position (186.5):** bars 1–2 met (a minority carries the sign DOWN), bar 3 failed (it is not the named minority) → (T) as named DEAD; (E) untestable (181). Residue ESTABLISHED, DESCRIPTIVE: `layer4.1.bn2.weight` / `layer4.0.bn2.weight` / `layer4.0.shortcut.1.weight` carry the sign on 944/944 pinned records, the 59-rest votes UP on all, state frozen from the ascent peak; one trajectory across six seeds; n = 1 cell. **Gradient-scale attack splits:** per-element BN-scale dominance generic to both arms (conceded, a reduction artefact per element); the carriers' TOTAL dominance ×106–212 / ranks 1-2-3 is scalar-trajectory-specific (layerwise: ranks 5/6/11, |L| 30× smaller) — but that comparison is across different `alpha` histories and CIFAR-10 has no per-tensor terms, so "generic to BN" is bounded, not settled. **Companion:** carriers pin under own betas (path-independent DOWN), convs ascend and never pin — confirmed; `linear.weight` pins at −15 (step 18,400 ×3) while voting UP 100 % under scalar — story breaks at the classifier. Cut-position names (49 / 52) verified; cliff mechanism NOT licensed. **`ciso1` decides:** `D_ISO ≥ +25` with `ISO-59-FREE` → residue becomes a mechanism at this cell; `D_ISO ≤ +5` → symptom; `CTRL ≥ +25` → NOT-SPECIFIC; `ONE` splits static vs ladder reading. None on the record yet.
* **Ledger:** seventeen registered (183's sixteen + H-ISOLATE), fifteen dead, one live (`crn1`), one pending (`ciso1`). Briefing's "both queues empty / 2,740 rows / HEAD 9ef4ac3 / sixteen registered" corrected in 186.1. Next free number 187.

## CYCLE 146 (TRACK B) — **`ciso1` REGISTERED AND LAUNCHED: H-ISOLATE, THE CAUSAL TEST OF `ctd1`'s THREE-TENSOR DOMINATION — THE SEVENTEENTH REGISTERED CANDIDATE. CORRECTIONS 185. 15 JOBS, ONE SUBMISSION, ~13 GPU-h. NOTHING LANDED, SCORED OR INGESTED BY THIS TRACK; CORPUS 2,746 (184), UNCHANGED HERE.**

* **Design:** `k01` scalar / `k62` layerwise anchors + `ISO` (`sets:<59>/50,53,59`, [59,3]) + `CTRL` (`sets:<59>/20,23,29`, the layer2 structural homologues, [59,3]) + `ONE` (`sets:<61>/50`, [61,1]), × seeds {21,22,23}, standard cell, `PROBE=100 PROBE_TENSOR=1` on every arm. Carriers verified **by name** on the live model at 50/53/59 (1-based; `184`'s 49/52/58 are the same tensors 0-based); `ISO ^ CTRL` asserted to be exactly the six names at identical sizes. **The brief's control ("512-param BN scales from layer1/layer2") does not exist** — 512-wide BN scales are layer4-only, with two non-carriers; numel is NOT matched (384 vs 1,536), disclosed. `CTRL`'s carrier share on `ctd1`'s DISAGREE records: 0.0000/0.0000/0.0008.
* **Fifth arm, with reason:** on `ctd1`'s 944 pinned records the trio dominates the other 59 on 0.9799 but tensor 50 alone on 0.0000 and the pair {53,59} on 0.0000 (|L| ratios 0.387/0.257/0.436, trio 1.080); the static reading says `ONE` rescues, the corpus ladder (k50 30.34, k53 22.08) says it does not — 3 jobs decide.
* **Registered** `analysis/cIS1_ciso1_isolate_score.py` (`f9310405…`) at **`be15a15`** `21:27:23Z`, Submit `21:27:52Z`, **margin +29 s**; `--selftest` 68/68 on both hosts at registration (2,740 rows). Bars in pp and SE (`SIGMA_W` 0.925518 re-derived, `SE_ARM_DIFF` 0.755682): `SUPPORT +25 = 33.08 SE` (77 % of k49 − k01 = +32.63), `REFUTE +5 = 6.62 SE` (below k50 − k01 = +7.50), `READ 2 SE`, `DEAD 5 pp`; `ISO-59-FREE` (floor occupancy ≤ 0.10 and terminal `beta[0] > −13.0`) gates `CAUSAL-SUPPORTED`; `CTRL-59-PINS`, `ISO-3-PINS`, `PREMISE-REPLICATES` stamps; point predictions `ISO ≈ 50 [45,56]`, `ONE` RESCUES (static reading). Branches: `NOT-SPECIFIC` > `CAUSAL-SUPPORTED` > `CAUSAL-REFUTED` > `UNRESOLVED-{PARTIAL,PROVENANCE,DIVERGED,MANIFEST-MISMATCH}`.
* **Inertness:** new additive `tests/test_probe_tensor_blockwise.py` ALL PASS on the three `sets:` arms against the live sha `4732b74a…` (beta bit-identical 130 × 2 coords, records identical modulo appended keys, per-group decomposition ≤ 2.7e−7); namesets N1–N3 re-proved on the live code with a stripped-live `--pre` (disclosed); no test or patch edited.
* **Launch:** jobs **4925518–4925532**, 7 started 23:28:49 (node851 ×3 = 2080ti, node887 ×4 = L4), 8 pending. RULE 20 `--batch-consistency` **PASS 7/7** (guard `81cea8b5…` unedited); ENV one line ×7; `PROBE_TENSOR` blockwise ×3 / layerwise ×2 / scalar ×2; 2-coordinate records on every m=2 arm; 0/15 `RUN_DONE`; no traceback. **Coverage 7/15 — open obligation; no number quotable.** ETA all 15 ~03:00–04:00 CEST 9 Sep, UNSURE.
* **Disclosed:** after `184`'s `ctd1` ingest the selftest reads **64 PASS / 4 FAIL** — exactly the frozen floor/ceiling anchor equalities (scalar n 32 / 22.8797, layerwise n 26 / 69.4916); nothing gates on them — `score()` reads bands live, bars are in pp, `ciso1-*` excluded everywhere (185.10). `ciso1-` in `bin/PROTECTED.txt` on both hosts. `c98_reproduce.py` exit 1, inherited. `paper/` untouched.
...
|---|---|---|---|
| **TMLR** | **best** | **~0.6–0.7 (297, judgement, after the `cgw1` headline rewrite; not re-estimated at 310, where the scalar row became a tie after re-tuning)** **[SUPERSEDED: 0.80]** | No novelty bar, no length cap; "claims supported" + "of interest" both strongly met. The pre-registered self-reversal reaches the abstract |
...
| NeurIPS / ICML / ICLR main | poor | 0.15 — **not pursued (297: C1 closed by its gate)** | 9-page limit is fatal |
