...
**Variance control.** Common random numbers: seed index k fixes init and data order identically across arms. Then compare per-seed differences (paired Wilcoxon / paired bootstrap), which cuts the CI substantially at 5 seeds. Pre-register the effect size that counts (suggest: Δ ≥ 0.5pp with a 95% paired CI excluding 0) and the number of seeds, before looking.
