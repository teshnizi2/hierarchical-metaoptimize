...
**READ TOGETHER, THE THREE POINT ONE WAY.** The granularity gap this campaign has been measuring is **large, real, robust to per-arm tuning, and not explained by step-size adaptation** (`cru1`) — and it is **a gap between MetaOptimize arms that all sit below a tuned plain SGD baseline** (`cdn1`). The mechanism question remains open, but is now known to involve **how the meta-signal is composed within a group** (`crn1`), not only how the groups are drawn.
