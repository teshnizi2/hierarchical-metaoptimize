...
AMENDED 2026-09-20 at CORRECTIONS 282** — **§2.5, §3.3c (E25–E27), §9.0, §10, §0 item 15 and T22 are NEW; §0 items 11
and 14's preamble, §3's SE table, §5, §7 (O-12 closed, O-14 added), §8, T16, T19, T21, E18, E24, A3 and A4 changed** —
against `master` at
**`bc73c24`** — *the 279 header said `97eb049`, which was already stale when 280 landed; the commit is now restated at
each amendment* — corpus **3,268 rows / 3375.0 GPU-hours** (`results/all_runs.csv` at `064dff6`),
...
11. **IDENTITY IS NOT SEPARATED FROM POSITION CLASS; THE COUNT/DOSE RIVAL IS NOW REFUTED AT THIS CELL BY `cwd4`**
   ...
   **What `cwd4` settled** (21 runs, seeds {143, 144, 145}, every arm SCALAR, all on `NVIDIA L4`): at **count two**
   ...
   `TWO-SPECIFIC`. **What it did NOT settle, by its own registration**: `MAGNITUDE-NOT-SEPARATED` and
   `POSITION-CLASS-NOT-SEPARATED` are on that FINAL **unconditionally**, and item 12 is therefore untouched.
...
is an independent parser's output, `results/cwd4_attack_indep_alice2.txt`. **Its CORRECTIONS landing entry is being
written by another track in this same cycle and its number is not yet fixed**, so the rows below cite **280 for the
registration and the committed scorer output for the numbers**, and nothing here re-scores anything. Every figure was
read from that committed output; I ran no scorer and no job. `SE_ARM_DIFF` here is **0.526756**, from the prior
...
exceeds the LARGEST admissible control tensor by **128.8×**"*. The registration records that the magnitude arm the
...
* Written against `master` **`bc73c24`** *(corrected at this amendment — the bullet still said `97eb049`, which the
  header had already restated; a stale commit in the provenance section of a provenance document is the P8 defect
  twice in one file)*; corpus `results/all_runs.csv` at `064dff6`, **3,268** rows / **3375.0**
  GPU-hours, `results/CORPUS-EXCLUSIONS.tsv` **183** rows (re-counted here: 3,269 lines including the header; 204 lines
  including 20 comments and the header). **[SUPERSEDED headers: `97eb049` at 279; `2ab824c` / 3,253 rows / 3364.4
  GPU-h at 277; `84e4bcb` at 276; `064dff6` at 278.]** **The `cwd4` rows E25–E27 were added after `5b5b372`, which is
  the commit that landed `results/cwd4_countwd_score_alice2.txt`; the corpus line above is PRE-`cwd4`-ingest and is
  left as it stands, because this file ingests nothing.**
...
* **`cwd4` (E25–E27, §3.3c), added at this amendment: every level, contrast, state, bar, stamp and branch word was
  ...
  **Its CORRECTIONS landing entry belongs to another track of this cycle and had not been appended when this was
  written**, so §3.3c cites 280 for the registration and the artefact for the numbers, and names no landing number.
...
alone lifts it to **67.9567** (`cwd4` `ONE59`, E26); and two matched carrier-free scales lift it **not at all**
(**22.8547**, a floor location, E25). **So at this cell the gap is not a fact about granularity; it is a fact about a
weight-decay configuration interacting with a single shared step size, and it is removable for the price of a
one-line change to the optimiser's parameter groups.**
