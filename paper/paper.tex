...
null, now replicated: at fixed count and fixed size multiset, permuting which weights share a
group is worth $-0.009 \pm 0.157\pp$ in the first batch and $-0.018 \pm 0.079\pp$ in a
pre-registered replication that decouples the permutation draw from the run seed and doubles the
power, narrowing the interval from $[-55\%, +51\%]$ to $[-34\%, +28\%]$ of $\Dstat$; the draw's own
variance component is zero. Of nine candidate mechanisms, none
...
(This count is 7 higher than the 1{,}724 of earlier drafts for one reason only: \arm{rp1}'s eight
...
are addressed by four batches submitted while this draft was being written. Three of the four ---
...
count-matched cells across three networks, two datasets and four base optimisers. \arm{rp1} doubles
...
no-partition baseline 1 --- \textbf{no \arm{permnode} row is inadmissible}; earlier drafts
...
(Earlier drafts read 249 of 256, the seven exceptions being \arm{rp1}'s mid-flight snapshots;
