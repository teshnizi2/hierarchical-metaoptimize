#!/usr/bin/env python3
# =============================================================================
# cVK2_vggcut_score.py -- REGISTERED SUCCESSOR to analysis/cVK1_vggcut_score.py
#   (sha c8d46b66..., FROZEN, not edited -- RULE 16).  CORRECTIONS 207 / 208.
#
# THE OPERATOR'S DECISION O2 (207.6): FREEZE THE BARS.  cVK1's score() built
# every bar from SIGMA_USED = max(SIGMA_PRIOR frozen, SIGMA_INBATCH,
# SIGMA_NARROW_live, SIGMA_VGG_live), and the two *_live terms were re-derived
# by pooled_sigma() from the corpus READ AT SCORE TIME.  207.3 showed that
# 3 invented VGG std-cell rows move the verdict of the SAME 27 runs from
# PEAK-AT-22 to UNRESOLVED-TIE-WITH-22.  Here the two live terms are PINNED to
# the values the frozen pooled_sigma() returns at THIS file's registration
# (2,809-row corpus, sha 50c7f9c8..., cvk1- excluded); SIGMA_INBATCH stays live
# because it is cvk1's own data.  No corpus read can move a bar.
#
# THE ONLY NON-COMMENT DIFFERENCES FROM cVK1 (proved in CORRECTIONS 208):
#   (a) SIGMA_NARROW_PIN / SIGMA_VGG_PIN literals and compose_sigma(), which
#       score() calls in place of the two live pooled_sigma() terms; the live
#       values are still PRINTED as a NOTE and feed nothing.  The FINAL stamp
#       that names the sigma source reads SIGMA-NARROW-PINNED (was -LIVE).
#   (b) the selftest's live-corpus sigma pins (the SIGMA_VGG one FAILED on
#       2,809) become NOTEs; new block "O2" asserts the literals and that
#       SIGMA_USED == max(floor, in-batch, the two pins), corpus-free.
#   (c) selftest section E drives the REAL score() over synthetic harness-
#       format runs (the analysis/cVK1_smoke_gen.py format), including the SAME
#       runs against the live corpus, NO corpus, and corpora carrying invented
#       VGG std-cell rows that moved cVK1's bars: the FINAL line must not move.
# Every threshold, SE multiplier, branch, arm, seed, spec string, primary and
# reader is byte-identical to cVK1.  Two strings still name cVK1 (the
# --selftest banner and the argparse description), left for diff-minimality.
#
# =============================================================================
# (below: cVK1's header, unchanged except where it described the live terms)
# THE REGISTERED SCORER FOR `cvk1`, THE CUT-POSITION
...
# [cVK1 read: "At score time SIGMA_USED = max(SIGMA_PRIOR, SIGMA_NARROW_live,
# SIGMA_VGG_live, SIGMA_INBATCH) -- monotone, so corpus drift can only WIDEN the
# bars in SE."  True of the bars, false of the verdict (207.3).]
# cVK2 (O2): SIGMA_USED = max(SIGMA_PRIOR, SIGMA_INBATCH, SIGMA_NARROW_PIN,
# SIGMA_VGG_PIN), the two pins re-derived at THIS file's registration on the
# 2,809-row corpus (sha 50c7f9c8...), cvk1- excluded:
#   SIGMA_NARROW_PIN 0.5862321062215915  df 77, 11 cells, 88 members
#   SIGMA_VGG_PIN    0.3638879864280581  df 10,  2 cells, 12 members
# The NARROW pin exceeds the floor by 1.062e-7, so on today's corpus the bars
# equal cVK1's exactly and a floor-only composition's to < 1.1e-7 in sigma.
# Only SIGMA_INBATCH (cvk1's own 27 runs) can widen a bar.
...
# O2 (207.6): the two corpus terms cVK1 read LIVE, PINNED at this file's
# registration by the frozen pooled_sigma() on the 2,809-row corpus (sha
# 50c7f9c8...), cvk1- excluded.  They are the ONLY new constants.
SIGMA_NARROW_PIN = 0.5862321062215915   # ResNet18_c100 / CIFAR100, df 77, 11 cells
SIGMA_VGG_PIN = 0.3638879864280581      # VGG11_bn_c100, df 10, 2 cells
PEAK_SE = 2.0
FLOOR_SE = 2.0
FLAT_FRAC = 0.10
FLAT_MIN_SE = 4.0
...
def compose_sigma(sigma_in):
    """O2.  SIGMA_USED = max(floor, in-batch, the two PINNED corpus terms), in
    cVK1's candidate order (so a tie resolves exactly as cVK1's max() did).
    It takes NO corpus: nothing read at score time can reach a bar."""
    cands = [("SIGMA_PRIOR_frozen", SIGMA_PRIOR), ("SIGMA_INBATCH", sigma_in),
             ("SIGMA_NARROW_pinned", SIGMA_NARROW_PIN), ("SIGMA_VGG_pinned", SIGMA_VGG_PIN)]
...
def score(runsdir, csvpath, manifest_path):
    ...
    print("\nFLOOR     SIGMA, FROZEN (O2): no corpus read can move a bar")
    ...
    for nm, v in cands:
        print("  %-20s %.6f" % (nm, v))
    try:                     # DISCLOSURE ONLY: the live corpus terms feed NO bar
        sn = pooled_sigma(rows, lambda r: r["network"] == "ResNet18_c100"
                          and r["dataset"] == "CIFAR100")[0] if rows else None
        sv = pooled_sigma(rows, lambda r: r["network"] == NET)[0] if rows else None
        note("live corpus terms, NOT USED (O2): NARROW %s  VGG %s" % (fmt(sn, 6), fmt(sv, 6)))
    except Exception as ex:
        note("live corpus terms unreadable, NOT USED (O2): %s" % type(ex).__name__)
...
# =============================================================================
# SECTION E HELPERS (c): synthetic harness-format runs for the REAL score().
# Format copied from analysis/cVK1_smoke_gen.py (itself copied from the
# harness): NODE, ARGS, ENV, GPU line, warnings, Epoch 0..99, minutes, RUN_DONE;
# file <jobname>-<jobid>.out.  EVERY NUMBER IS INVENTED; evidence of nothing.
# =============================================================================
_E_ENV = ("ENV: AUGMENT=1 BETA_CLIP=-15:-2.3026 HIER=none LAM=na ETA_RATIO=na "
          ...
          "SCHED_WARMUP=none SCHED_MIN=none PROBE=0 PROBE_DIR=none EB_RHO=na EB_LOG=0")
_E_LV = {"k01": 35.0, "kL": 66.0, "k13": 47.0, "k16": 54.0, "k19": 60.0,
         "k20": 58.5, "k21": 59.0, "k22": 61.2, "k23": 37.0}
_E_CH = (3, 64, 128, 256, 256, 512, 512, 512, 512)


def _e_tensors():
    t = []
    for i in range(8):
        t += [("conv%d.weight" % (i + 1), _E_CH[i] * _E_CH[i + 1] * 9),
              ("bn%d.weight" % (i + 1), _E_CH[i + 1]), ("bn%d.bias" % (i + 1), _E_CH[i + 1])]
    return t + [("linear.weight", 512 * 100), ("linear.bias", 100)]


def _e_manifest_text():
    """the manifest the launcher's guard 4 writes, composed from VGG11's cfg"""
    tens = _e_tensors()
    names, numel = [n for n, _ in tens], [q for _, q in tens]
    T, tot = len(tens), sum(numel)
    lines = ["NUM_PARAM_TENSORS %d" % T, "TOTAL_PARAMS %d" % tot, "NETWORK %s" % NET,
             "CLIP_C %s" % CLIP, "EPOCHS %d" % EPOCHS, "META_STEPS %d" % (EPOCHS * (50000 // BATCH))]
    lines += ["TENSOR %d %s %d" % (i + 1, names[i], numel[i]) for i in range(T)]
    lines.append("ARMSPEC k01 SPEC scalar M 1 SIZES %d PARAMS %d" % (T, tot))
    lines.append("ARMSPEC kL SPEC layerwise M %d SIZES %s PARAMS %s"
                 % (T, ",".join(["1"] * T), ",".join(str(x) for x in numel)))
    for k in GRID:
        lines.append("ARMSPEC k%d SPEC %s M 2 SIZES %d,%d PARAMS %d,%d GROUP0 %s GROUP1 %s"
                     % (k, SPEC["k%d" % k], k, T - k, sum(numel[:k]), sum(numel[k:]),
                        " ".join(names[:k]), " ".join(names[k:])))
    return "\n".join(lines) + "\n"


def _e_write(d, arm, s, jid, level, env=_E_ENV, n_ep=EPOCHS, run_done=True):
    rn = "cvk1-%s-s%d" % (arm, s)
    args = ("--optimizer HF --alg-base SGDm --momentum-param-base 0.99 "
            ...
            "--NN-name VGG11_bn_c100 --batch-size 100 --max-time 999:00:00 --gamma 1 "
            "--meta-stepsize 1e-3 --alpha0 1e-6 --num-epochs 100 --stepsize-groups %s "
            "--seed %d --save-directory /home/s5014158/metaopt/runs/cvk1 --run-name %s"
            % (SPEC[arm], s, rn))
    ...
    for e in range(n_ep):
        ...
        lines.append("Epoch %d, Train Accuracy: %.2f %%, Test Accuracy: %.2f %%" % (e, te - 0.4, te))
    if run_done:
        lines += ["27  minutes", "RUN_DONE"]
    open(os.path.join(d, "%s-%d.out" % (rn, jid)), "w").write("\n".join(lines) + "\n")


def _e_build(d, levels=None, jit=(0.0, 0.2, -0.2), over=None, drop=None, shadow=None,
             dup=None, env_break=None):
    """27 runs; last five epochs of (arm, seed) sit EXACTLY at level + jit[seed]"""
    lv = dict(_E_LV, **(levels or {}))
    ...
    for i, s in enumerate(SEEDS):
        for a in ARMS:
            ...
            _e_write(d, a, s, jid, L, env=env)
            if dup == (a, s):
                _e_write(d, a, s, jid + 1000, L)                       # complete resubmission
            if shadow == (a, s):
                _e_write(d, a, s, jid + 2000, L, n_ep=40, run_done=False)  # truncated resubmission


def _e_stress_csv(src, dst, vals):
    """src corpus + INVENTED VGG11_bn_c100 std-cell scalar rows (prefix zstress-) in ONE
    existing VGG cell (207's mk_stress.py).  -> the VGG rows' template run, or None."""
    rows = list(csv.DictReader(open(src)))
    hdr = list(rows[0].keys())
    tmpl = next((r for r in rows if r.get("network") == NET and r.get("granularity") == "scalar"
                 and usable(r) and std_cell(r)), None)
    if tmpl is None:
        return None
    new = []
    for i, v in enumerate(vals):
        r = dict(tmpl)
        for k, x in (("run", "zstress-k01-s%d" % (90 + i)), ("job_id", str(9990000 + i)),
                     ("seed", str(90 + i)), ("plateau5", "%.4f" % v), ("plateau", ""),
                     ("best_test", ""), ("final_test", ""), ("dup_group", "")):
            if k in r:
                r[k] = x
        new.append(r)
...
def _e_minimal_csv(dst, vals=(35.0, 35.0, 41.0)):
    """a corpus holding ONLY invented VGG11_bn_c100 std-cell rows: no host corpus needed"""
    hdr = ["run", "job_id", "seed", "superseded", "collapsed", "complete", "plateau5"] + CELLKEYS
    cell = {"network": NET, "dataset": DSET, "granularity": "scalar", "base": "SGDm",
            "meta": "Lion", "meta_stepsize": MST, "alpha0": A0, "gamma": "1", "augment": AUG,
            "beta_clip": CLIP, "batch_size": str(BATCH), "epochs_requested": str(EPOCHS),
            "hier": "none", "lam": "na", "eta_ratio": "na"}
    w = csv.DictWriter(open(dst, "w", newline=""), fieldnames=hdr)
    w.writeheader()
    for i, v in enumerate(vals):
        w.writerow(dict(cell, run="zstress-k01-s%d" % (90 + i), job_id=str(9990000 + i),
                        seed=str(90 + i), superseded="0", collapsed="0", complete="1",
                        plateau5="%.4f" % v))
...
def selftest(csvpath, runsdir):
    ...
    print("\nO2 -- THE BARS ARE FROZEN (207.6): no corpus term is read into a bar")
    chk(SIGMA_PRIOR == 0.586232, "the frozen floor SIGMA_PRIOR is the literal 0.586232")
    chk(SIGMA_NARROW_PIN == 0.5862321062215915 and SIGMA_VGG_PIN == 0.3638879864280581,
        "the two O2 pins are the literals re-derived at registration (2,809 rows, 50c7f9c8)")
    chk(0.0 < SIGMA_NARROW_PIN - SIGMA_PRIOR < 1.1e-7,
        "the NARROW pin sits %.3e above the floor: O2's bars equal a floor-only composition"
        " to < 1.1e-7 in sigma" % (SIGMA_NARROW_PIN - SIGMA_PRIOR))
    chk(SIGMA_VGG_PIN < SIGMA_PRIOR,
        "the VGG pin loses the max to the floor by %.6f" % (SIGMA_PRIOR - SIGMA_VGG_PIN))
    ...
    names = ["SIGMA_PRIOR_frozen", "SIGMA_INBATCH", "SIGMA_NARROW_pinned", "SIGMA_VGG_pinned"]
    sweep = (0.0, 0.2, SIGMA_VGG_PIN, SIGMA_PRIOR, SIGMA_NARROW_PIN, 0.5862322, 0.75, 1.5, 5.0)
    bad = []
    for s_in in sweep:
        which, v, c = compose_sigma(s_in)
        want = max(SIGMA_PRIOR, s_in, SIGMA_NARROW_PIN, SIGMA_VGG_PIN)
        if not (v == want and v >= SIGMA_PRIOR and [n for n, _ in c] == names
                and dict(c)[which] == v):
            bad.append(s_in)
    chk(not bad, "SIGMA_USED == max(floor, in-batch, NARROW pin, VGG pin) >= floor, over %d"
        " in-batch values" % len(sweep), "bad %s" % bad if bad else "")
    chk(compose_sigma(0.2)[0] == "SIGMA_NARROW_pinned" and compose_sigma(1.5)[0] == "SIGMA_INBATCH",
        "below the pins the NARROW pin is used (stamp SIGMA-NARROW-PINNED); above, the in-batch term")
    ...
    if rows is None:
        ...
        # O2: the live values are NOTED, never pinned.  Under cVK2 they feed no bar, so
        # an ingest cannot fail this selftest (cVK1's SIGMA_VGG pin did: 205.3).
        for nm, live, pin in (("SIGMA_VGG", sV, SIGMA_VGG_PIN), ("SIGMA_NARROW", sN, SIGMA_NARROW_PIN)):
            note("%s live %s vs PINNED %.10f (drift %s) -- feeds NO bar (O2)"
                 % (nm, fmt(live, 10), pin, "n/a" if live is None else "%+.3e" % (live - pin)))
    ...
    want_e1 = ("FINAL: PEAK-AT-22 | CARRIER-CUT-SUPPORTED | A2-A4-REFUTED | GATES-CLEAN | "
               "ALIAS-CLASS-IS-INDEX-MOD-3 | MAGNITUDE-NOT-SEPARATED | HORIZON-100-ONLY | "
               "SHARED-MS-1E-3 | NOT-A-ONE-VARIABLE-ABLATION | SIGMA-NARROW-PINNED | "
               "TRAIN-ARGMAX-AGREES | D22-16-POSITIVE | CLIFF-POINT | GAP-REPLICATES-CVG1")
    live_man = os.path.join(runsdir, PREFIX, "PARTITION-MANIFEST.txt") if runsdir else ""
    if live_man and os.path.exists(live_man):
        chk(open(live_man).read() == _e_manifest_text(),
            "E0 the synthetic manifest is BYTE-IDENTICAL to the live one guard 4 wrote (%s)" % live_man)
    else:
        note("E0 no live manifest under %s/%s -- the synthetic one is composed from VGG11's cfg"
             % (runsdir, PREFIX))
    with tempfile.TemporaryDirectory() as t:
        corpora = [("NO corpus", None)]
        if csvpath and os.path.exists(csvpath):
            corpora.insert(0, ("the corpus given (%s)" % os.path.basename(csvpath), csvpath))
            for vals in ((35.0, 35.5, 38.5), (35.0, 35.0, 41.0)):
                p = os.path.join(t, "stress_%s.csv" % "_".join("%.1f" % v for v in vals))
                cellrun = _e_stress_csv(csvpath, p, vals)
                if cellrun:
                    corpora.append(("+3 invented VGG rows %s in %s's cell"
                                    % ("/".join("%.1f" % v for v in vals), cellrun), p))
        pm = os.path.join(t, "minimal.csv")
        _e_minimal_csv(pm)
        corpora.append(("a corpus of ONLY 3 invented VGG rows 35.0/35.0/41.0", pm))
        se1 = SIGMA_NARROW_PIN * math.sqrt(2.0 / 3.0)
        M1 = dict((k, _E_LV["k%d" % k]) for k in GRID)
        for nm, p in corpora:
            if p is None:
                continue
            sv = pooled_sigma(read_corpus(p), lambda r: r["network"] == NET)[0]
            if nm.startswith("the corpus given"):
                note("E1 live VGG term on %s: %s (feeds no bar)" % (nm, fmt(sv, 6)))
                continue
            chk(sv is not None and sv > SIGMA_PRIOR,
                "E1 STRESS IS REAL: %s lifts the live VGG term to %s > floor %.6f"
                % (nm, fmt(sv, 6), SIGMA_PRIOR))
            if sv:
                note("    a LIVE composition (cVK1) would score these runs %s there; cVK2 must not move"
                     % decide(M1, _E_LV["k01"], sv * math.sqrt(2.0 / 3.0), 31.0)[0])
        d = os.path.join(t, "runs_e1")
        os.makedirs(d)
        _e_build(d)
        outs = []
        for nm, p in corpora:
            ...
            outs.append("\n".join(ln for ln in out.splitlines()
                                  if not ln.startswith("  NOTE live corpus terms")))
            chk(rc == 0 and fin == want_e1, "E1 near-bar runs (k22 leads k19 by 1.20 pp, PEAK_BAR"
                " %.4f) vs %s -> PEAK-AT-22 | CARRIER-CUT-SUPPORTED, rc 0" % (2 * se1, nm),
                "rc %s %s" % (rc, fin[:110]))
        chk(len(set(outs)) == 1 and outs[0] != "",
            "E1-INV the WHOLE score() output (minus the disclosure NOTE) is byte-identical across"
            " %d corpora" % len(corpora), "%d distinct" % len(set(outs)))
        cases = [
            ("E2 one run missing", dict(drop=("k22", 56)), 2, "FINAL: INCOMPLETE"),
            ("E3 a truncated later resubmission shadows a complete run", dict(shadow=("k22", 55)),
             2, "FINAL: INCOMPLETE"),
            ("E4 a complete later resubmission (higher job id kept)", dict(dup=("k13", 57)),
             0, want_e1),
            ("E5 AUGMENT=0 on one run's ENV line", dict(env_break=("k16", 55)),
             1, "FINAL: PEAK-AT-22 | CARRIER-CUT-SUPPORTED | A2-A4-REFUTED | GATES-2-FAIL"),
            ("E6 one seed 6 pp off its arm", dict(over={("k19", 57): 66.0}),
             1, "FINAL: UNRESOLVED-DIVERGED | CARRIER-CUT-NOT-TESTED"),
            ("E7 flat sweep", dict(levels={"k13": 50.0, "k16": 50.5, "k19": 51.0, "k20": 50.2,
                                           "k21": 50.8, "k22": 51.5, "k23": 50.1}),
             0, "FINAL: NO-PEAK-FLAT | CARRIER-CUT-REFUTED | POSITION-NOT-LOCALISING"),
            ("E8 19 and 22 tied", dict(levels={"k22": 60.5}),
             0, "FINAL: UNRESOLVED-TIE-WITH-22 | CARRIER-CUT-UNRESOLVED | TIE-19-22"),
            ("E9 in-batch noise 1.5 pp dominates the pins", dict(jit=(0.0, 1.5, -1.5)),
             0, "FINAL: UNRESOLVED-TIE-WITH-22 | CARRIER-CUT-UNRESOLVED | TIE-19-21-22"),
            ("E10 every cut floored", dict(levels=dict(("k%d" % k, 35.3) for k in GRID)),
             0, "FINAL: ALL-CUTS-FLOORED | CARRIER-CUT-REFUTED | NO-CUT-RESCUES"),
            ("E11 A2's peak at 16", dict(levels={"k16": 64.0, "k22": 55.0}),
             0, "FINAL: PEAK-ELSEWHERE-16 | CARRIER-CUT-REFUTED | PARAM-BALANCE-FAVOURED"),
        ]
        for label, kw, want_rc, want_fin in cases:
            d = os.path.join(t, "runs_" + label.split()[0])
            os.makedirs(d)
            _e_build(d, **kw)
            rc, fin, out = _e_score(d, csvpath if csvpath and os.path.exists(csvpath) else None)
            ok = rc == want_rc and fin.startswith(want_fin)
            if label.startswith("E9"):
                ok = ok and "SIGMA-INBATCH" in fin
            chk(ok, "%s -> %s, rc %d" % (label, want_fin[7:], want_rc), "rc %s %s" % (rc, fin[:110]))
