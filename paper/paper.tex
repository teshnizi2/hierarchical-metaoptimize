...
audit executes \textbf{628 claim-carrying assertions covering 411 of the 982 distinct
...
identically in all three readings below. In the \textbf{as-published} reading alone it does
\textbf{not} also cancel the step-size clip box or the GPU class, because
...
In the table, $\se$ is the standard error of the mean of the $n$ \emph{per-seed paired
differences} $\delta_s$ --- the estimator the registered scorer's \texttt{band\_flat()} gate
consumes. It is \textbf{not} the Welch two-sample $\se$ used for the level $\Dstat(300)$ two
paragraphs below, and the two happen to print almost the same number here ($0.0927$ against
$0.093$) while estimating different things. Wherever this paper writes $\pm$ against a
\emph{slope} it is the paired-difference $\se$; against a \emph{level}, the Welch $\se$.

\textbf{The registered bar is more permissive than $0.05$, and we did not say so until now.}
$|t| \ge 2.0$ on $5$ degrees of freedom is a two-sided test of size
$\alpha = 0.102$, not $0.05$ --- about twice the conventional threshold. Two things bound the
damage, and both are checkable. First, the bar was \textbf{not shopped}: it is the parent
registration's \texttt{CONFIRM\_T}, frozen in \texttt{band\_flat()} before \arm{hz3q} existed,
and the two readings it declares \texttt{FLAT} ($t\ -1.42$, $t\ -1.94$) are flat under either
threshold, so its permissiveness never decided them. Second, the repaired reading clears a strict
$0.05$ as well --- but \emph{only just}, and the margin is the honest number to print:
$|t| = 2.5721$ against a size-$0.05$ critical value of $2.5706$ at $5$ df, giving
$p = 0.0499$ and a $95\%$ upper limit of $-0.0001$ pp. The withdrawal survives the stricter
threshold by fifteen ten-thousandths of a $t$. A reader who prefers $0.05$ to our registered bar
reaches the same verdict, and by that margin only.
...
cannot support, which only more runs would move. T9--T13 are \textbf{limits of the review process}
