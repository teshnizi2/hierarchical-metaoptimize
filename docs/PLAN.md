...
8. **Write the pre-registration and the cut list.** One page in the repo, dated: primary metric, seed policy, effect size that counts (Δ ≥ 0.5pp, 95% paired CI excluding 0 **[AMENDED, CORRECTIONS 115 — STANDING RULE 14.]** A 95% *paired* CI is admissible ONLY when both arms of the contrast are drawn from the SAME submission batch. A CROSS-BATCH contrast must be Welch-unpaired, and its interval must additionally carry a batch floor of sqrt(2/k_eff)·sd_batch, where sd_batch ≈ 0.21 pp (two-way batch × seed ANOVA on 42 complete cells: BATCH F(62,85)=5.47, p=6.9e-13; SEED F(60,85)=1.21, p=0.213 — seed is statistically NULL) and k_eff is the harmonic-mean number of distinct batches contributing to each arm. The floor is ZERO when the two arms have MATCHED batch composition, and it does not shrink with n otherwise. Pooling already-differenced within-batch contrasts across batches remains a legitimate meta-analysis and is NOT penalised.), which analyses are exploratory (Gate 0/1) vs confirmatory (Phase 2+), and the Phase-3-onward items you have deleted. **[RULING on the "pre-register before looking" issue]** You have already looked. Calling Phase 2 pre-registered is honest; calling Gate 0/1 pre-registered is not. Say which is which.
