...
Updated 8 Sep 2026 (**cycle 142**). Detail lives here; chat stays short.
Authority: `docs/CORRECTIONS.md` (highest number wins, now **173**) > `docs/FINDINGS.md` > everything else.
...
Draft = `paper/paper.tex` + `paper/DRAFT-v4.md` (**76 pp**). Corpus = **2,680 rows** (**+18: `crn1` INGESTED this cycle** — 18 added, 0 changed, 0 removed). **`alice`: `crn1` 18/18 COMPLETED, SCORED, INGESTED (173); queue now EMPTY.** **`alice2`: `cru1` 47/60 — MID-FLIGHT, NOT TOUCHED, 0 rows before and 0 after.** `crn1` cost **13.4667 GPU-h** (18 jobs, ingested `wallclock_min`). **Nothing outside `hmo-crn1` was modified on `alice`** (two files written, both under `hmo-crn1/_stage/rednorm_verify/`), **and no job this campaign did not submit was touched on either account.**
**`c98_reproduce.py` EXITS 1** — reported as-is, **author scope, deliberately not fixed**. **10 checks fail, the SAME 10 as before this ingest**: only the derived counts move — rows 2,662 → **2,680** (paper 2,177), admissible 2,220 → **2,238**, wallclock-carrying 2,647 → **2,665**, GPU-h 2,843.83 → **2,857.30**. **This ingest introduced no new failing check.** 628 `chk()` sites, 411 distinct quantity numerals, 41.9% coverage.
