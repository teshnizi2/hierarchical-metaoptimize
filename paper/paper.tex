...
\item \textbf{An instrument that does not share an arm with $\Dstat$ does not settle it either.}
  ...
  under a tenth of $-0.173$, so the artefact is not the explanation. (An earlier version of this paper printed
...
$-0.044 \pm 0.070$ ($t\ -0.63$) to the $-0.173 \pm 0.051$ above. The paper is weaker here than the
...
$+2.80$ pp --- more than three times the error of simply predicting the corpus mean. A covariate that
...
The deposit is \textbf{6.3 MB in 137 files}, with \texttt{MANIFEST.md5} covering every one of them
...
is 6.3 MB, carries an md5 manifest for every file, and re-derives every number
