...
- **Judgement (266.11): CONTINUE, narrowly, in this order** — 1 register the frozen `cst1` successor scorer (**ZERO GPU**, unblocks ~41 pp already paid for); 2 `cwd1` (S1b) to separate the WD routes `cmo1` opened; 3 `csv1` (N1) for carrier necessity; 4 `cwd2` (N4); 5 `cvt10` (R1, L3 unmoved); 6 the newly live S4 CTL-at-M9 arm. **Still in flight, not landed:** `cwd1` / `cwd2` (0 of 24 started at 260.11 / 261.11), `csv1` (18 jobs, RULE 20 owed), `cvt10` (18 of 30 queued, RULE 20 owed). *[SUPERSEDED at cycle 154's close, CORRECTIONS **267**, state re-read from `sacct` at 2026-09-18T21:46Z: the queue drained while 264–266 were being written. `cvt10` **30/30 COMPLETED** (25.76 GPU-h) and `cwd1` **9/9 COMPLETED** (7.64); `csv1` **14/18 COMPLETED + 4 RUNNING**; `cwd2` **15 RUNNING**, none finished. Nothing cancelled or failed. **RULE 20 at FULL coverage is now CLOSED and PASSING for `cvt10` (30/30), `cwd1` (9/9) and `cwd2` (15/15)** with `argsline_guard.py` `81cea8b5…` unedited; `cwd2`'s first armed-helper log ends `FAIL` on **one** run and that FAIL is a **33-second race**, withdrawn and re-run PASS at 267.2 — do not read it out of context. `csv1`'s batch-consistency half is PASS at 18/18; its registered witness-value audit is **still owed by Track F before any `csv1` number is read**. No batch has been scored and no number of any of the four has been read.]*
