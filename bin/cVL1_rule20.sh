#!/bin/bash
# cVL1_rule20.sh -- CORRECTIONS 303: RULE 20 at current coverage for `cvl1` (a copy of cGW1_rule20.sh with cvl1's
# table; the ENV half is analysis/cvl1_rule20_envaudit.py, which ALSO checks each run's VAL_SPLIT witness line).
...
#   bash bin/cVL1_rule20.sh            (run from ~/stage_cvl1 on alice2)
set +u
...
WS=$METAOPT_WS; R=$WS/runs; P=cvl1
S="$(cd "$(dirname "$0")/.." && pwd)"
...
for f in $(ls $R/$P-*.out 2>/dev/null); do
  ...
  n=$((n+1))
  case $arm in
    *W1) WD=0.1;  SOK="184 185 186 187" ;;
    *W4) WD=5e-4; SOK="184 185 186 187" ;;
...
python3 $S/analysis/cvl1_rule20_envaudit.py $R | sed 's/^/  /'
