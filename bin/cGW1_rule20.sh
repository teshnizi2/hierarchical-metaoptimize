#!/bin/bash
# cGW1_rule20.sh -- CORRECTIONS 291: RULE 20 at current coverage for `cgw1` (a copy of cWD5_rule20.sh with cgw1's
# table; the ENV half is analysis/cgw1_rule20_envaudit.py).  READ-ONLY: no accuracy number is opened.
...
#   bash bin/cGW1_rule20.sh            (run from ~/stage_cgw1 on alice2)
set +u
...
WS=$METAOPT_WS; R=$WS/runs; P=cgw1
S="$(cd "$(dirname "$0")/.." && pwd)"
...
for f in $(ls $R/$P-*.out 2>/dev/null); do
  ...
  n=$((n+1))
  case $arm in
    *W1) WD=0.1;  SOK="152 153 154" ;;
    *W2) WD=1e-2; SOK="152 153 154" ;;
    *W4) WD=5e-4; SOK="152 153 154 155" ;;
