...
## CYCLE 159 (LANDING) — **CORRECTIONS 285: `cwd5` LANDS 27/27 — `THRESHOLD-W1-W2 | W1-COLLAPSE+W2-NOGAP+W3-NOGAP+W4-NOGAP+CAR-UNREADABLE`.  THE WEIGHT-DECAY LADDER, THE ONE EXPERIMENT §10 SAID DECIDES WHETHER THE MECHANISM LINE IS PUBLISHABLE AT ALL, IS RUN — AND IT IS THE ADVERSE, PRE-REGISTERED OUTCOME.  THE COLLAPSE IS PRESENT AT THE CAMPAIGN'S COUPLED wd 0.1 AND ABSENT AT 1e-2, 1e-3 AND 5e-4.  THE REFEREE'S CORNER-CASE CHARGE LANDS; `WRITEUP-mechanism`'s O-14 CLOSES AGAINST THE GENERALITY OF THE RESULT.  ZERO GPU SUBMITTED BY THIS CYCLE.  CORPUS 3,316 / 3407.9 GPU-h.**
...
| still owed | unchanged by this entry: the **site import of rows 237 + 238 together**; **O-13** (magnitude) is the top live item; **§10.6 R2 (decoupled) UNANSWERED by design**; `WRITEUP-mechanism` §8 **Q1 / Q2 are the professor's decisions** |
...
| what `cwd5`'s verdict does to THIS row, said rather than left implicit | **it does not move a single `cwd4` number** — no `cwd5` level enters any `cwd4` bar, sigma or contrast, and `cwd4` was scored and ingested before `cwd5` was read.  What it does is **bind the SCOPE sentence**: `cwd4`'s result is at **coupled wd 0.1**, and `cwd5` reads the collapse as existing at that rung and **not at 1e-2 / 1e-3 / 5e-4**.  Reading the two together is that landing's job, not this one's; `WRITEUP-mechanism` §10 already carries the wd-0.1 scope INLINE (283.6 **W7**) |
...
| 271 | `cwd1` `ResNet18_c100`, coupled WD off on the 20 BatchNorm scales (S1b) | 9 | `COLLAPSE-VANISHES`: `k01NWD` **70.7760** vs `k01` 22.9513, `P_NWD` **+47.8247 pp = +85.99 SE**, `G_NWD` −1.4340 pp — the scalar arm is ABOVE its own masked layerwise reference, `RATIO` 1.0207 | the mask is ONE SET and BOTH ROUTES at once, so *which tensors* not *which route*; the reference arm is also masked; no ISO arm; 7 × L4 / 2 × 2080 Ti, ungated; **no bar is near (6.43 pp = 11.57 SE)** | 7.53 |
...
- **The headline, in one line: the carrier account survives momentum but is CONDITIONAL ON COUPLED WEIGHT DECAY**, and dominance travels with collapse across the one non-collapsing cell we can reach. LIMITS-PREP §6's conditional has fired.
